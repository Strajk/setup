...
OAuth.setProperties(OAuth, {
  percentEncode: function (a) {
    ...
    if (d instanceof
      Array) {
      for (a = 0; a < d.length; ++a) if (d[a][0] == c) if (b === undefined) d.splice(a, 1); else {
        d[a][1] = b;
        b = undefined
      }
    ...
    if (!(c ==
      null || !c.length)) if (c = c[c.length - 1].src) {
      var b = c.indexOf("?");
      if (!(b < 0)) {
...
OAuth.setProperties(OAuth.SignatureMethod, {
  sign: function (a, c) {
    ...
    return OAuth.percentEncode(a.method.toUpperCase()) +
      "&" + OAuth.percentEncode(OAuth.SignatureMethod.normalizeUrl(c)) + "&" + OAuth.percentEncode(OAuth.SignatureMethod.normalizeParameters(b))
    ...
    for (var b = 0; b < c.length; ++b) {
      var d = c[b];
      d[0] != "oauth_signature" && a.push([OAuth.percentEncode(d[0]) + " " + OAuth.percentEncode(d[1]),
        d])
    }
    a.sort(function (e, f) {
      if (e[0] < f[0]) return -1;
