...
function str2binb(a) {
  for (var c = [], b = (1 << chrsz) - 1, d = 0; d < a.length * chrsz; d += chrsz) c[d >> 5] |= (a.charCodeAt(d / chrsz) & b) << 32 - chrsz - d % 32;
  return c
}
...
function binb2hex(a) {
  for (var c = hexcase ? "0123456789ABCDEF" : "0123456789abcdef", b = "", d = 0; d < a.length * 4; d++) b += c.charAt(a[d >> 2] >> (3 - d % 4) * 8 + 4 & 15) + c.charAt(a[d >> 2] >> (3 - d % 4) * 8 & 15);
  return b
}

function binb2b64(a) {
  for (var c = "", b = 0; b < a.length * 4; b += 3) for (var d = (a[b >> 2] >> 8 * (3 - b % 4) & 255) << 16 | (a[b + 1 >> 2] >> 8 * (3 - (b + 1) % 4) & 255) << 8 | a[b + 2 >> 2] >> 8 * (3 - (b + 2) % 4) & 255, e = 0; e < 4; e++) c += b * 8 + e * 6 > a.length * 32 ? b64pad : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(d >> 6 * (3 - e) & 63);
  return c
};
