...
function log(a, c) {
  try {
    var b = logLevels.indexOf(c), d = logLevels.indexOf(D.logLevel);
...
function forEach(a, c, b) {
  if (a.length !== undefined) for (var d = 0, e = a.length; d < e; d++) c.call(b, a[d], d); else for (d in a) c.call(b, a[d], d)
}
...
if (!Array.prototype.indexOf) Array.prototype.indexOf = function (a, c) {
  var b = this.length, d = Number(c) || 0;
...
function unique(a, c) {
  for (var b = [], d = 0, e = a.length, f; f = a[d], d < e; d++) if (c) find(b, function (g) {
    return c(g, f)
  }) || b.push(f); else b.indexOf(f) == -1 && b.push(f);
  return b
}
...
function gsub(a, c, b) {
  var d = "";
  a = a;
  for (var e; a.length > 0;) if (e = a.match(c)) {
    d += a.slice(0, e.index);
    d += str_interpret(b(e));
...
var Utils = {
  parseColor: function (a) {
    ...
    build: function () {
      var a = [], c = arguments, b, d, e = 0, f;
      for (c = c[0] instanceof Array ? c[0] : c; e < c.length; e++) if (c[e + 1] instanceof Object) {
