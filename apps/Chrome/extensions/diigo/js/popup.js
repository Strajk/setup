var Bg = chrome.extension.getBackgroundPage(), permissions = Bg.GlobalData ? Bg.GlobalData.permissions || {} : {};
...
$(document).ready(function () {
  ...
  function x(a, b) {
    if (y !== true) {
      ...
      if (a.generated == true) {
        ...
        } else getSelectedTab(function (d) {
  ...
  function z(a, b) {
    var c = $("#diigobm-tag-input"), d = ParseTags.parseTags(c.val(), true), f = $.inArray(a, d);
    if (b === undefined) f >= 0 ?
      d.splice(f, 1) : d.push(a); else if (b) f == -1 && d.push(a); else f >= 0 && d.splice(f, 1);
  ...
  function s() {
    var a = $("#diigobm-tag-input"), b = ParseTags.parseTags(a.val(), true);
  ...
  function q(a, b) {
    if (a == "recommended") {
      ...
      if (b.length == 0) return; else $("#diigobm-recommend-tag").show();
      b = arrayUnique(b)
    }
    ...
    if (a == "recommended") $(c).appendTo($("#diigobm-recommend-tag")); else if (a == "group") {
  ...
  function r() {
    ...
    if (a !=
      ...
      var c = {url: a.url, mode: a.mode, title: a.title, tags: a.tags, description: a.description, unread: a.unread, urlId: popCtx.bm.urlId, cache: a.cache, groups: a.shareGroups, shareExistingAnnotations: a.shareAnnotations, lists: a.shareLists};
      getSelectedTab(function (d) {
        c.tabId = d.id;
  ...
  function H(a) {
    if (a.length == 0) {
      ...
      var d = c.maxCount;
      a = c.first;
      var f = c.second, g = c.third, e = b.length;
  ...
  function A(a,
    ...
    if (d.length) {
      ...
      forEach(d, function (e) {
        ...
        $('<div class="menu-item" data-id="' + e.name + '">' + h + "</div>").on("click", function () {
          console.log("Click");
          var i = e.name;
          if (popCtx.groupTagsDict[i] !=
            undefined) q("group", popCtx.groupTagsDict[i]); else {
            $("#diigobm-group-tag").show();
            getSelectedTab(function (l) {
              chrome.tabs.sendMessage(l.id, {name: "loadGroupTagsDictionary", groupName: i})
            })
          }
        }).appendTo(f)
      })
    ...
    if (c.length) {
      ...
      forEach(c, function (e) {
        ...
        $('<div class="menu-item" data-id="' + e.name + '">' + h + "</div>").on("click", function () {
          console.log("Click");
          var i = e.name;
          if (popCtx.groupTagsDict[i] != undefined) q("group",
            popCtx.groupTagsDict[i]); else {
            $("#diigobm-group-tag").show();
            getSelectedTab(function (l) {
              chrome.tabs.sendMessage(l.id, {name: "loadGroupTagsDictionary", groupName: i})
            })
    ...
    if (d.length > 0) {
      ...
      g.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
      forEach(d, function (e) {
        b ? g.append(Utils.dom.buildOne("option", {value: e.name}, [e.displayName])) :
          g.append(Utils.dom.buildOne("option", {value: e.name}, [e.displayName + (t(e.name) ? " (shared)" : "")]))
      });
      g.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]))
    }
    g.append(Utils.dom.buildOne("option", {value: 0}, ["Share to a Group"]));
    g.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
    $(Utils.dom.buildOne("option", {value: -2}, ["Create a Group..."])).appendTo(g);
    g.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
    forEach(c, function (e) {
      b ? g.append(Utils.dom.buildOne("option",
        {value: e.name}, [e.displayName])) : g.append(Utils.dom.buildOne("option", {value: e.name}, [e.displayName + (t(e.name) ? " (shared)" : "")]))
    });
    g.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
    ...
    g.change(function () {
      ...
      if (e != 0 && e != -1 && e != -2 && e != -3) {
        ...
        console.log(popCtx);
        if (popCtx.groupTagsDict[e] != undefined) q("group", popCtx.groupTagsDict[e]); else {
          $("#diigobm-group-tag").show();
          chrome.tabs.getSelected(null, function (h) {
            chrome.tabs.sendMessage(h.id, {name: "loadGroupTagsDictionary", groupName: e})
          })
        }
      }
    });
  ...
  function I(a, b) {
    ...
    if (c != undefined) {
      ...
      m($("#diigobm-title-input"), "change");
      $("#diigobm-url-input").val(c.url)
    } else chrome.tabs.getSelected(function (e) {
      $("#diigobm-title-input").val(e.title);
      m($("#diigobm-title-input"),
        "change");
    ...
    A(a.groups, false);
    s();
    $("#diigobm-recommend-tag").find(".diigo-tag").length == 0 && getSelectedTab(function (e) {
    ...
    if (localStorage.openedOutlinerId && localStorage["prefs.outlinerIcon"] == "true") {
      ...
      var f = $("#diigobm-list"), g = f.find(".menu-item[data-id=" + d + "]").text();
  ...
  function K() {
    $("#search-input").on("keydown", function (f) {
      if (f.which == 13) {
        f = $("#search-input").val();
        D(f)
      }
    ...
    if (a.tab == "recent") L("recent"); else a.tab == "search" && L("search", a.key);
    a = j.GlobalData.myTags;
    for (var b = [], c = 0; c < a.length; c++) b.push(a[c]);
    b = removeFromArray("no_tag", b);
    console.log(b);
    a = {resultsClass: "diigolet ac_results ac_search", data: b, mode: "multiple", multipleSeparator: " ,", id: "diigolet-ac", moveToSelect: true};
    try {
  ...
  function E(a) {
    ...
    if (a == "recentTab") $("#recent-result-iframe").attr({src: "https://www.diigo.com/chrome/items"});
    else a == "unreadTab" && $("#recent-result-iframe").attr({src: "https://www.diigo.com/chrome/items?read=no"})
  }
  ...
  function D(a) {
    if (a == "") $("#recent-result").hasClass("current") ||
    F("recent"); else {
      ...
      if (localStorage.search_type === "meta") {
        ...
        $("#viewAllLink").attr("data-href",
          "https://www.diigo.com/search?adSScope=my&what=" + encodeURIComponent(a) + "&snapshot=yes")
      }
  ...
  function M(a) {
    ...
    u({list: a.lists ? a.lists : j.GlobalData.myBmLists, outliners: a.outliners ? a.outliners : j.GlobalData.outliners}, true);
    A(b, true);
    b = a.lastUsedTags ? a.lastUsedTags : [];
    if ($("#diigobm-recent-tag").find(".diigo-tag").length == 0) if (b.length > 0) {
    ...
    chrome.storage.local.get("researchModeData", function (g) {
      if (g.researchModeData) J(g); else {
  ...
  function v(a) {
    ...
    if (a) {
      ...
      $("#research-mode-close-notify").on("click",
        function () {
          $("#research-mode-notification").hide();
    ...
    $("#link-icon").on("click", function () {
      var e = $("#diigobm-url");
      if (b) e.toggleClass("mac-unfold"); else e.hasClass("unfold") ? e.addClass("fold").removeClass("unfold") : e.addClass("unfold").removeClass("fold")
    });
    ...
    $("#diigobm-options,#Diigo-Bookmark-checkShareExisting").on("click",
      ".op-checkbox-container", function () {
        $(this).toggleClass("checked")
      });
    $("#bm-main").on("keydown", function (e) {
      if (e.keyCode == 13) e.target.id == "diigobm-tag-input" && !$("#diigolet-ac").is(":visible") && r(); else if (e.keyCode == 83 && (e.ctrlKey || e.metaKey)) r(); else e.keyCode == 27 && window.close()
    });
    ...
    a = {resultsClass: "diigolet ac_results", data: a, mode: "multiple", multipleSeparator: " ,", id: "diigolet-ac"};
    try {
    ...
    $("#diigobm-list-addBtn").on("click", function () {
      console.log("click");
      if (!$(this).parent().hasClass("processing")) if (j.GlobalData.permissions.createOutliner) {
        var e = $("#diigobm-list-addInput").val(), h = $("#diigobm-list-add .diigo-alert-tip"), i = [], l = popCtx.lists.length, k;
        for (k = 0; k < l; k++) i.push(popCtx.lists[k].title);
        if (e.match(/^\s*$/)) {
          h.show().find("span").text("Input a name");
          $("#diigobm-list-addInput").focus()
        } else if ($.inArray(e, i) !== -1) h.show().find("span").text("Name existed !"); else {
  ...
  var p, y = false, C, j = chrome.extension.getBackgroundPage();
  getSelectedTab(function (a) {
    chrome.browserAction.getBadgeText({tabId: a.id},
      function (b) {
        if (b == "New") {
          chrome.browserAction.setBadgeText({text: ""});
          chrome.tabs.create({url: "https://blog.diigo.com/2018/03/23/simplified-focused-research/"});
  ...
  chrome.cookies.get({url: "https://www.diigo.com", name: "diigoandlogincookie"}, function (a) {
    if (a) {
      ...
      if (p = a.value.split("-.-")[1]) {
        $("#sign-out-btn").find("span").text("Sign Out (" +
          p + ")");
        $("#userInfo").find(".username").text(p).attr("title", p).on("click", function () {
        })
      }
      ...
      $("[data-link]").click(function () {
        var b, c = $(this).attr("data-link");
        if (c != "more") if (c ===
          "tutorial") getSelectedTab(function (d) {
          ...
          switch (c) {
            ...
            case "upgrade":
              ...
              j.sendAnalysisData({gcc_cookie_ad_click_from_extension_old: 1});
              break;
            case "feedback":
              b = "/feedback.html";
              break
          }
          c == "feedback" ? getSelectedTab(function (d) {
            b =
              b + "?refer=" + encodeURIComponent(d.url);
  ...
  chrome.runtime.onMessage.addListener(function (a, b) {
    if (a.name == "isSaved") {
      ...
      console.log(a.data);
      q("group", a.data)
    } else if (a.name == "popupclose") window.close(); else if (a.name == "shownew") window.close(); else if (a.name == "updateLists") u(a.data); else if (a.name == "updateGroups") A(a.data); else if (a.name == "updateGroupTag") {
      ...
      c = a.newlist;
      var d = $("#diigobm-list");
      d.find(".content").text(c.title).attr("data-id",
        c.id);
  ...
  $("#sign-up-btn").click(function () {
    var a = "https://www.diigo.com/account/thirdparty/openid?openid_url=https://www.google.com/accounts/o8/id&redirect_url=" +
      encodeURIComponent("https://www.diigo.com/chrome_diigo_extension_done") + "&request_from=chrome_diigo_extension";
    window.open(a)
  });
  $("#bookmark").on("click", "div", function (a) {
    switch (a.target.id) {
      case "bookmark-save":
        getSelectedTab(function (b) {
          var c = b.url.substring(b.url.length - 4) == ".pdf", d = popCtx.permissions.pdfPermission.pdfPermission;
          if (c && d && 0) chrome.tabs.create({url: "/pdf/viewer.html?file=" + b.url + "&save=true"}); else {
            o(b.id);
            $("#main").hide();
            $("#bm-main").show();
  ...
  $("#toggle-outliner").on("click", function () {
    getSelectedTab(function (a) {
      chrome.tabs.executeScript(a.id, {file: "js/checkTab.js"}, function () {
      });
  ...
  $("#save-bookmark").on("click", function () {
    $(this).hasClass("disabled") || chrome.tabs.getSelected(null, function (a) {
  ...
  $("ul").on("click", "li", function (a) {
    ...
    switch (a) {
      ...
      case "read-later":
        getSelectedTab(function (c) {
          o(c.id);
          var d = $("#read-later").hasClass("unread") ? true :
            false;
        ...
        window.addEventListener("message", function (c) {
          ...
          if (d == "openUrl") {
            ...
            chrome.tabs.getSelected(null, function () {
              c.data.ctrl ? chrome.tabs.create({url: f, active: false}) : chrome.tabs.create({url: f})
            })
          } else if (d ==
            "searchTag") {
  ...
  $("#search").on("click", function () {
    ...
    window.addEventListener("message", function (a) {
      ...
      if (b == "openUrl") {
        var c = a.data.url;
        chrome.tabs.getSelected(null, function () {
          a.data.ctrl ? chrome.tabs.create({url: c, active: false}) :
  ...
  $("#share").on("click", function () {
    $(this).hasClass("disbaled") || getSelectedTab(function (a) {
      o(a.id);
  ...
  $("#read-later").on("click", function () {
    $(this).hasClass("disbaled") || getSelectedTab(function (a) {
      o(a.id);
      var b = $("#read-later").hasClass("unread") ? true :
        false;
  ...
  $("#screenshot-main").find(".back").on("click", function () {
  ...
  $("#research-mode-checkbox").on("change", function () {
    if ($(this).prop("checked")) if (!j.GlobalData.permissions.autoShowAnnotation) {
