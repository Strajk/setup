var popCtx = {groupTagsDict: {}}, username, inResearchMode = false, ac, GlobalData = null, _url = getParameterByName("url"), _title = getParameterByName("title");
...
function receiveMessage(a) {
  ...
  switch (a.type) {
    ...
    case "listCreateSuccessAndUpdateLists":
      ...
      a = a.newlist;
      var b = $("#diigobm-list");
      b.find(".content").text(a.title).attr("data-id",
        a.id);
...
chrome.runtime.onMessage.addListener(function (a) {
  if (a.name == "updateLists") updateLists(a.data); else if (a.name == "updateGroups") updateGroups(a.data); else if (a.name == "listCreateSuccessAndUpdateLists") {
    updateLists({list: a.lists, outliners: a.outliners});
    a = a.newlist;
    var b = $("#diigobm-list");
...
function toggleTag(a, b) {
  var d = $("#diigobm-tag-input"), e = ParseTags.parseTags(d.val(), true), g = $.inArray(a, e);
  if (b === undefined) g >= 0 ? e.splice(g, 1) : e.push(a); else if (b) g == -1 && e.push(a); else g >= 0 && e.splice(g, 1);
...
function showTags(a, b) {
  ...
  if (a == "recommended") $(d).appendTo($("#diigobm-recommend-tag")); else if (a == "group") {
...
function formToObject() {
  ...
  if (!isURL(b) || b.match(/^\s*$/)) if (a == true) {
    a = false;
  ...
  if (a != false) {
    ...
    chrome.runtime.sendMessage({
      ...
      data: {url: a.url, mode: a.mode, title: a.title, tags: a.tags, description: a.description, unread: a.unread, urlId: popCtx.bm.urlId, cache: a.cache, groups: a.shareGroups, shareExistingAnnotations: a.shareAnnotations, lists: a.shareLists}
    });
    setTimeout(function () {
...
function updateBookmark(a) {
  if (inResearchMode !== true) if (a.generated == true) {
    var b = a.bm;
...
function initBookmarkWindow(a) {
  ...
  $("#link-icon").on("click", function () {
    var c = $("#diigobm-url");
    if (b) c.toggleClass("mac-unfold"); else c.hasClass("unfold") ? c.addClass("fold").removeClass("unfold") : c.addClass("unfold").removeClass("fold")
  });
  autoTextarea.init(document.getElementById("diigobm-title-input"), 40);
  autoTextarea.init(document.getElementById("diigobm-des-input"),
    80);
  ...
  $("#recommend-tag").on("click", function (c) {
    c.preventDefault();
    $("#diigobm-recommend-tag").find(".diigo-tag").each(function () {
      $(this).trigger("click")
    })
  });
  $("#diigobm-recent-tag").on("click", "a", function () {
    $("#diigobm-recent-tag").find(".diigo-tag").each(function () {
      toggleTag($(this).addClass("selected").text(),
        true)
    })
  });
  ...
  $("#bm-main").on("keydown", function (c) {
    if (c.keyCode == 13) c.target.id == "diigobm-tag-input" && !$("#diigolet-ac").is(":visible") && formToObject(); else if (c.keyCode == 83 && (c.ctrlKey || c.metaKey)) {
      c.preventDefault();
      formToObject()
  ...
  $("#diigobm-list-addBtn").on("click", function () {
    if (!$(this).parent().hasClass("processing")) if (GlobalData.permissions.createOutliner) {
      var c = $("#diigobm-list-addInput").val(), h = $("#diigobm-list-add .diigo-alert-tip"), i = [], k = popCtx.lists.length, j;
  ...
  $("#bottom").on("click", function (c) {
    switch (c.target.id) {
      case "diigobm-saveBtn":
        isInResearchMode() ?
          saveResearchMode(formToObject) : formToObject();
        break;
...
function objectToform(a) {
  ...
  } else chrome.tabs.getSelected(function (g) {
  ...
  if (b.description.length > 0) {
    ...
    triggerEvent($("#diigobm-des-input"),
      "change")
  } else if (a.selection) {
  ...
  updateTagStatus();
  $("#diigobm-recommend-tag").find(".diigo-tag").length == 0 && sendMessage({type: "getRecommendedTags"});
...
function updateLists(a, b) {
  ...
  var d = a.list, e = a.outliners, g = $("#diigobm-list").find(".item-container");
  g.empty();
  if (e.length) {
  ...
  $(Utils.dom.buildOne("option", {value: -2}, ["Create an Outliner..."])).appendTo(f);
  f.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
  forEach(e, function (c) {
    b ? f.append(Utils.dom.buildOne("option",
      {value: c.id}, [c.title])) : f.append(Utils.dom.buildOne("option", {value: c.id}, [c.title + (inList(c.id) ? " (added)" : "")]))
  });
  f.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
  if (d.length) {
    ...
    f.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
    forEach(d, function (c) {
      b ? f.append(Utils.dom.buildOne("option", {value: c.id}, [c.title])) : f.append(Utils.dom.buildOne("option", {value: c.id}, [c.title + (inList(c.id) ?
        " (added)" : "")]))
    })
  }
  f.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
...
function updateGroups(a, b) {
  ...
  if (e.length) {
    ...
    forEach(e, function (c) {
      ...
      $('<div class="menu-item" data-id="' + c.name + '">' + h + "</div>").on("click", function () {
        var i = c.name;
        if (popCtx.groupTagsDict[i] !=
          undefined) showTags("group", popCtx.groupTagsDict[i]); else {
  ...
  if (e.length > 0) {
    ...
    forEach(e, function (c) {
      b ? f.append(Utils.dom.buildOne("option", {value: c.name}, [c.displayName])) : f.append(Utils.dom.buildOne("option", {value: c.name}, [c.displayName + (inGroup(c.name) ? " (shared)" :
        "")]))
    });
    f.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]))
  }
  ...
  f.append(Utils.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
  $(Utils.dom.buildOne("option", {value: -2}, ["Create a Group..."])).appendTo(f);
  f.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
  forEach(d, function (c) {
    b ? f.append(Utils.dom.buildOne("option", {value: c.name}, [c.displayName])) : f.append(Utils.dom.buildOne("option",
      {value: c.name}, [c.displayName + (inGroup(c.name) ? " (shared)" : "")]))
  });
  f.append(Utils.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
  $(Utils.dom.buildOne("option", {value: -3}, ["Refresh"])).appendTo(f);
  f.change(function () {
    ...
    if (c != 0 && c != -1 && c != -2 && c != -3) {
      ...
      if (popCtx.groupTagsDict[c] != undefined) showTags("group", popCtx.groupTagsDict[c]); else {
        $("#diigobm-group-tag").show();
        chrome.tabs.getSelected(null, function (h) {
          chrome.tabs.sendMessage(h.id, {name: "loadGroupTagsDictionary", groupName: c})
        })
      }
    }
  });
...
function fillReasearchModeData(a, b) {
  ...
  if (d.outliner) {
    ...
    g.find(".content").text(e).attr("data-id",
      d.outliner).end().addClass("inputed")
  }
  if (d.group) {
    g = $("#diigobm-group");
...
function buildTagCloud(a) {
  if (a.length == 0) {
    ...
    var e = d.maxCount;
    a = d.first;
    var g = d.second, f = d.third, c = b.length;
