...
Utils.lang = {
  isArray: function (a) {
    ...
    for (var a = [].slice.call(arguments), b = a.shift(), g = a.pop(), i = 0, j = a.length, k; k = a[i], i < j; i++) for (var q in k) {
      var h =
        g(q);
    ...
    if (a.forEach) a.forEach(b, g); else for (var i in a) b.call(g, a[i], i)
  }
};
...
Utils.array = {
  findIndex: function (a, b, g) {
    ...
    for (var g = [], i = 0, j = a.length, k; k = a[i], i < j; i++) if (b) this.find(g, function (q) {
      return b(q, k)
    }) || g.push(k); else g.indexOf(k) == -1 && g.push(k);
    return g
  }, reverse: function (a) {
...
Utils.string = {
  md5: function (a) {
    ...
    for (a = 0; a < h.length; a += 16) {
      ...
      d =
        k(d, e, f, c, h[a + 13], 21, 1309151649);
    ...
    if (b.toTemplateReplacements) b =
      b.toTemplateReplacements();
    return a.replace(/(^|.|\r|\n)(#\{(.*?)\})/g, function (g, i, j, k) {
      if (b == null) return "";
      g = i || "";
      if (g == "\\") return j;
    ...
    Utils.each(a, function (i, j) {
      if (j) if (i && typeof i == "object") Utils.isArray(i) && Utils.each(i,
        function (k) {
          b(j, k)
        }); else b(j, i)
    });
...
Utils.dom = {
  build2: function () {
    var a = [], b = arguments, g, i, j = 0, k;
    for (b = b[0] instanceof Array ? b[0] : b; j < b.length; j++) if (b[j + 1] instanceof Object) {
...
function parseDomain(a) {
  /^[^:\/?#]+:\/\//.test(a) || (a = "http://" + a);
  return a.match(/^(([^:\/?#]+):)?(\/\/([^\/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?/)[4].toLowerCase().trim()
}

function isHomePage() {
  var a = window.location.pathname.toLowerCase();
  if (!a || a == "/") return true;
  return (a = a.match(/^\/([^\.]+)\.(.*)$/)) && a.length == 3 ? $.inArray(a[1], ["default", "index", "home"]) > -1 && $.inArray(a[2], ["htm", "html", "shtml", "php", "jsp", "asp", "aspx", "cfm"]) > -1 : false
};
