var CrossPromotion = function () {
  var g = {
    messages: null, lastShownIndex: -1, lastLoadedAt: null, requestCounter: 0, init: function () {
      ...
      Prefs.setUserData({readMsgs: JSON.stringify(b)})
      ...
      for (e = b.length; a < e; a++) {
        f = b[a];
        c = 0;
        for (d = this.messages.length; c < d; c++) if (this.messages[c].id == f) {
  ...
  extend(g, {
    pickMessage: function () {
      ...
      if (++this.lastShownIndex >= this.messages.length) this.lastShownIndex =
        0;
