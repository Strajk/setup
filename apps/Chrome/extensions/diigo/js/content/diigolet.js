window.diigolet == undefined && function () {
  function Bb(a) {
  ...
  function I() {
    ...
    a.unshift("[Diigolet]");
    if (x.logLevel != "never") try {
      var c = Eb.indexOf("debug"), d = Eb.indexOf(x.logLevel);
  ...
  function M(a, c, d) {
    if (a.length !== undefined) for (var f = 0, h = a.length; f < h; f++) c.call(d, a[f], f); else for (f in a) c.call(d, a[f], f)
  }
  ...
  function Hb(a, c) {
    for (var d = [], f = 0, h = a.length, i; i = a[f], f < h; f++) if (c) Pa(d, function (l) {
      return c(l, i)
    }) || d.push(i); else d.indexOf(i) == -1 && d.push(i);
    return d
  }
  ...
  function fb(a, c, d) {
    var f = "";
    a = a;
    for (var h; a.length > 0;) if (h = a.match(c)) {
      f += a.slice(0, h.index);
      f = f;
      var i = d(h);
      f = f + (i == null ? "" : String(i));
  ...
  function rb() {
    M([U, H, da, ga,
      ...
      if (a.j) {
        ...
        a.j.unbind(".pay4sin").bind("mouseup.pay4sin", c).bind("mousedown.pay4sin", c).bind("keydown.pay4sin", function (d) {
          d.stopPropagation()
        })
      }
    });
    sb.mousedown(function (a) {
      if (a.target.tagName == "INPUT" || a.target.tagName == "TEXTAREA") tb = true; else if (a.target.id == "diigolet-csm-dropdown" || a.target.className == "diigolet-csm-coloritem" || a.target.parentNode.id == "diigolet-panel-btnHighlight") Ra = true;
      window.getSelection();
      mousedownTop =
        a.pageY;
      mousedownLeft = a.pageX
    }).mouseup(dc).mouseover(ec).mouseout(fc).mousemove(function (a) {
  ...
  function ub(a) {
    var c;
    if (a == "myLibrary") c = x.urls.getUserBookmarksPageURL(); else if (a == "signIn") c = x.urls.getSignInURL() + "?referInfo=" + encodeURIComponent("/images/diigo-logo.png#SIGNED_IN"); else if (a == "signOut") c = x.urls.getSignOutURL();
    else if (a == "feedback") c = x.urls.getFeedbackURL(); else if (a == "followOnTwitter") c = x.urls.getFollowOnTwitterURL(); else if (a == "rating") c = x.urls.getRatingURL();
    c && x.openURL(c)
  }
  ...
  function Mb() {
    ...
    a = {
      resultsClass: "diigolet ac_results", data: a, mode: "multiple", multipleSeparator: " ,", onItemSelect: function () {
        U.clickedOn = true
      }, id: "diigolet-ac"
    };
    try {
      new Nb("#Diigo-Bookmark-Tag-Input",
        a)
    } catch (f) {
      I(f)
    }
    ...
    if (!k.draggingFloatNote) {
      ...
      if (d) {
        ...
        if (d.type == ia || d.type == Wa) {
          Ca.reset(c);
          Z.scheduleShow(a, c);
          ma.showed && $(".diigoHighlight.id_" + c.id).addClass("hover").attr("draggable", true)
        } else if (d.type == fa || d.type == ANNOTATION_TYPE_ICON) {
          Ca.removeEditMode();
          Ca.ann = null;
          var f = H.ann != c;
          if (!H.isEditing()) if (f || !H.shown) {
            H.shown && H.hide();
            H.scheduleShow(a, c)
          } else H.cancelHide()
        }
        if (c.comments.length > 0 && (d.type ==
          ia || d.type == Wa)) {
          Ca.reset(c);
  ...
  function dc(a) {
    ...
    H.shown && !H.clickedOn && !H.isEditing() &&
    H.hide();
    if (!tb && !Ra && ab.isTextSelected()) k.isHighlightPen ? diigolet.handle(a, "highlight") : K.get("prefs.autoShowHighlightIcon", function (c) {
      if (c === "true") {
  ...
  function na() {
    ...
    a = function (i) {
      ...
      if (l.test(i)) return i.replace(l, function (m) {
    ...
    for (var c = "", d, f, h = /([^{}@]*)({[^{}]*})/g; d = h.exec(qb);) {
      f = d[1];
      d = d[2].replace(/\r\n/g, "\n");
      c += $.map(f.split(","), function (i) {
        if (/(^|\s)body([\s.\[#]|$)/.test(i)) return i.replace("body",
          "body#" + a);
        if (/(^|\s)html(\s|$)/.test(i)) return i.replace("html", "html body#" + a);
        return "body#" + a + " " + i.replace(/^\s+/m, "")
      }).join(",") + d + "\n"
    }
    if ($("head").find('style[id="diigolet-chrome-css"]').length == 0) document.createStyleSheet ? document.createStyleSheet().cssText = c : $("head").add(document.body).eq(0).append($(document.createElement("style")).attr({type: "text/css", id: "diigolet-chrome-css"}).text(c))
  }

  function Ma(a, c) {
    var d = gb(a), f = d.path;
    if (!/^https?$/i.test(d.protocol) && !/^ftp?$/i.test(d.protocol)) return false;
    if (/\.?diigo\./i.test(d.host)) if (/^\/annotated\//i.test(f)) return false; else if (/(player|slides?)\.diigo\./i.test(d.host) && /^\/(list|feed)/i.test(f)) return false; else if (/^\/bookmark\//i.test(f)) return false; else if (/^\/cached/i.test(f)) return false; else if (/^\/text_view/i.test(f)) return false; else if (/^\/site\/text_view/i.test(f)) return false; else if (/^\/item\/pdf/i.test(f)) return c ? true : false; else {
  ...
  function Rb() {
    ...
    var c = k.myTagsWithCount.slice(0, 101), d = c[0].count, f = c.length;
    c.sort(function (l, m) {
  ...
  var x = window.diigolet = {
    debug: false, logLevel: "never", version: "3.4.5", show: function () {
      da.show();
      J.show()
    }, callback: function () {
  ...
  E.lang = {
    isArray: function (a) {
      ...
      for (var a = [].slice.call(arguments), c = a.shift(), d = 0, f = a.length, h; h = a[d], d < f; d++) for (var i in h) c[i] = h[i];
      return c
    }, protectiveExtend: function (a,
                                   c) {
      for (var d in c) if (a[d] !== undefined) a[d] = c[d]
    }, extendFiltered: function () {
      for (var a = [].slice.call(arguments), c = a.shift(), d = a.pop(), f = 0, h = a.length, i; i = a[f], f < h; f++) for (var l in i) {
        var m = d(l);
      ...
      if (a.forEach) a.forEach(c, d); else for (var f in a) c.call(d, a[f], f)
    }
  };
  ...
  E.array = {
    findIndex: function (a, c, d) {
      ...
      for (var d = [], f = 0, h = a.length, i; i = a[f], f < h; f++) if (c) this.find(d, function (l) {
        return c(l, i)
      }) || d.push(i); else d.indexOf(i) == -1 && d.push(i);
      return d
    }, reverse: function (a) {
  ...
  E.string = {
    md5: function (a) {
      ...
      E.each(a, function (f, h) {
        if (h) if (f && typeof f == "object") E.isArray(f) && E.each(f, function (i) {
          c(h, i)
        }); else c(h, f)
      });
      return d.join("&")
    }, escapeRegexp: function (a) {
      if (!arguments.callee.sRE) arguments.callee.sRE =
        RegExp("(\\/|\\.|\\*|\\+|\\?|\\||\\(|\\)|\\[|\\]|\\{|\\}|\\\\)", "g");
  ...
  E.dom = {
    build2: function () {
      var a = [], c = arguments, d, f, h = 0, i;
      for (c = c[0] instanceof Array ? c[0] : c; h < c.length; h++) if (c[h + 1] instanceof Object) {
  ...
  if (!Array.prototype.indexOf) Array.prototype.indexOf = function (a, c) {
    var d = this.length, f = Number(c) || 0;
    f = f < 0 ? Math.ceil(f) : Math.floor(f);
    if (f < 0) f += d;
    for (; f < d; f++) if (f in
      this && this[f] === a) return f;
    return -1
  };
  var bc = /(^|.|\r|\n)(#\{(.*?)\})/;
  E = {
    parseColor: function (a) {
      ...
      return c[a] ? {r: c[a][0], g: c[a][1], b: c[a][2]} : (result = /^rgb\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*\)$/.exec(a)) ?
        {r: parseInt(result[1]), g: parseInt(result[2]), b: parseInt(result[3])} : (result = /rgb\(\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*,\s*([0-9]+(?:\.[0-9]+)?)\%\s*\)$/.exec(a)) ? {r: parseFloat(result[1]) * 2.55, g: parseFloat(result[2]) * 2.55, b: parseFloat(result[3]) * 2.55} : (result = /^#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])$/.exec(a)) ? {r: parseInt("0x" + result[1] + result[1]), g: parseInt("0x" + result[2] + result[2]), b: parseInt("0x" + result[3] + result[3])} : (result = /^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$/.exec(a)) ?
          {r: parseInt("0x" + result[1]), g: parseInt("0x" + result[2]), b: parseInt("0x" + result[3])} : {r: 255, g: 255, b: 255}
    }, dom: {
      build: function () {
        var a = [], c = arguments, d, f, h = 0, i;
        for (c = c[0] instanceof Array ? c[0] : c; h < c.length; h++) if (c[h + 1] instanceof Object) {
  ...
  ja.extend = function (a, c) {
    ...
    var h = f.constructor, i = f.constructor = function () {
      if (!ja._prototyping) if (this._constructing || this.constructor == i) {
  ...
  ja.prototype = {
    extend: function (a, c) {
      if (arguments.length > 1) {
        ...
        for (var i = {toSource: null},
               l = ["constructor", "toString", "valueOf"], m = ja._prototyping ? 0 : 1; t = l[m++];) a[t] != i[t] && h.call(this, t, a[t]);
  ...
  ja = ja.extend({
    ...
    ancestor: Object, version: "1.1", forEach: function (a, c, d) {
  ...
  jQuery.Draggable = function (a, c) {
    ...
    function f(A) {
      l || m.beforeDrag(A);
      l = true;
      var G = parseInt(a.css("left")), q = parseInt(a.css("top")), o = A.pageX - t, r = A.pageY - D;
  ...
  var Nb = function (a, c) {
      ...
      function i(n) {
        ...
        if (!(n < 0)) {
          ...
          A.trigger("input");
          r.hide();
          o.onItemSelect && setTimeout(function () {
            o.onItemSelect(u)
          }, 1);
          setTimeout(function () {
            A[0].focus();
      ...
      function l(n) {
        ...
        if (u.size() != 0) {
          q += n;
          if (q < 0) q = u.size() - 1; else if (q >= u.size()) q = 0;
      ...
      function m(n) {
        ...
        u = [];
        s = [];
        for (var w = 0, y; y = o.data[w], w < o.data.length; w++) if (y.toLowerCase().indexOf(n) ==
          0) u.push(y); else y.toLowerCase().indexOf(n) != 0 && y.toLowerCase().indexOf(n) != -1 && s.push(y);
        s.sort(function (B, C) {
      ...
      function t(n, u) {
        if (u && u.length > 0) {
          ...
          r.find("li").unbind();
          r.find("ul").html($.map(u, function (w) {
            return "<li>" +
              w + "</li>"
          }).join("")).end().css({top: s.y + A[0].offsetHeight, left: s.x}).show();
          r.find("li").mouseover(function (w) {
      ...
      var o = c = $.extend({
        inputClass: "ac_input", resultsClass: "ac_results", minChars: 1, delay: 100, matchCase: 0, matchSubset: 1, matchContains: 0, mustMatch: 0, loadingClass: "ac_loading",
        selectFirst: false, selectOnly: false, mode: "multiple", multipleSeparator: " "
      }, c || {});
  ...
  x.urls = {
    getUserBookmarksPageURL: function (a) {
      ...
      return "https://groups.diigo.com/user/" +
        k.user
  ...
  x.handlers = {
    tb_showDropDownMenu: function (a, c) {
      ...
      if ($.browser.msie && $.browser.version >= 9) {
        tbar = $("#diigolet-toolbar");
        Ka(da.jHelpTip.css({top: tbar.offset().top + tbar.height() - 5}).show())
    ...
    mouseOnBorder: function () {
      ...
      k.penColor && k.penColor.length > 0 ? Q("diigolet-colorMenu-" + k.penColor).toggleClass("colorchecked", true) : Q("diigolet-colorMenu-yellow").toggleClass("colorchecked", true);
      $.browser.msie && $.browser.version >= 9 ? Ka($("#diigolet-tb-colorMenu").css({left: a.offset().left, top: $(document).scrollTop() + 25}).show()) : Ka($("#diigolet-tb-colorMenu").css({left: a.offset().left}).show())
    },
    diigoTwitterit: function () {
      ...
      if (qa()) {
        ...
        if (a.ok) {
          ...
          var f = N.add({user: k.user, realName: k.realName, content: a.html, type: ia, nth: a.nth}, {}, c, d);
        ...
        if (typeof k.permissions.annPermission !==
          "undefined" && !k.permissions.annPermission.annotationPermission) {
          ca.init();
          ca.show("highlight")
        }
        return f
      ...
      var c = $(document.body);
      c.removeClass(pa(k.defaultColors, function (d) {
        return "diigoHiPen-" + d
      }).join(" "));
      ...
      k.isHighlightPen && $(document.body).removeClass(pa(k.defaultColors, function (d) {
        return "diigoHiPen-" + d
      }).join(" ")).addClass("diigoHiPen-" + k.penColor)
      ...
      !qa() || H.isEditing() || N.add({
        user: k.user, realName: k.realName,
        content: "", type: fa
      }, {dontSave: true}).jumpHere(false, true)
  ...
  var ra = ja.extend({
    constructor: function (a) {
      W(this,
        {url: "", urlId: "", b_id: "", title: "", description: "", user: "", unread: false, mode: 0, tags: [], alert: false, onlyInGroup: false, groups: [], lists: [], outliners: [], datetime: null, saved: false, toAddToBookmarkLists: null, toShareToGroups: null, toShareExistingAnnotations: false}, a || {})
    }, getTitle: function () {
  ...
  ra.fromDocument = function () {
    var a = document;
    return new ra({url: a.location.href, title: a.title || a.location.href, mode: K.getFromCache("prefs.bookmark.privateByDefault", "b") ? 2 : 0})
  };
  ra.readlater = function () {
    var a = document;
    return new ra({
      url: a.location.href, title: a.title || a.location.href,
      mode: K.getFromCache("prefs.bookmark.privateByDefault", "b") ? 2 : 0, unread: true
    })
  };
  var Ra = false, tb = false, xb;
  x.openURL = function (a) {
    O.send({name: "createTab", details: {url: a}})
  };
  var k = {
    generated: false, chromeTabId: null, isSignedIn: false, autoLoad: false, keywords: [], orphanHighlights: [], googleSearchSentence: "", autoShowHighlightIcon: false, chromeExtensionID: null, user: "", userId: null, realName: "", userLevel: 0, isHighlightPen: false, penColor: "yellow", mouseClass: "mouseovered", defaultColors: ["yellow", "blue", "green", "pink"],
    signedIn: false, launchMode: {normal: diigoletLaunchMode == 0, permalink: diigoletLaunchMode == 3, pagePlayer: diigoletLaunchMode == 5}, permalinkParams: null, silent: false, bookmark: new ra, groupTagsDict: {}, recommendedTagsLoaded: false, recommendedTags: [], selectionResult: null, annotations: [], pageComments: [], currentHighlight: "", myTags: [], myTagsWithCount: [], myBmLists: [], myGroups: [], myContacts: [], permissions: {capture: true, image: true, highlight: true, premium: true, snapshot: true}, isCommented: function () {
    ...
    isAnnotated: function () {
      ...
      if (a != this.silent) if (this.silent = a) {
      ...
      if (k.launchMode.permalink) k.permalinkParams = {user: window.diigo_permalink_user, key: window.diigo_permalink_key, mode: diigo_permalink_mode, url: diigo_bookmark_url, legacy: diigo_permalink_legacy};
      ...
      for (var a = $(".diigoHighlight"),
             c = [], d = 0; d < a.length; d++) c[d] = a[d].className.match(/id(\w)+/g)[0];
      a = db(c);
      for (d = 0; d <= a.length; d++) if ($("." + a[d]).length == 1) $("." + a[d]).scrollmarker(); else {
        c = $("." + a[d]);
        $(c[0]).scrollmarker()
      }
      k.orphanHighlights.length != 0 ? $("#diigolet-panel-panel").addClass("orphanHighlight") : $("#diigolet-panel-panel").removeClass("orphanHighlight")
  ...
  var ic = {
    ...
    ele: null, shown: false, init: function () {
      ...
      if (k.signedIn) {
        ...
        d.find("." + {selection: "_selection", highlight: "_highlight", selection_3df: "_selection_3df"}[c]).show();
      ...
      this.ele = $($a).css({position: "absolute"}).hide().appendTo(document.body).hide()
    }
  };
  x.run = function (a, c) {
    K.updateCache(a.preloadedPrefs);
    na();
    $("#diigolet-panel-panel").length == 0 && M([J, U, H, Z, L, R, wa], function (d) {
      d.init()
    });
    ...
    M([U, H, da, ga, Z, La, J, L, R], function (d) {
      if (d.j) {
        ...
        d.j.unbind(".pay4sin").bind("mouseup.pay4sin", f).bind("mousedown.pay4sin", f).bind("keydown.pay4sin", function (h) {
          h.stopPropagation()
        })
      }
    });
  ...
  x.chromeRun = function (a, c) {
    ...
    else if (x.started) $.isReady || d(); else {
      x.started = true;
      if (a.logLevel) x.logLevel = a.logLevel;
      if (Ma(window.location.href)) if (window.top != window.self) I("[BUG] I don't run in a frame"); else {
        if (!$.isReady) {
          I("DOM not ready.");
          a.userClick && d()
        }
  ...
  var Fa = {
    ...
    getDocumentSource: function () {
      var a = document, c = document.doctype, d = "";
      if (c) {
      ...
      a = a.documentElement;
      a = a.outerHTML ? a.outerHTML : XMLSerializer ? (new XMLSerializer).serializeToString(a) : "<html>" + document.innerHTML +
        "</html>";
      return d + a
    ...
    uploadCache: function (a) {
      if (Fa.hasCacheUpload()) {
        x.Messenger.send({name: "uploadCache", details: {text: this.text, html: this.html, urlMD5: k.bookmark.urlId, groups: a.groups, ispdf: this.ispdf, pdfurl: this.pdfurl}});
        L.notifyWait(chrome.i18n.getMessage("upload_uploading"), {timeout: 8E3})
      } else {
        ca.init();
        ca.show("cache")
      }
      ...
      if (a.overQuota) {
        ...
        x.Messenger.send({name: "updateGlobalData", details: {permissions: k.permissions}});
        Fa.onPrivilegeChange()
      }
      L.notifyAlert(a.msg ||
        chrome.i18n.getMessage("upload_error"), {timeout: 8E3})
    }, uploadDidSucceed: function () {
      L.notifyOK(chrome.i18n.getMessage("upload_success"))
    }
  }, Vb = {
    uploadMHTML: function (a) {
      if (Fa.hasCacheUpload()) x.Messenger.send({name: "importMHTML", details: {link_id: a.link_id, mhtml_upload_url: a.mhtml_upload_url, charset: document.charset, blob: this.blob, tabid: a.tabid}}); else {
        ca.init();
        ca.show("acache")
      }
    }, uploadDidSucceed: function (a) {
      (a = a.visit_url) ? L.notifyOK("Upload succeeded! <a href=" + a + ' target="_blank">check it out</a>') :
        L.notifyOK(chrome.i18n.getMessage("upload_success"))
    }
  }, Ub = {
    afterRun: function () {
      ...
      $("script[src*=opentracker]").length && window.addEventListener("mousedown", function (a) {
        a = $(a.target);
  ...
  var Wb = {
    quoteTag: function (a) {
      ...
      for (var f = [], h = [], i = false, l = 0, m = a.length, t; t = a.charAt(l), l < m; l++) if (t == '"') if (i) {
        i = false;
        d()
      } else i = true; else if (i) f.push(t); else /\s/.test(t) || t == "," ? d() : f.push(t);
      d();
      if (c) h = Gb(Hb(h), function (D) {
        return Qa(D) || null
      });
      return h
    }, unparseTags: function (a, c) {
      c = c || " ";
      return pa(a, function (d) {
  ...
  var N = ja.extend({
    constructor: function (a) {
      a = a || {};
      W(this, {
        id: null, user: "", mode: 2, content: "", comments: [], saved: false, datetime: "", painted: false,
        paintedSuccessfully: false, type: null, groups: [], onlyInGroup: false, extra: {}
      }, a);
      ...
      if (this.isPrivate() &&
        this.user != k.user) return false;
      if (this.isPublic() && 0) return false;
      return true
    }, addGroups: function (a) {
      var c = this;
      M(a, function (d) {
        if (Ia(c.groups, function (f) {
      ...
      var a;
      a = this.type == ia ? $("em." + S.HIGHLIGHT_ID_CLASS + this.id) : $("." + S.HIGHLIGHT_TYPE_CLASS + this.type + "." + S.HIGHLIGHT_ID_CLASS + this.id);
      if (a.size() <= 0) return {left: 0, top: 0};
      var c = window, d = {left: 0, top: 0}, f = 0, h = 0;
      a.parents("div").filter(function () {
        if (c.getComputedStyle(this,
      ...
      if (this.paintedSuccessfully) {
        ...
        if (d.size() == 0) k.notify("No corresponding text is found on the page. This page has probably been changed since you highlighted it."); else {
          if (a) {
            var f = d.eq(0).offset();
  ...
  W(N, {
    add: function (a,
        ...
        $("." + i).on("dragstart", function (l) {
          ...
          $("." + i).each(function () {
            if ($(this).hasClass("diigoHighlight")) {
              ...
              if ($(this).parent()[0].tagName == "A") t = '<a href="' + $(this).parent().attr("href") +
                '"">' + t + "</a>";
              m += t + " "
            }
          });
      ...
      a.groups && M(a.groups, function (l) {
  ...
  var ta = N.extend({
    constructor: function (a) {
      ...
      a = k.bookmark.url;
      if (!this.content) if (this._focusedNode) this.content = Y('<img src="#{0}" title="#{1}" alt="#{2}" />', [ta.makeAbsUrl(this._focusedNode.src, a), this._focusedNode.title, this._focusedNode.alt]); else throw"Must specify either content or _focusedNode for image highlight";
      if (!this.extra.imageUrl) this.extra.imageUrl = ta.makeAbsUrl((this.content.match(/<img.*?\s+src=['"]?(.*?)['"]?[\s>]/im) ||
        [])[1] || "", a);
      if (!this.extra.imageUrl) throw"invalid imageUrl";
      if (!this.saved && this._focusedNode) for (var c = ta.makeRelUrl(this.extra.imageUrl, a), d = ta.makeAbsUrl(this.extra.imageUrl, a), f = document.getElementsByTagName("img"), h = 0, i = 0, l = f.length, m; m = f[i], i < l; i++) if (m.src.indexOf(c) > -1 && d == ta.makeAbsUrl(m.src, a)) {
        h++;
      ...
      if (a.size() == 0) for (var c = ta.makeRelUrl(this.extra.imageUrl, k.bookmark.url),
                                d = ta.makeAbsUrl(this.extra.imageUrl, k.bookmark.url), f = document.getElementsByTagName("img"), h = 0, i = 0, l = f.length, m; m = f[i], i < l; i++) if (m.src.indexOf(c) > -1 && d == ta.makeAbsUrl(m.src, k.bookmark.url)) {
        h++;
  ...
  W(ta, {
    paint: function (a) {
      function c(i, l) {
        $(i).addClass(S.HIGHLIGHT_ICON_CLASS).addClass(S.HIGHLIGHT_ID_CLASS + l.id).addClass(S.HIGHLIGHT_TYPE_CLASS + ANNOTATION_TYPE_ICON).addClass("ImageIcon").toggleClass("public", l.isPublic() && f.length > 0).toggleClass("private", l.isPrivate() &&
          f.length > 0).toggleClass("group", l.inAnyGroups() && f.length > 0).toggleClass("diigoHighlightcommented", f.length > 0);
        if (f.length > 0) {
          $(i).html("<span class='" + S.HIGHLIGHT_ID_CLASS + l.id + " " + S.HIGHLIGHT_ICON_CLASS + " " + S.HIGHLIGHT_TYPE_CLASS + ANNOTATION_TYPE_ICON + "'>" + f.length + "</span>");
      ...
      if (d) {
        var f = a.comments;
        d.addClass(ha.HIGHLIGHT_CLASS).addClass(ha.HIGHLIGHT_ID_CLASS +
          a.id).addClass(ha.HIGHLIGHT_TYPE_CLASS + a.type).removeClass("yellow blue green pink").addClass(a.extra.color && a.extra.color.length > 0 ? a.extra.color : "yellow").toggleClass("diigoHighlightcommented", f.length > 0);
        if (d.size() > 0) {
          var h = d.offset();
          a.extra.top = h.top;
          a.extra.left = h.left
      ...
      a =
        N.find(a);
      var c = a.getEle();
      c && c.removeClass(ha.HIGHLIGHT_ID_CLASS + a.id).removeClass(ha.HIGHLIGHT_CLASS).removeClass("diigoHighlightcommented").removeClass("public").removeClass("group")
    }, getBaseUrl: function (a, c) {
      var d = a, f = d.replace(/:\/\//, "123")[(c ? "i" : "lastI") + "ndexOf"]("/");
  ...
  var vb = N.extend({
    constructor: function (a) {
      ...
      var a = 5 * eb(k.annotations, function (i) {
        return i.type == fa
      }).length, c = $(document), d = c.scrollTop(), f = c.scrollLeft(), h = c.height();
      c = c.width();
      this.extra = W({left: a + f + 10, top: a + d + 50}, this.extra || {}, {
        winWidth: c,
        winHeight: h
      })
    }, getEle: function () {
    ...
    paint: function (a) {
      ...
      if (a.painted) {
        ...
        if (c) {
          ...
          setTimeout(function () {
            (H.getCommentCount("group", a.comments) || "") > 0 && c.find("span").text(H.getCommentCount("group",
              a.comments)).show()
          }, 100)
        }
      } else {
        c = $(mb).addClass(ha.HIGHLIGHT_CLASS).addClass(ha.HIGHLIGHT_ID_CLASS + a.id).addClass(ha.HIGHLIGHT_TYPE_CLASS + a.type).css({left: a.extra.left, top: a.extra.top}).appendTo(document.body);
        ...
        d == "onlyPrivate" ||
        d == "noComment" ? c.addClass("private") : c.addClass("group");
  ...
  var cb = ja.extend({
    constructor: function (a) {
      W(this, {id: null, user: "", mode: 2, saved: false, datetime: "", groups: [], onlyInGroup: false}, a || {});
      if (this.onlyInGroup && !(this.id > 0)) this.id = Math.random().toString().substr(2)
    }, del: function (a) {
      return this.constructor.del(this,
        a)
    }, canDelete: function () {
  ...
  W(va, {
    add: function (a) {
      ...
      } else if (a.mode != ann.mode && a.mode !=
        3) {
        ann.mode = a.mode;
      ...
      if (c) {
        ...
        a.saved && z.deleteInlineComment(a, a.groups[f].idInGroup,
          d);
        a.groups.splice(f, 1);
  ...
  M([ra, N, cb], function (a) {
    a.implement({
      isPublic: function () {
        ...
        return c == "_all" ?
          this.isPublic() || this.isPrivate() || this.inAnyGroups() : c == "_smasher" ? this.isPublic() || this.isPrivate() : c == "_public" ? this.isPublic() : c == "_private" ? this.isPrivate() : this.inGroup(c)
      }
    })
  });
  var z = {
      count: 3, transId: 1, callbacks: {}, callback: function (a, c, d, f) {
        if (a) {
          var h = a.transId, i = a.cmd;
          I("[WebAPI response]", i, a.code, a, a.result)
        }
        if (a) {
          this["cb_" + i] && this["cb_" + i](a);
          a.code == 1 && this["cb_" + i + "_success"] && this["cb_" + i + "_success"](a.result, f);
          a.code == 0 && this["cb_" + i + "_failure"] && this["cb_" + i + "_failure"](a.result,
            a.status, f)
        }
        ...
        var i = this;
        x.Messenger.send({name: "WebAPI", details: {cmd: a, data: c, transId: h, isInGroup: /group.diigo.(.*)\/item\/pdf/.test(window.location.href)}}, function (l) {
        ...
        chrome.runtime.sendMessage({name: "listCreateSuccess", data: a.list});
        if (U.shown) {
          U.updateLists();
          $("#diigo-list").find("select").find("option[value=" + a.list.id + "]").attr("selected", true);
        ...
        if ($("#diigosc-savepopup").length > 0) {
          ka.updateLists();
          $("#diigosc-lists").find("option[value=" + a.list.id + "]").attr("selected", true);
        ...
        chrome.runtime.sendMessage({name: "listCreateSuccessAndUpdateLists", lists: k.myBmLists, newlist: a.list, outliners: k.outliners});
        Sa({type: "listCreateSuccessAndUpdateLists", lists: k.myBmLists, newlist: a.list, outliners: k.outliners})
      }, cb_list_create_failure: function (a) {
        if (a ==
          500) L.notifyAlert(chrome.i18n.getMessage("server_error"), {timeout: 0}); else a == 0 && L.notifyAlert(chrome.i18n.getMessage("network_error"), {timeout: 0});
        U.shown && $("#diigo-list-addBtn").removeClass("processing");
        $("#diigosc-savepopup").length > 0 && $("#diigosc-list-addBtn").removeClass("processing");
        chrome.runtime.sendMessage({name: "listCreateFailureAndUpdateLists"});
        ...
        if (D) c = "image";
        b = atob(f);
        c = {type: c, title: a.title, description: "", mode: a.mode, src_url: a.src_url, src_title: a.src_title, src_description: "", src_unread: false, file_type: i, file_name: l, single_item: a.single_item, file_md5: Ja(b), shareit: false, twitterit: d, tags: a.tags, extra: h};
        I(a.list, a.groups);
        if (a.list != 0 && a.list != -1) c.lists = a.list;
        if (a.groups != 0 && a.list != -1) c.groups = a.groups;
        d = function (A) {
          if (D) {
            ma.port.postMessage({type: "upload-image-done", resp: A, image_src: a.image_src});
            yb.hide()
          }
        };
        if (typeof t !==
        ...
        if (k.generated == false) O.send({name: "initialData"}, function (f) {
        ...
        if (a == 500) L.notifyAlert(chrome.i18n.getMessage("server_error"), {timeout: 0}); else a == 0 && L.notifyAlert(chrome.i18n.getMessage("network_error"), {timeout: 0})
      }, loadBookmark: function (a) {
        var c = {url: k.bookmark.url, what: "bookmarkInfo annotations pageComments", permalinkParams: k.launchMode.permalink ? k.permalinkParams : null};
        ...
        if (k.generated == false) {
          ...
          a.pageComments && M($b(a.pageComments), function (f) {
            ya.add(W({saved: true}, f))
          });
          if (a.annotations) {
            k.annotations = [];
            M(a.annotations, function (f) {
              var h = N.add(Ya({saved: true},
                f, Va("id user realName mode type content datetime extra groups onlyInGroup")), {dontPaint: true, dontSort: true});
        ...
        U.bookmarkLoaded();
        c != "prepareCtx" && O.send({name: "bookmarkChanged", details: {saved: d.saved}});
        k.bookmark.saved == true && chrome.runtime.sendMessage({name: "saved"})
      },
      saveBookmark: function (a, c, d, f) {
        x.Messenger.send({name: "updateBrowserActionIconToDoing"});
        var h = k.bookmark, i = {url: h.url, mode: h.mode, title: h.title, tags: h.tags, description: h.description, unread: h.unread, groups: h.toShareToGroups, shareExistingAnnotations: h.toShareExistingAnnotations, lists: h.toAddToBookmarkLists};
        if (!h.url) {
        ...
        if (a) {
          if (l = a.pageComment) {
            i.pageComment = {content: l.content, mode: l.mode, groups: l.groups, justForGroups: l.justForGroups};
            i.groups =
              l.groups
          }
          if (l = a.annotation) {
            i.annotation = {id: l.id, content: l.content, type: l.type, groups: l.groups, extra: l.extra};
            i.groups = l.groups;
            if (l = a.annotation.inlineComment) {
              i.annotation.inlineComment = {mode: l.mode, content: l.content, groups: l.groups, justForGroups: l.justForGroups};
              i.groups = l.groups
            }
          }
        ...
        if (a.groups.length > 0) {
          ...
          if ((h = a.groups[0].access_mode) && (h == 5 || h == 6)) {
            ...
            k.notify('Site archived in Teams. <br><a target="_blank" id="team_url" href="' + h + '">Click here to go directly to the team clean reader page.</a>', 3E4)
          }
        }
        a.lists && a.lists.forEach(function (m) {
          i.lists.push(m)
        });
        if (h = a.pageComment) ya.add({
          id: h.id, user: h.user, realName: h.realName, mode: h.mode,
          datetime: h.datetime, content: h.content, userOnline: h.userOnline, groups: h.groups, onlyInGroup: h.onlyInGroup, saved: true
        });
        if (h = a.annotation) {
          var l = N.find(h.id);
          l.saved = true;
          h.groups && l.addGroups(h.groups);
          if (h = a.annotation.inlineComment) va.add({id: h.id, annotationId: h.annotationId, user: h.user, realName: h.realName, mode: h.mode, datetime: h.datetime, content: h.content, userOnline: h.userOnline, groups: h.groups, onlyInGroup: h.onlyInGroup, saved: true})
        }
        if (h = a.result_shareExisting) {
          ...
          M(h.annotations, function (m) {
            ...
            if (t) {
              ...
              m.inlineComments && M(m.inlineComments, function (D) {
                var A = va.find(D.id);
        ...
        J.shown && J.updateUI();
        i.saved && O.send({name: "bookmarkChanged", details: {saved: true}});
        if (c == "qsave") {
          k.generated = true;
          chrome.runtime.sendMessage({name: "showToolbar", type: "qsave", tabId: d});
          ...
          k.paintAllAnnotations();
          N.sort()
        } else if (c == "readlater") K.get("prefs.autoCloseReadLater", function (m) {
          m == "true" && chrome.runtime.sendMessage({name: "closeTab"})
        }); else if (c == "video") if (a.annotation) {
          c = a.annotation.id;
          $("." + c).addClass("success").find("#diigo-video-loading-text").text("Success!");
          $("." + c).delay(800).fadeOut(function () {
          ...
          $("#diigo-video-collector-loading").delay(800).fadeOut(function () {
        ...
        if (a.annotation && a.annotation.annPermission) {
          ...
          chrome.runtime.sendMessage({name: "updatePermission", type: "highlight", data: a.annotation.annPermission})
        }
      }, cb_bm_saveBookmark_failure: function (a, c) {
        if (this.count > 0) {
          this.saveBookmark(null, function () {
          }, c);
          this.count -= 1
        } else {
          if (a ==
            500) L.notifyAlert(chrome.i18n.getMessage("bookmark_server_error"), {timeout: 0}); else a == 0 && L.notifyAlert(chrome.i18n.getMessage("bookmark_network_error"), {timeout: 0});
          $(".diigolet.notice>div.alert p #retry").unbind("click");
          $(".diigolet.notice>div.alert p #retry").click(function (d) {
            d.preventDefault();
            z.saveBookmark(null, function () {
            }, c);
            L.hide()
          });
          O.send({name: "bookmarkChanged", details: {saved: k.bookmark.saved}})
        }
      }, deleteBookmark: function (a, c) {
        a.urlId || alert("a");
        this.invoke("bm_deleteBookmark", {urlId: a.urlId},
          c)
      }, cb_bm_deleteBookmark_success: function (a) {
        var c = k.bookmark = ra.fromDocument();
        c.loaded = true;
        k.fireEvent("bm_del", [c]);
        k.pageComments.filter(function (d) {
          return d.user == k.user && !d.onlyInGroup
        }).forEach(function (d) {
          d.saved = false;
          ya.del(d)
        });
        k.annotations.filter(function (d) {
          return d.user == k.user && !d.onlyInGroup
        }).forEach(function (d) {
          d.saved = false;
          d.del()
        ...
        if (a.annPermission) {
          ...
          chrome.runtime.sendMessage({name: "updatePermission", type: "highlight", data: a.annPermission})
        }
      }, addAnnotation: function (a, c) {
        var d = {urlId: k.bookmark.urlId, id: a.id, content: a.content, type: a.type, extra: a.extra, groups: a.type == fa ? null : k.bookmark.getGroupNamesSharedByMe()};
        ...
        if (k.bookmark.saved) {
          c ==
          "small" && chrome.runtime.sendMessage({name: "showToolbar"});
          h = function () {
          };
          ...
          chrome.storage.local.get(null, function (i) {
            if (i.researchMode) {
              ...
              c == "video" ? f.saveBookmark({annotation: d}, h, "video", i.researchModeData) : f.saveBookmark({annotation: d}, h, "qsave", i.researchModeData)
            } else if (c == "video") f.saveBookmark({annotation: d}, h, "video"); else {
              var l = {annotation: d};
              K.get(["prefs.bookmark.privateByDefault",
                "prefs.bookmark.unreadByDefault"], function (m) {
        ...
        I(c, a);
        if (c == "video") {
          $("." + a.id).addClass("success").find("#diigo-video-loading-text").text("Success!");
          $("." + a.id).delay(800).fadeOut(function () {
        ...
        if (c) d.idInGroup = c; else d.id = a.id;
        this.invoke("annotation_delete",
          d)
        ...
        c == "video" && $("." + a.id).addClass("fail").find("#diigo-video-loading-text").html('Failed!<a href="#">Retry</a>')
      }, addPageComment: function (a) {
        a = {urlId: a.urlId, mode: a.mode, content: a.content, justForGroups: a.justForGroups, groups: a.groups};
        k.bookmark.saved ? this.invoke("pc_add",
          a) : this.saveBookmark({pageComment: a})
      }, cb_pc_add_success: function (a) {
        ya.add({id: a.id, user: a.user, realName: a.realName, mode: a.mode, datetime: a.datetime, content: a.content, userOnline: a.userOnline, groups: a.groups, onlyInGroup: a.onlyInGroup, saved: true});
        ...
        if (k.bookmark.urlId == "") k.bookmark.urlId = Ja(window.location.href);
        a = {urlId: k.bookmark.urlId, annotationId: a.annotationId, mode: a.mode, content: a.content, justForGroups: a.justForGroups, groups: a.groups};
        if (a.justForGroups) a.urlId = k.bookmark.urlId;
        this.invoke("ic_add", a)
      }, cb_ic_add_success: function (a) {
        va.add({id: a.id, annotationId: a.annotationId, user: a.user, realName: a.realName, mode: a.mode, datetime: a.datetime, datetime2: a.datetime2, content: a.content, userOnline: a.userOnline, groups: a.groups, onlyInGroup: a.onlyInGroup, saved: true});
        ...
        if (c) f.idInGroup = c; else f.id = a.id;
        this.invoke("ic_delete", f, d)
      }, cb_ic_delete_success: function () {
      }, editInlineComment: function (a, c) {
        if (k.bookmark.urlId == "") k.bookmark.urlId = Ja(window.location.href);
        this.invoke("ic_edit", {id: a, urlId: k.bookmark.urlId, content: c})
        ...
        if (a.myGroups) {
          ...
          U.updateGroups();
          chrome.runtime.sendMessage({name: "updateGroups", data: a.myGroups})
        }
        if (a.myList) {
          k.myBmLists = a.myList;
          k.outliners = a.outliners;
          U.updateLists();
          chrome.runtime.sendMessage({name: "updateLists", data: {list: a.myList, outliners: a.outliners}})
        }
        ...
        x.Messenger.send({name: "updateGlobalData", details: {myTags: k.myTags, myTagsWithCount: k.myTagsWithCount, myGroups: k.myGroups, myBmLists: k.myBmLists, outliners: k.outliners, realName: k.realName, myContacts: k.myContacts, permissions: k.permissions}})
      }, updateExtra: function (a) {
        this.invoke("annotation_updateExtra", {
          urlId: k.bookmark.urlId, id: a.id, idsInGroup: pa(eb(a.groups, function (c) {
            return c.user ==
              k.user
          }), function (c) {
            return c.idInGroup
        ...
        chrome.runtime.sendMessage({name: "updateGroupTag", data: k.groupTagsDict})
      },
      signIn: function () {
        ...
        this.invoke("share_makeAnnotatedLink", {url: a, mode: c, annId: d}, f)
    ...
    da = {
      ...
      show: function () {
        ...
        a = a || k.user;
        $("a#diigolet-tb-btnMyBookmarks").attr({href: x.urls.getUserBookmarksPageURL(a)});
        this.showInfo(k.signedIn ? "<em>Welcome " + a + "</em>" : '<em><a href="#" title="Sign in into Diigo.com" onclick="return diigolet.handle(event, \'tb_signIn\')">Sign in</a> to add annotations or <a href="https://www.diigo.com/sign-up" title="Create a Diigo account" target="_blank">Create a Diigo account now</a> (free!)</em>',
          null)
      }, updateUI: function () {
        ...
        var a = Y(hb, {DIIGOLET_VERSION: x.version, URL_DIIGO: "https://www.diigo.com"});
        try {
          var c = this.j = $(a).css({position: $.browser.supportPositionFixed ? "fixed" : "absolute"}).appendTo(document.body).hide()
        } catch (d) {
        }
        setTimeout(function () {
        ...
        $("._diigomenu").click(function (f) {
          ...
          if (className.indexOf("URL_MY_LIBRARY") >= 0) h = x.urls.getUserBookmarksPageURL(); else if (className.indexOf("URL_MY_LIST") >= 0) h = x.urls.getListHome(); else if (className.indexOf("URL_MY_GROUP") >= 0) h = x.urls.getGroupHome(); else if (className.indexOf("URL_NETWORK") >= 0) h = x.urls.getNetwork();
        ...
        if (k.launchMode.permalink) {
          ...
          $("a._forwardUserUrl", c).attr({href: a ? "https://www.diigo.com/user/" + a : "https://www.diigo.com"}).text(a || "Diigo");
        ...
        if ($.browser.msie && $.browser.version >= 9) {
          tbar = $("#diigolet-toolbar");
          f = $("#diigolet-notify").css({top: tbar.offset().top + tbar.height() - 5}).show()
        } else f = $("#diigolet-notify").show();
        f.find("span").html(a);
        f.toggleClass("right", d == 1);
        clearTimeout(f[0].timerId);
        if (c) f[0].timerId = setTimeout(function () {
          f.fadeOut("slow")
        }, c)
      }, onHighlightPenModeChanged: function (a) {
        $("#diigolet-button-highlight").find("b b").text(a ?
          "Highlighter" : "Highlight").end().add("#diigolet-button-highlight-dropdown").toggleClass("checked", a).toggleClass(k.mouseClass, !a)
  ...
  var J = {
    ...
    destroy: function () {
      ...
      var a = this.j, c = $(window), d = c.width();
      c = c.height();
      var f = parseInt(a.css("left")), h = parseInt(a.css("top"));
      f = a.outerWidth();
      var i = a.outerHeight();
      a.css("left", f = d - f + 1);
      ...
      if (!this.shown) {
        ...
        K.get("prefs.lastUsedColor", function (d) {
          ...
          k.isHighlightPen && $(document.body).removeClass(pa(k.defaultColors, function (f) {
            return "diigoHiPen-" + f
          }).join(" ")).addClass("diigoHiPen-" + k.penColor)
        });
        ...
        k.bookmark.saved == false && chrome.storage.local.get(null, function (d) {
      ...
      if (this.shown) {
        ...
        $(document.body).removeClass(pa(k.defaultColors, function (a) {
          return "diigoHiPen-" + a
        }).join(" "));
      ...
      a ? $("#diigolet-panel-status").text("Saving...") :
      ...
      $("#diigolet-panel-closeBtn").click(function () {
        ...
        k.toggleSilent(true);
        k.isHighlightPen = false;
        $(document.body).removeClass(pa(k.defaultColors, function (c) {
          return "diigoHiPen-" +
            c
        }).join(" "));
        da.onHighlightPenModeChanged(false);
      ...
      $("#diigolet-panel-hightlight-dropdown").on("click", function () {
        var c = $("#diigolet-panel-colorPicker").toggleClass("dropdownShown"), d = function () {
          c.removeClass("dropdownShown").unbind("mouseleave", d);
          $(document.body).unbind("click", d)
        };
        c.hasClass("dropdownShown") ? setTimeout(function () {
          $(document.body).one("click", d)
        }, 13) : d()
      });
      $("#diigolet-panel-moreBtn").click(function () {
        var c = $(".optList", this).toggleClass("dropdownShown"), d = function () {
          c.removeClass("dropdownShown").unbind("mouseleave", d);
          $(document.body).unbind("click", d)
        };
        c.hasClass("dropdownShown") ? setTimeout(function () {
      ...
      $("#diigolet-panel-feedback").click(function () {
        var c = $(".optList", this).toggleClass("dropdownShown"), d = function () {
          c.removeClass("dropdownShown").unbind("mouseleave",
            d);
          $(document.body).unbind("click", d)
        };
        c.hasClass("dropdownShown") ? setTimeout(function () {
      ...
      new $.Draggable(a.j, {
        handle: "#diigolet-panel-space", axis: "y", cursor: "n-resize", afterDrag: function () {
          a.adjustPosition()
        }
      })
    }, showInfo: function () {
    ...
    onHighlightPenModeChanged: function (a) {
      ...
      var a = this, c = parseInt(this.j.css("left")), d = $("#diigolet-panel-main").find(".diigolet-panel-btn").length, f = 0;
      if (d == 3) f = 121; else if (d == 4) f = 161;
      this.j.addClass("fold").animate({left: c + f + "px"}, 150, function () {
        $("#diigolet-panel-main").css("width", "0px");
        a.adjustPosition()
      });
      k.isHighlightPen = false;
      $(document.body).removeClass(pa(k.defaultColors, function (h) {
        return "diigoHiPen-" +
          h
      }).join(" "));
      da.onHighlightPenModeChanged(false);
      ...
      this.j.removeClass("fold").animate({left: c - f + "px"}, 150, function () {
        a.adjustPosition()
      })
    }
  };
  ...
  var ha, S, ab = ha = S = {
    HIGHLIGHT_CLASS: "diigoHighlight", HIGHLIGHT_ICON_CLASS: "diigoIcon",
    HIGHLIGHT_ID_CLASS: "id_", HIGHLIGHT_TYPE_CLASS: "type_", isHighlightElement: function (a) {
      try {
        if (a.nodeType == 1 && a.className && (a.className.indexOf(this.HIGHLIGHT_CLASS) > -1 || a.className.indexOf(this.HIGHLIGHT_ICON_CLASS) > -1)) {
          var c = {type: -1, ids: []};
    ...
    docHtml: "", docTxt: "", docTxtOffsetList: [], domSnapshot: function () {
      var a = this;
      a.docTxt = "";
      a.docTxtOffsetList = [];
      var c = false;
      (new sa(document.body, function (d) {
        if (d.nodeType == 3) {
          var f = d.nodeValue.replace(/\s+/mg, " ");
          if (f.length != 0) if (/^\s$/m.test(f)) {
            if (!c) {
              a.docTxt += " ";
              c = true
            }
            ...
            a.docTxtOffsetList.push({offset: a.docTxt.length, node: d, length: f.length});
            a.docTxt += f;
            c = /\s$/m.test(f)
          }
        }
      ...
      if (d == h) {
        ...
        for (c = c - d.offset; c < l.length; c++) if (l.slice(c).replace(/\s+/mg, " ").indexOf(a) == 0) {
          i.startOffset = c;
          break
        }
        if (i.startOffset !== undefined) for (c = i.startOffset + a.length; c <= l.length; c++) if (l.slice(i.startOffset,
          c).replace(/\s+/mg, " ").indexOf(a) > -1) {
          i.endOffset = c;
          break
        }
      } else {
        ...
        for (c = c - d.offset; c <= l.length - m.length; c++) if (l.slice(c).replace(/\s+/mg, " ").indexOf(m) == 0) {
          i.startOffset = c;
          break
        }
        if (i.startOffset !== undefined) {
          ...
          for (c = f - h.offset + 1; c <= l.length; c++) if (l.slice(0, c).replace(/\s+/mg, " ").indexOf(m) > -1) {
            i.endOffset = c;
            break
          }
        }
      ...
      for (var f = [], h = 0, i = 0, l = 0; (h = c.indexOf(a,
        i)) > -1;) {
        l++;
        i = h + 1;
        f.push(h);
      ...
      c = c || 1;
      var d = this.html2txt(a), f = -1, h = 0, i = this.findOccurrences(d, this.docTxt, c);
      if (i.length >= c) f = i[c - 1]; else if (i.length > 0) f = i[i.length - 1];
      if (f == -1) return null;
      h = f + d.length - 1;
      ...
      if (m && t) {
        ...
        for (q = m; q = q.parentNode;) if (f(t, q)) {
          G = q;
          break
        }
        if (!G) return null
      }
      return m && t ? {startNode: m, endNode: t, startOffset: D, endOffset: A, endIndex: h, commonAncestor: G, txt: d} : null
    }, paint: function (a) {
      function c(n, u) {
        $(n).addClass(S.HIGHLIGHT_CLASS).addClass(l).addClass(S.HIGHLIGHT_TYPE_CLASS + u.type).removeClass("yellow blue green pink").addClass(u.extra.color && u.extra.color.length > 0 ? u.extra.color :
          "yellow");
        return n
      }
      ...
      function f(n, u) {
        $(n).addClass(S.HIGHLIGHT_ICON_CLASS).addClass(l).addClass(S.HIGHLIGHT_TYPE_CLASS + ANNOTATION_TYPE_ICON).addClass("TextIcon").toggleClass("public", u.isPublic() && m.length > 0).toggleClass("private", u.isPrivate() && m.length > 0).toggleClass("group", u.inAnyGroups() && m.length > 0 && (H.getCommentsInfo(u.comments) == "onlyGroup" || H.getCommentsInfo(u.comments) == "both")).toggleClass("diigoHighlightcommented",
          m.length > 0).removeClass("diigoadd yellow blue green pink").addClass(u.extra.color).css({bottom: "0px"});
        if (m.length > 0) {
          $(n).html("<span class='" + l + " " + S.HIGHLIGHT_ICON_CLASS + " " + S.HIGHLIGHT_TYPE_CLASS + ANNOTATION_TYPE_ICON + "'>" + m.length + "</span>");
      ...
      var h = this, i = N.find(a), l = S.HIGHLIGHT_ID_CLASS + i.id, m = i.comments;
      if (i.paintedSuccessfully) {
        c("em." + l, i);
        f("div." + l, i);
        $("em." + l).filter(":last").toggleClass("diigoHighlightcommented",
          m.length > 0)
      } else {
        i.painted = true;
        this.domSnapshot();
        ...
        if (t) {
          ...
          if (t.startNode === t.endNode) {
            ...
            var D = t.startNode.splitText(t.startOffset), A = t.endNode.splitText(t.endOffset).previousSibling, G = false, q = [];
            (new sa(t.commonAncestor, function (n) {
              if (!G && n ===
                D) G = true;
              G && n.nodeType == 3 && n.nodeValue.replace(/(^\s+|\s+$)/mg, "").length > 0 && q.push(n);
              if (G && n === A) {
                G = false;
                throw sa.$end;
              }
            }, {filter: this.isHighlightableNode})).walk();
            M(q, function (n) {
              d(n)
            })
          }
      ...
      var c = window, d;
      try {
        $("em." + S.HIGHLIGHT_ID_CLASS + a.id).parents().filter(function () {
      ...
      $("div.diigoIcon.diigoHighlightcommented.private." + S.HIGHLIGHT_ID_CLASS + a.id).attr("style", "background-color:" +
        d + " !important")
    }, adjustColor: function (a) {
      function c(d) {
        var f = d.r / 255, h = d.g / 255;
        d = d.b / 255;
        return 0.2126 * (f <= 0.03928 ? f / 12.92 : Math.pow((f + 0.055) / 1.055, 2.4)) + 0.7152 * (h <= 0.03928 ? h / 12.92 : Math.pow((h + 0.055) / 1.055, 2.4)) + 0.0722 * (d <= 0.03928 ? d / 12.92 : Math.pow((d + 0.055) / 1.055, 2.4))
      }

      a = N.find(a);
      $("em." + S.HIGHLIGHT_ID_CLASS + a.id).each(function (d, f) {
        var h = $(f), i = E.parseColor(h.css("backgroundColor")), l = E.parseColor(h.css("color"));
        var m = c(l), t = c(i);
        if (m > t) {
          l = m;
          m = t
      ...
      return window.getSelection ? Qa(String(window.getSelection())).length > 0 : $.browser.msie ? document.selection.type == "Text" && Qa(document.selection.createRange().text).length > 0 : false
    }, checkSelection: function () {
      var a,
        c, d = {ok: false, txt: "", html: "", msg: "", pen: false}, f = null;
        ...
        if (a.removeAllRanges) {
          ...
          if (!function (l) {
            ...
            for (var m = 0; m < l.length; m++) {
              var t = $(l[m]);
              if (t.is(".diigoHighlight,.diigolet") ||
                t.parents(".diigoHighlight,.diigolet").length > 0) return false
            }
            return true
          }(c)) {
      ...
      if (f) {
        var h = this, i = "";
        (new sa(document.body, function (l) {
          if (l.nodeType == 3) {
            var m = l.nodeValue;
      ...
      return this.stripScripts(a.htmlText ==
      undefined ? $("<div></div>").append(a.cloneContents()).html() : a.htmlText)
    }, stripScripts: function (a) {
      return a.replace(RegExp("(?:<script.*?>)((\n|\r|.)*?)(?:<\/script>)", "img"), "")
    }, html2txt: function (a) {
      a = $("<div></div>").html(this.stripScripts(a));
      var c = this, d = "";
      (new sa(a[0], function (f) {
      ...
      a.remove();
      return d.replace(/\s+/mg, " ").replace(/^\s|\s$/mg,
        "")
      ...
      (new sa(a, function (f) {
        if (f.nodeType ==
          3 && $(f).parents("script,style,noscript").size() == 0) c += (d == 3 ? "" : "\n") + f.nodeValue;
        d = f.nodeType
      })).walk();
      ...
      if (window.getSelection) {
        ...
        a = c.toString();
        c.removeAllRanges ? c.removeAllRanges() : c.collapse()
      } else if ($.browser.msie) a = a.text; else {
        a = "";
  ...
  var H = {
    j: null, shown: false, showMode: "view", dragging: false, backupContent: "", editInterval: null, tempObj: {}, saveInterval: null,
    privateEditing: false, filterMode: "_all", ann: null, init: function () {
      if ($("#diigolet-dlg-sticky").length === 0) {
        ...
        $(".FN-switcher").on("click", function (c) {
          if (c.target.id == "FN-switcher-private" || c.target.parentNode.id == "FN-switcher-private") a.switchTab("private"); else if (c.target.id == "FN-switcher-group" || c.target.parentNode.id == "FN-switcher-group") a.switchTab("group")
        });
        $("#diigolet-dlg-sticky-addTab").on("click", function () {
          if (!($("#FN-group-content-postform").find("button").prop("disabled") ==
            true || $("#FN-group-post").prop("disabled") == true)) if (a.j.hasClass("onlyPrivate")) {
            if ($("#FN-content-footer").find("#editing").is(":visible")) if ($("#FN-private-editor").val().match(/^\s*$/)) {
        ...
        $("#FN-current-group").on("click", function (c) {
          c.stopPropagation();
          $("#FN-group-content-postform").find("button").prop("disabled") == true || $("#FN-group-post").prop("disabled") == true || $("#FN-group-menu").toggle()
        });
        $(document).on("click", function () {
          $("#diigolet-dlg-sticky-colorPicker").hide();
          $("#FN-group-menu").hide()
        });
        $("#diigolet-dlg-sticky-color").find(".dlg-colorItem").on("click", function () {
        ...
        $("#FN-group-menu").on("click", "li", function (c) {
          if (c.target.id == "FN-group-share-new") {
            ...
            d.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
            M(k.myGroups, function (f) {
              $("#FN-group-ul").find('li[data-dgroupname="' +
                f.name + '"]').length == 0 && d.append(E.dom.buildOne("option", {value: f.name}, [f.displayName]))
            });
            $("#FN-current-group").find("span").text("+Share to a new group");
            $("#FN-group-post").focus()
          } else {
            c = {name: $(this).attr("data-dGroupName"), displayName: $(this).text()};
            a.j.removeClass("groupNew");
            a.switchGroup(c)
          }
        });
        $("#FN-group-share").empty().unbind().change(function () {
          $("#FN-post-form").removeClass("notEdit")
        });
        $("#FN-post-form").find("button").on("click", function () {
          a.shareToNewGroup()
        });
        $("#FN-post-form").find("a").on("click",
          function () {
        ...
        $("#FN-group-content-container").on("click", ".FN-group-comment-item-delete", function () {
          var c = $(this).parent().parent().attr("data-cid"), d = $("#FN-current-group").find("span").attr("data-group");
          x.handle(event, "dlgIC_del_ic", c, d)
        });
        $("#FN-private-delete").on("click",
          function (c) {
        ...
        $("#FN-private-editor").on("input propertychange", function () {
        ...
        $("#FN-private-saveBtn").on("click", function () {
          if (!$("#FN-private-editor").val().match(/^\s*$/)) {
            a.ann.onlyInGroup || !a.ann.saved ||
            a.getCommentCount("private", a.ann.comments) == 0 ? a.addPrivateComment() : a.uploadPrivateComment();
        ...
        $("#FN-group-content-postform").find("textarea").focus(function () {
          $("#FN-group-content-postform").addClass("active")
        });
        $("#FN-group-content-postform").find("a").on("click", function () {
      ...
      if (c == "click") {
        if (!this.ann.extra || this.ann.extra == "undefined") this.ann.extra = {color: a}; else this.ann.extra.color = a;
      ...
      if (f === undefined) f = i.showMode; else i.showMode = f;
      ...
      setTimeout(function () {
        ...
        var D =
          $("#FN-group-share").empty();
        D.append(E.dom.buildOne("option", {value: 0}, ["Choose a group"]));
        D.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
        M(k.myGroups, function (A) {
          $("#FN-group-ul").find('li[data-dgroupname="' + A.name + '"]').length == 0 && D.append(E.dom.buildOne("option", {value: A.name}, [A.displayName]))
        });
        i.j.hasClass("onlyPrivate") ? $("#diigolet-dlg-sticky-addTab").attr("title", "Add a group note") : $("#diigolet-dlg-sticky-addTab").attr("title", "Add a private note")
      }, h)
    }, mergePrivateComments: function (a) {
      for (var c =
        this.ann, d = c.comments.length, f = [], h = 0; h < d; h++) c.comments[h].mode === 2 && f.push(c.comments[h].id);
      ...
      setTimeout(function () {
        for (var i = 0; i < f.length; i++) {
          var l = va.find(f[i]), m = l.getAnnotation();
      ...
      var a =
        $("#FN-post-form"), c = a.find("textarea"), d = c.val(), f = a.find("select"), h = f.val();
      if (d.match(/^\s*$/)) {
        c.addClass("notify");
        setTimeout(function () {
          c.removeClass("notify")
    ...
    hide: function () {
      ...
      var a = this.j.find("div.IconFeild").offset(), c = Q("diigolet-annMenu");
      ...
      if (d.type == fa) {
        ...
        d.extra && d.extra.color && d.extra.color.length > 0 ? Q("diigolet-context-" + d.extra.color).toggleClass("colorchecked", true) : Q("diigolet-context-yellow").toggleClass("colorchecked", true)
      }
      ...
      var a =
        $("#FN-private-editor").val(), c = $("#FN-private-delete").attr("data-cid");
      ...
      if (a === "private") c.removeClass("group").addClass("private"); else {
        c.removeClass("private").addClass("group");
        $("#FN-group-post").focus()
      }
    }, privateEditOn: function (a) {
      if (a === true) $("#FN-content-footer").find("#editDone").hide().end().find("#editing").show(); else a === false && $("#FN-content-footer").find("#editing").hide().end().find("#editDone").show()
      ...
      this.ann.getComments(this.filterMode).length == 0 && this.j.find("div.diigoletFNContent").html('<span class="noComments">There are no sticky notes yet.</span>')
    },
    updatePos: function (a) {
      this.j.css("display") == "none" && this.j.css({left: -999, top: -999, display: "block"});
      var c = this.ann, d = this.j.find("div.diigoletFNContent");
      d.css({height: ""});
      ...
      var h = $(document).scrollTop(), i = c + $(window).width() - d.width() - 10, l = h + $(window).height() - d.height() - 10;
      if (f > i) f = i;
      if (f < c) f = c;
      if (a > l) a = l;
      if (a < h) a = h;
  ...
  W(H, {
    showDelay: 200, showTimer: null, hideDelay: 300, hideTimer: null, cancelShow: function () {
      ...
      var d = this, f = {pageX: a.pageX - 10, pageY: a.pageY - 10};
      d.cancelHide();
      d.cancelShow();
  ...
  W(H, {
    eleId_ic: function (a) {
      ...
      var c = this.filterMode, d = "", f = "";
      if (a.canDelete()) f = '<a href="javascript:void(0)" class="del"><img diigoonclick="#{ONCLICK}" title="Delete" src="https://www.diigo.com/client/ietoolbar/spacer.gif" style="width:12px;height:12px;cursor:pointer" alt="" /></a>';
      if (a.isPublic() && (c ==
        "_all" || c == "_public") && a.user == k.user) d += '<label class="_public"><span>Public</span>DEL</label>'.replace("DEL", Y(f, {ONCLICK: "diigolet.handle(event, 'dlgIC_del_ic', '" + a.id + "')"}));
      if (a.isPrivate() && (c == "_all" || c == "_private")) d += '<label class="_private"><span>Private</span>DEL</label>'.replace("DEL", Y(f, {ONCLICK: "diigolet.handle(event, 'dlgIC_del_ic', '" + a.id + "')"}));
      M(a.groups, function (h) {
        if (c == h.name || c == "_all") d += Y('<label class="_#{GROUP_NAME}"><a href="#{GROUP_URL}" class="link">#{GROUP_DISPLAY_NAME}</a>#{DEL}</label>',
          {GROUP_NAME: h.name, GROUP_DISPLAY_NAME: h.displayName, GROUP_URL: x.urls.getGroupURL(h.name), DEL: Y(f, {ONCLICK: "diigolet.handle(event, 'dlgIC_del_ic', '" + a.id + "', '" + h.name + "')"})})
      });
      return d
    }, addInlineComment: function (a) {
      if (a.mode == 2) {
        ...
        var c = a.groups[0].name, d = a.groups[0].displayName;
        a = this.generateGroupItem(a);
        if (this.j.hasClass("groupNew")) {
          $("#FN-group-content-container").append('<div class="FN-group-content-section" data-dgroup="' +
            c + '"></div>');
      ...
      /minute|day|hour/.test(a.datetime) ?
        $("#FN-private-datetime").text("") : $("#FN-private-datetime").text(a.datetime.replace("on ", ""))
    }, getCommentsInfo: function (a) {
      var c = 0, d = 0, f = a.length, h;
      for (h = 0; h < f; h++) if (a[h].mode == 2) c++; else a[h].mode == 3 && d++;
      return c == 0 && d == 0 ? "noComment" : c > 0 && d == 0 ? "onlyPrivate" : c == 0 && d > 0 ? "onlyGroup" : "both"
    }, ifMoreThanOnePrivateComment: function (a) {
      var c = 0, d = a.length, f;
      if (d === 0) return false;
      for (f = 0; f < d; f++) a[f].mode == 2 && c++;
      return c > 1 ? true : false
    }, updatePrivateComments: function (a) {
      var c = a.length, d = "", f = "", h, i = this.getCommentCount("private",
        a), l;
      if (i >= 1) for (l = 0; l < c; l++) {
        if (a[l].mode === 2) {
          d += Cb(a[l].content);
      ...
      var c = {}, d = "", f = [];
      if (a.length != 0) {
        for (var h = 0; h < a.length; h++) if (a[h].mode === 3) {
          f.push(a[h]);
          for (var i = 0; i < a[h].groups.length; i++) c[a[h].groups[i].name] =
            a[h].groups[i].displayName
        }
        a = $("#FN-group-ul").empty();
        for (g in c) {
      ...
      return Y('<div class="FN-group-comment-item" data-cid="#{ID}"><div class="FN-group-comment-item-tbar"><a href="#" class="FN-group-comment-name">#{USERNAME}</a><span class="FN-group-comment-item-time">#{DATETIME}</span>#{DELETE}</div><div class="FN-group-comment-item-content">#{CONTENT}</div></div>',
        {ID: a.id, USERNAME: c, CONTENT: a.content, DATETIME: a.datetime, DELETE: k.user == a.user ? '<a class="FN-group-comment-item-delete">delete</a>' : ""})
    }, getGroups: function (a) {
      for (var c = [], d = 0; d < a.length; d++) if (a[d].mode == 3) for (var f = 0; f < a[d].groups.length; f++) {
        var h = {};
      ...
      var d = c.length, f = 0, h;
      if (a === "private") var i = 2; else if (a === "group") i = 3;
    ...
    removeInlineComment: function (a, c) {
      ...
      aa.editItem("delInlineComment",
        null, a)
    }, onann_del: function (a) {
      if (a == this.ann) if (a.comments.length == 0) {
        a.deleted = true;
        this.hide()
      }
  ...
  W(H, {
    NO_PUBLIC_STICKY_NOTES_ALLOWED: true, isEditing: function () {
      return $("#FN-group-content-postform").find("textarea").val() != "" || $("#FN-group-post").val() != "" || this.ifChooseGroup() || this.privateEditing || $("#FN-group-content-postform").hasClass("active")
      ...
      jEdit: null, init: function () {
        ...
        var a = this, c = a.jEdit.find("textarea").val();
        if (c) {
        ...
        if (this.jEdit[0].parentNode) {
          $("#FN-group-content-postform").find("textarea,button").prop("disabled",
            false);
          this.jEdit.find("textarea").val("");
          this.jEdit.removeClass("active")
        }
      }, show: function () {
        var a = $(".diigoletFNPriSlct").empty(), c = H.ann, d = H.filterMode;
        if ((d == "_all" || d == "_public") && (c ? true : true) && (c ? !c.onlyInGroup && c.getComments("_private").length == 0 : true)) H.NO_PUBLIC_STICKY_NOTES_ALLOWED || a.append('<option value="_public">Public</option>');
        if ((d == "_all" || d == "_private") && (c ? !c.onlyInGroup && c.getComments("_public").length == 0 : true)) a.append('<option value="_private">Private</option>');
        M(k.myGroups,
          function (h) {
            if ((d == "_all" || d == h.name) && (c && (c.user != k.user || c.onlyInGroup) ? c.inGroup(h.name) : true)) a.append(E.dom.buildOne("option", {value: h.name}, [h.displayName]))
          });
        d == "_all" && c && c.isPrivate() && a.val("_private");
        if (a.find("option").length == 0) H.j.find(".diigoletFNComment").hide(); else {
  ...
  var U = {
    ...
    shown: false, ele: null, init: function () {
      ...
      if (!(!k.signedIn || this.shown && this.j.is(":visible"))) {
        ...
        this.j.show();
        var d = this;
        if (k.recommendedTagsLoaded) d.showRecommendedTags(k.recommendedTags); else {
          d.showTagList("recommend", null);
          var f = k.bookmark, h = f.getTitle();
        ...
        K.getUserData("tagsUsedLastTime",
          function (i) {
            i = i ? JSON.parse(i) : [];
        ...
        /https:\/\/www.hnsearch.com\/search/.test(window.location.href) && $("#Diigo-Bookmark-Url,#Diigo-Bookmark-Title,#Diigo-Bookmark-Description,#Diigo-Bookmark-Tag").bind("focus", function () {
          $(window).unbind("keypress");
          var i = this;
          $(window).bind("keypress", function (l) {
            l.preventDefault();
            var m = $(i).val();
            $(i).focus().val(m + String.fromCharCode(l.charCode))
          })
        })
      }
      ...
      x.Messenger.send({name: "pickPromotionMessage"}, function (c) {
        if (a.shown && c) {
          ...
          d.empty().append($("<a>", {href: c.url, target: "_blank", html: c.content}).click(function () {
            d.hide();
            x.Messenger.send({name: "promotionMessageClicked", details: {id: c.id}})
          })).show()
        }
      })
    }, _tagContainerIdForTagListName: function (a) {
      return "#diigolet-bm-tagListContainer-" +
        a
    }, showTagList: function (a, c) {
      var d = this, f = $(d._tagContainerIdForTagListName(a));
      if (a == "group") {
        f.show();
        f.find(".tagButton").remove()
      } else if (a == "recommend" && f.find(".tagButton").length > 0) return;
      if (c) if (c.length == 0) f.hide(); else {
        a == "group" && f.find(".loading").hide();
        var h = k.bookmark, i = k.myTags;
        f.find(".tagButton").remove();
        M(c, function (l) {
          var m = $(E.dom.buildOne("div", {id: "diigolet-tag-" + a + "-" + d.escapeTagForElementId(l), "class": "tagButton", tag: l}, [l])).appendTo(f.show());
          m.toggleClass("selected", $.inArray(l,
            h.tags) >= 0).toggleClass("inused", $.inArray(l, i) >= 0);
          m.click(function () {
            d.toggleTag(l);
            return false
          })
      ...
      var d = $("#Diigo-Bookmark-Tag-Input"), f = x.parseTags(d.val(), true), h = $.inArray(a, f);
      if (c === undefined) h >= 0 ? f.splice(h, 1) : f.push(a); else if (c) h == -1 && f.push(a); else h >= 0 && f.splice(h, 1);
      f = x.unparseTags(f);
      if (f.length) f += " ";
      d.val(f);
      ...
      var a = this, c = $("#Diigo-Bookmark-Tag-Input"), d = x.parseTags(c.val(), true);
      $("#Diigo-Bookmark-Tag-Cloud").find(".Diigo-Bookmark-Tag-item").each(function () {
        var f = $(this), h = f.text();
        $.inArray(h, d) != -1 ? f.addClass("selected") : f.removeClass("selected")
      });
      a.tagListNames.forEach(function (f) {
        $(a._tagContainerIdForTagListName(f)).find("div.tagButton").each(function () {
          var h = $(this), i = h.attr("tag");
          $.inArray(i, d) != -1 ? h.addClass("selected") :
      ...
      var c = null, d = $("#diigo-group").find("select").val();
      if (d != -1) c = d;
      c ? z.loadGroupTagsDictionary([c], function () {
        a.showTagList("group", k.groupTagsDict[c])
      }) : a.showTagList("group", [])
    }, selectAllTags: function (a) {
      var c = this;
      $(c._tagContainerIdForTagListName(a)).find(".tagButton").each(function () {
        c.toggleTag($(this).addClass("selected").text(), true)
      })
    }, placeCursorAtLast: function (a) {
      var c = this, d = a.value.length;
      setTimeout(function () {
        c.placeInputCursor(a,
          d)
      }, 13)
      ...
      a.j = (new cc(ib)).css({
        position: $.browser.supportPositionFixed ?
          "fixed" : "absolute"
      }).hide().appendTo(Ba.body).hide();
      $("#diigolet-bm-tagListContainer-recent").find("a").on("click", function () {
      ...
      $("#Diigo-Bookmark-Title").find("#link-icon").on("click", function () {
      ...
      $("#Diigo-Bookmark-Title-Input,#Diigo-Bookmark-Url-Input").on("focus", function () {
        $(this).parent().find(".diigo-alert-tip").hide()
      }).on("input", function () {
        $(this).parent().find(".diigo-alert-tip").hide()
      });
      $("#Diigo-Bookmark-Options,#Diigo-Bookmark-checkShareExisting").find(".op-checkbox-container").on("click", function () {
        $(this).toggleClass("checked")
      });
      $("#Diigo-Bookmark-Title-Input,#Diigo-Bookmark-Description-Input,#Diigo-Bookmark-Tag-Input").on("focus", function () {
      ...
      a.j.css({left: $(window).width() - a.j.outerWidth() - 5, right: ""});
      new $.Draggable(a.j, {
        handle: "._dragHandle", afterDrag: function () {
          a.adjustPosition()
        }
      });
      ...
      $("#diigo-list-addBtn").on("click", function () {
        if (!$(this).hasClass("processing")) {
          var d = $("#diigo-list-addInput").val(), f = $("#diigo-list-add .diigo-alert-tip"), h = [], i = k.myBmLists.length, l;
          for (l = 0; l < i; l++) h.push(k.myBmLists[l].title);
          if (d.match(/^\s*$/)) f.show().find("span").text("Input a name");
          else if ($.inArray(d, h) !== -1) f.show().find("span").text("Name existed !"); else {
      ...
      if (k.myTagsWithCount.length == 0) {
        var d = document.getElementById(a), f = document.createElement("span");
        f.id = "no-tag";
        f.innerHTML = "No tags";
        d.appendChild(f)
      ...
      var h = this, i = k.bookmark;
      if (a == "data->form") {
        $("#diigolet-tagForward-remove").toggle(i.saved).attr("data-gtooltip",
        ...
        if (c || i.saved) {
          ...
          i.mode == 2 ? $("#Diigo-Bookmark-Privacy").find(".op-checkbox-container").addClass("checked") : $("#Diigo-Bookmark-Privacy").find(".op-checkbox-container").removeClass("checked");
          i.unread == true ? $("#Diigo-Bookmark-Unread").find(".op-checkbox-container").addClass("checked") : $("#Diigo-Bookmark-Unread").find(".op-checkbox-container").removeClass("checked")
        }
        ...
        if (Za(a.val())) if (Za(i.description)) Za(c) || a.val('"' + c + '"'); else a.val(i.description); else {
          a.val("");
          i.saved && !Za(i.description) &&
          a.val(i.description)
        }
        h.updateGroups();
        h.updateLists();
        i.saved == false ? chrome.storage.local.get(null, function (l) {
        ...
        i.tags = x.parseTags($("#Diigo-Bookmark-Tag-Input").val(),
          true);
        ...
        if (a == "-1" || a == "-2" || a == "0") i.toShareToGroups = []; else {
          i.toShareToGroups = [a];
        ...
        x.Messenger.send({name: "initialData"}, function (l) {
          l.preloadedPrefs["prefs.bookmark.privateByDefault"] == "true" ? $("#Diigo-Bookmark-Privacy").find(".op-checkbox-container").addClass("checked") :
      ...
      a["private"] ? $("#Diigo-Bookmark-Privacy").find(".op-checkbox-container").addClass("checked") : $("#Diigo-Bookmark-Privacy").find(".op-checkbox-container").removeClass("checked");
      a.unread &&
      $("#Diigo-Bookmark-Unreadr").find(".op-checkbox-container").addClass("checked");
      ...
      j.append(E.dom.buildOne("option",
        {value: 0}, ["Add to an Outliner"]));
      j.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
      $(E.dom.buildOne("option", {value: -2}, ["Create an Outliner..."])).appendTo(j);
      j.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
      M(k.outliners, function (c) {
        a ? j.append(E.dom.buildOne("option", {value: c.id}, [c.title])) : j.append(E.dom.buildOne("option", {value: c.id}, [c.title + (k.bookmark.inList(c.id) ? " (added)" : "")]))
      });
      j.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
      if (k.myBmLists.length) {
        j.append(E.dom.buildOne("option", {value: -8}, ["Add to a List"]));
        j.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
      ...
      var c = this, d = k.myGroups.filter(function (i) {
        return i.access_mode < 5
      }), f = k.myGroups.filter(function (i) {
        return i.access_mode >= 5
      }), h = $("#diigo-group").find("select").empty().unbind().removeClass("processing");
      if (f.length > 0) {
        h.append(E.dom.buildOne("option", {value: 0}, ["Share to a Team"]));
        h.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
        M(f, function (i) {
          a ? h.append(E.dom.buildOne("option", {value: i.name}, [i.displayName])) : h.append(E.dom.buildOne("option", {value: i.name}, [i.displayName + (k.bookmark.inGroup(i.name) ? " (shared)" : "")]))
        });
        h.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]))
      }
      h.append(E.dom.buildOne("option", {value: 0}, ["Share to a group"]));
      h.append(E.dom.buildOne("option",
        {value: -1}, [Array(20).join("-")]));
      $(E.dom.buildOne("option", {value: -2}, ["Create a Group..."])).appendTo(h);
      h.append(E.dom.buildOne("option", {value: -1}, [Array(20).join("-")]));
      M(d, function (i) {
        a ? h.append(E.dom.buildOne("option", {value: i.name}, [i.displayName])) : h.append(E.dom.buildOne("option", {value: i.name}, [i.displayName + (k.bookmark.inGroup(i.name) ? " (shared)" : "")]))
      });
      ...
      if (a) {
        ...
        a = k.bookmark.tags;
        a.length > 0 && K.setUserData({tagsUsedLastTime: JSON.stringify(a.slice(0, 8))});
        var f = function () {
        };
      ...
      var a = {}, c = true, d = $("#Diigo-Bookmark-Url-Input").val();
      if ($("#Diigo-Bookmark-Title-Input").val().match(/^\s*$/)) {
        c = false;
        $("#Diigo-Bookmark-Title").find(".diigo-alert-tip").show()
      }
      ...
      var a = $("#Diigo-Bookmark-uploadCache"), c = a.next("label");
      if (Fa.hasCacheUpload()) {
        a.removeAttr("disabled");
        c.attr("title",
          "Upload a copy of the page")
      } else {
    ...
    j: null, floating: false, container: null, init: function () {
      ...
      var d = $({inline: "#d3df-togglePanelInline", page: "#d3df-togglePanelPage"}[a]),
        f = $({inline: "#d3df-inlineCommentBox", page: "#d3df-pageCommentBox"}[a]), h = !d.is(".collapsed");
      c = c !== undefined ? c : !h;
      if (c != h) {
      ...
      var c = this.j.find("div.popOut"), d = c.is(".popIn");
      c.toggleClass("popIn", !d);
      c.attr("title", d ? "Pop out and pin" : "Pop in");
      k.launchMode.threeDForum || c.attr("title", "close");
      c = this.j;
      if (d) {
        c.appendTo(this.container).css({position: "static", left: "", top: "", width: ""}).find("._dragHandle").css({cursor: ""});
        c[0].draggable.destroy()
      } else {
        var f = a || c.offset();
        c.css({position: "absolute", left: f.left, top: f.top, width: 260}).appendTo(document.body);
        if ($.browser.supportPositionFixed) if (a) c.css({position: "fixed"}); else {
          a = ic.docScroll();
          c.css({position: "fixed", left: f.left - a[0], top: f.top - a[1]})
        }
      ...
      if (qa()) {
        ...
        M(k.myGroups, function (d) {
          c.append(E.dom.buildOne("option", {value: d.name}, [d.displayName]))
        });
        c.val("_private")
      } else z.signIn()
    }, refresh: function () {
      ...
      M(k.pageComments, function (d) {
        c += Y('<li class="bgColor3"><a class="avatar" href="#{DIIGO_URL}/user/#{USER}" title="#{USER}" target="_blank"><img alt="#{USER}" src="http://resources.diigo.com/resources_mana/user_avatar?user_name=#{USER}&amp;size=48" /></a><span class="commentInfo color1"><a href="#{DIIGO_URL}/user/#{USER}" target="_blank" class="color1">#{USER} </a>#{DATE}\n\t\t\t</span ><p class="labelList">#{LABELS_HTML}</p><p class="commentBody color2">#{CONTENT}</p></li>\n', {
          USER: d.user, CONTENT: E.content2Html(d.content),
          DIIGO_URL: "https://www.diigo.com", DATE: d.datetime, LABELS_HTML: a._labelsHtml(d)
        })
      });
      ...
      M(k.annotations, function (i) {
        if (i.type == 0 || i.type == 2 || i.type == 1) {
          ...
          M(i.comments, function (l) {
            f++;
            d += Y('<li class="bgColor3"><a class="avatar" href="#{DIIGO_URL}/user/#{USER}" title="#{USER}" target="_blank"><img alt="#{USER}" src="http://resources.diigo.com/resources_mana/user_avatar?user_name=#{USER}&amp;size=48" /></a><span class="commentInfo color1"><a href="#{DIIGO_URL}/user/#{USER}" target="_blank" class="color1">#{USER} </a>#{DATE}\n\t\t\t</span ><p class="commentBody"><a class="color2" href="#" onclick="diigolet.devil(\'Sidebar\').jumpToHighlight(\'#{HIGHLIGHT_ID}\');return false;">#{CONTENT}</a></p></li>\n',
              {HIGHLIGHT_ID: i.id, USER: l.user, CONTENT: E.content2Html(l.content), DIIGO_URL: "https://www.diigo.com", DATE: l.datetime})
          });
          c = Y('<li class="highlight"><a class="highlight headingColor2 #{FLOAT_NOTE_CLASS}" href="#" onclick="diigolet.devil(\'Sidebar\').jumpToHighlight(\'#{ID}\');return false;"><span class="jumpTo color1" style="float:right">...Jump to</span><em style="font-style:italic;line-height:24px;width:80%;" class="color2">#{CONTENT}</em></a><ul class="comments bgColor1">#{COMMENTS}\n</ul></li>\n',
            {ID: i.id, FLOAT_NOTE_CLASS: i.type == 2 ? "floatNote" : "", CONTENT: E.content2Html(i.prettyTxt || ab.html2txt_pretty(i.content)), COMMENTS: d});
          a += c
        }
      });
      ...
      if (a.length <= 0 || a.length > 500) alert("Comment length should < 500 characters"); else {
        var c = Q("diigolet-sb-selpc").val(), d, f;
        if (c == "_private") d = 2; else if (c == "_public") d = 0; else f = c;
  ...
  W(La, {
    filterMode: "_all", _labelsHtml: function (a) {
      var c =
        a.annotationId ? "ic" : "pc", d = this.filterMode, f = "", h = a.canDelete() ? '<a href="javascript:void(0)" class="del" onclick="#{ONCLICK};"><img title="Delete" src="http://www.diigo.com/client/ietoolbar/spacer.gif" style="width:12px;height:12px;cursor:pointer" alt="" /></a>' : "";
      if (a.isPublic() && (d == "_all" || d == "_public") && a.user == k.user) f += '<label class="_public"><span>Public</span>DEL</label>'.replace("DEL", Y(h, {ONCLICK: Y("diigolet.handle(event, 'sb_del_#{0}', '#{1}')", [c, a.id])}));
      if (a.isPrivate() && (d == "_all" ||
        d == "_private")) f += '<label class="_private"><span>Private</span>DEL</label>'.replace("DEL", Y(h, {ONCLICK: Y("diigolet.handle(event, 'sb_del_#{0}', '#{1}')", [c, a.id])}));
      M(a.groups, function (i) {
        if (d == i.name || d == "_all") f += Y('<label class="_#{GROUP_NAME}"><a href="#{GROUP_URL}" class="link">#{GROUP_DISPLAY_NAME}</a>#{DEL}</label>', {GROUP_NAME: i.name, GROUP_DISPLAY_NAME: i.displayName, GROUP_URL: x.urls.getGroupURL(i.name), DEL: Y(h, {ONCLICK: Y("diigolet.handle(event, 'sb_del_#{0}', '#{1}', '#{2}')", [c, a.id, i.name])})})
      });
      return f
  ...
  var ga = {
    j: null, direction: "normal", shown: false, init: function () {
      this.j || this.create()
    }, create: function () {
      this.j = $($a).css({position: "absolute"}).hide().appendTo(Ba.body).hide();
      var a = $("#diigolet-csm-highlight"), c = $("#diigolet-csm-highlightAndComment");
      K.get("prefs.lastUsedColor", function (l) {
        $("#diigolet-csm").addClass(l)
      });
      a.click(function (l) {
        window.getSelection().toString() != "" && O.send({name: "initialData"}, function (m) {
          if (k.generated == false) {
          ...
          if (m.globalData.user == "") k.notify(Da, 1E3); else {
      ...
      this.j.find(".diigolet-csm-coloritem").click(function (l) {
        var m =
          $(this).data("color"), t = $(this).parent().parent().attr("id");
        O.send({name: "initialData"}, function (D) {
          ...
          if (D.globalData.user == "") k.notify(Da, 1E3); else {
            if (J.shown) if (t == "diigolet-csm-highlight-wrapper") x.handle(l,
              "highlight", "normal", m); else t == "diigolet-csm-highlightAndComment-wrapper" && x.handle(l, "highlightAndComment", "normal", m); else if (t == "diigolet-csm-highlight-wrapper") x.handle(l, "highlight", "small", m); else t == "diigolet-csm-highlightAndComment-wrapper" && x.handle(l, "highlightAndComment", "small", m);
            return false
          }
        })
      });
      ...
      var d, f, h, i;
      this.j.find(".csm-btn").hover(function () {
          var l =
            $(this).find(".csm-action"), m = l.attr("id");
          if (m == "diigolet-csm-highlight") {
            h != null && clearTimeout(h);
            d = setTimeout(function () {
              l.parent().find(".diigolet-csm-color").animate({height: 21}, 200);
              l.parent().find(".diigolet-csm-coloritem").animate({height: 7}, 200)
            }, 500)
        ...
        function () {
          var l = $(this).find(".csm-action"), m = l.attr("id");
          if (m == "diigolet-csm-highlight") {
            d != null && clearTimeout(d);
            h = setTimeout(function () {
              l.parent().find(".diigolet-csm-color").animate({height: 0}, 200);
              l.parent().find(".diigolet-csm-coloritem").animate({height: 0}, 200);
            ...
            i = setTimeout(function () {
              l.parent().find(".diigolet-csm-color").animate({height: 0}, 200);
              l.parent().find(".diigolet-csm-coloritem").animate({height: 0},
                200);
              l.removeClass("editing")
            }, 350)
          }
      ...
      $("#diigolet-csm-search").click(function () {
        ...
        x.Messenger.send({name: "searchDiigoGoogle", details: {text: l}});
        return false
      })
      ...
      if (window.getSelection().toString().length >
        0) {
        this.j.show().css({left: a.pageX + 3, top: a.pageY + 3}).show();
      ...
      $("#diigolet-csm .diigolet-csm-coloritem").animate({height: 0}, 200);
      this.shown = false
    }
  }, Z = {
    j: null, ann: null, p: null, tempAnnId: null, showDelay: 350, showTimer: null, hideDelay: 350,
    hideTimer: null, shown: false, cancelShow: function () {
      clearTimeout(this.showTimer);
      this.showTimer = null
    }, cancelHide: function () {
      clearTimeout(this.hideTimer);
      ...
      var d = this, f = $(a.target).offset(), h = $(a.target).width(), i = {pageX: a.pageX, pageY: a.pageY, offsetTop: f.top, offsetLeft: f.left, width: h};
      d.cancelHide();
      d.cancelShow();
      d.showTimer = setTimeout(function () {
        d.show(i,
      ...
      $("#diigolet-context-yellow,#diigolet-context-blue,#diigolet-context-green,#diigolet-context-pink").click(function () {
        var c = $(this).attr("color");
        if (!a.ann.extra || a.ann.extra == "undefined") a.ann.extra = {color: c}; else a.ann.extra.color = c;
        z.updateExtra(a.ann);
        a.ann.paint();
        var d = "id_" + a.ann.id;
        if ($("." + d).length > 1) {
      ...
      Q("diigolet-annMenu-copy-highlights").click(function (c) {
        x.Messenger.send({name: "copy", details: {html: a.ann.content}});
        a.hide();
        c.preventDefault();
        return false
      })
      ...
      this.j =
        $(lb).css({position: "absolute"}).hide().appendTo(Ba.body).hide()
    }, show: function (a, c) {
      if (c.id != this.tempAnnId) {
        ...
        this.ann = c;
        this.buildMenu();
        this.j.hasClass("onlyMy") ? this.j.css({left: curcorX - 55, top: curcorY - 35}).show() : this.j.css({left: curcorX - 31, top: curcorY - 35}).show();
        this.shown = true;
        this.tempAnnId = c.id
      }
    }, hide: function () {
      ...
      if (a.extra && a.extra.color && a.extra.color.length > 0) {
        ...
        Q("diigolet-context-" + a.extra.color).toggleClass("colorchecked",
          true)
      ...
      var c = "#{MODE} #{WHAT} by #{REALNAME}", d = {REALNAME: a.realName.trim() ? a.realName : a.user};
    ...
    j: null, ann: null, showDelay: 200, showTimer: null, hideDelay: 300, hideTimer: null, editDelay: 350, editTimer: null, shown: false, cancelShow: function () {
      clearTimeout(this.showTimer);
      this.showTimer = null
    }, cancelHide: function () {
      clearTimeout(this.hideTimer);
    ...
    scheduleShow: function (a, c) {
      var d = this, f = {screenX: a.screenX, screenY: a.screenY};
      d.cancelHide();
      d.cancelShow();
      d.showTimer = setTimeout(function () {
        d.show(f, c)
      ...
      c.cancelHide();
      c.cancelToggleEdit();
      if (c.ann) if (c.ann.comments.length == 0) try {
        var f = Z.ann != c.ann;
        Z.p = c;
      ...
      this.hide();
      this.ann = a;
      this.j = $("div." + S.HIGHLIGHT_ID_CLASS + a.id).bind("mouseout", function () {
        c.scheduleHide()
      ...
      ga.shown && ga.hide();
      if (c.comments.length <= 0) if (c.type ==
        ia) this.j.css({top: -23}); else {
      ...
      try {
        var c = this;
        if (!(this.ann == null || this.ann.comments.length <= 0)) if (a) {
    ...
    j: null, k: null, v: null, init: function () {
      this.j || this.create()
    }, create: function () {
      this.j = $('<div id="diigo-video-capture"><div id="diigo-video-capture-container"><div id="diigo-video-capture-logo"></div>Capture</div></div>').appendTo("body").hide();
      this.k = $('<div id="diigo-video-capture-wrapper"><div id="diigo-video-capture-wrapper-tip">Drag to outliner or <span id="diigo-video-capture-upload">Upload</span></div><div id="diigo-video-capture-wrapper-close">Close</div></div>').appendTo("body");
      this.bindEvents()
    }, hide: function () {
      if (this.k && this.v[0]) {
      ...
      var f = d.v.offset().left, h = d.v.offset().top, i = d.v.width(), l = d.v.height();
      ...
      if (a == "loading") {
        ...
        var m = $('<img class="diigo-video-capture-temp-img" />').css({left: 0, top: 0, width: d.v.width()});
        m.appendTo(d.k);
        m.on("load", function () {
          $(this).addClass("animation");
          var t = Math.floor(i * 0.8), D = Math.floor((i - t) / 2);
          setTimeout(function () {
      ...
      $(document).on("mouseenter",
        "video", function () {
          if (!a.v) a.v = $(this);
          var c = a.v.offset().left, d = a.v.offset().top;
          a.v.width();
          K.get("prefs.showVideoCapture", function (f) {
            f === "true" && a.j.css({left: c + 5 + "px", top: d + 5 + "px"}).show()
          })
      ...
      a.k.find("#diigo-video-capture-upload").on("click",
        function () {
          chrome.runtime.sendMessage({name: "uploadImageToDiigo", src: {srcUrl: a.k.find("img").attr("src")}, type: "attach"});
          a.hide()
        });
  ...
  wb = {
    init: function () {
      ...
      var a = this, c = document.getElementById("annotations");
      c && (new MutationObserver(function (d) {
        d.forEach(function (f) {
          f.target.id == "kp-notebook-annotations-pane" &&
          a.onBookChange()
        })
      })).observe(c, {attributes: true, subtree: true, childList: true, characterData: true})
    }, onBookChange: function () {
      console.log("Book changed!");
      this.buildButton()
    }, buildButton: function () {
      var a = this, c = $("#kp-notebook-highlights-count").parent(), d = $('<div id="diigo-import-btn">Import to Diigo</div>');
      ...
      var a = $("h3.kp-notebook-metadata").text(), c = $("h3.kp-notebook-metadata").next(".kp-notebook-metadata").text(), d = [];
      $("#kp-notebook-annotations>div.a-row.a-spacing-base").each(function () {
        var f = $(this), h = {}, i = f.find("#annotationHighlightHeader").text().toLowerCase();
        h.content = f.find("#highlight").text();
        h.note = f.find("#note").text();
        if (h.content) {
          if (i && i.match(/(.*)\s(highlight|surlignement)/) &&
  ...
  var kc = ["screenshot", "qsavebookmark", "readlater", "qdeletebookmark", "ifbookmark", "getCtx", "saveImageToDiigo",
    "prepareCtx", "createList", "contextmenuHighlight", "SignedIn", "bookmarkLoaded", "share", "refreshBookmarkInfo", "googleSearchRecommend", "annotate", "showTutorial", "toggleOutlinerSidebar", "openOutliner", "closeOutliner", "showOutlinerIcon", "hideOutlinerIcon", "addAnnotation", "getRecommendedTags", "editItem"];
  chrome.runtime.onMessage.addListener(function (a, c, d) {
    if (k.chromeExtensionID == c.id || a.name == "run") a.fromTabId && k.chromeTabId == a.fromTabId || x.Messenger.onMessage(a, d); else $.inArray(a.name, kc) !== -1 && x.Messenger.onMessage(a,
      d)
  });
  var O = x.Messenger = {
    noop: function () {
      ...
      run: function (a) {
        ...
        if (Sb()) c !== "autoshow" && chrome.runtime.sendMessage({name: "openPdf", url: window.location.href}); else if (x.version != a.details.version) {
          a = navigator.platform.match(/Mac/) ? "\u2318?+ R" : "F5";
          alert(chrome.i18n.getMessage("refreshAfterUpdate",
            [chrome.i18n.getMessage("extName"), a]))
        } else if (x.ready && c != "autoshow") if (J.shown) if (c == "stickynote") {
          x.stickyNoteByContextMenu = true;
          x.handlers.addStickyNote()
        } else {
          if (c != "highlight") if (c != "qsave") {
            J.hide();
            k.toggleSilent(true)
          }
        } else {
        ...
        try {
          var c = a.user;
          I("onSignIn", c || "username unknown",
            "will loadMyStuff: ", false);
        ...
        if (a.code == 0) L.notifyAlert(chrome.i18n.getMessage("network_error"), {timeout: 0}); else a.code == 500 && L.notifyAlert(chrome.i18n.getMessage("server_error"), {timeout: 0});
        O.send({name: "bookmarkChanged", details: {saved: k.bookmark.saved}})
      }, checkVersion: function () {
        O.send({name: "initialData"}, function (a) {
          if (x.version != a.version) {
            a = navigator.platform.match(/Mac/) ?
              "\u2318?+ R" : "F5";
            alert(chrome.i18n.getMessage("refreshAfterUpdate", [chrome.i18n.getMessage("extName"), a]))
          }
        })
      }, saveBookmarkSuccess: function (a) {
        var c = k.bookmark, d = a.ctx;
        c.title = d.title;
        c.tags = d.tags;
        c.mode = d.mode;
        c.unread = d.unread;
        ...
        c = d.tags;
        c.length > 0 && K.setUserData({tagsUsedLastTime: JSON.stringify(c.slice(0, 8))});
        ...
        if (d.cache) {
          ...
          Vb.uploadMHTML({
            ...
            link_id: a.data.link_id, tabid: a.ctx.tabId
          });
          setTimeout(function () {
      ...
      uploadDidSucceed: function (a) {
        ...
        c.tags.length > 0 && K.setUserData({tagsUsedLastTime: JSON.stringify(c.tags.slice(0, 8))});
        ...
        if (qa()) {
          a =
            a.ifUnread;
          if (k.bookmark) k.bookmark.unread = a ? false : true; else k.bookmark = ra.readlater();
          a = function () {
          };
        ...
        Ma(window.location.href) ? O.send({name: "initialData"}, function (a) {
          xa(a.extensionID);
          if (x.version != a.version) {
            a = navigator.platform.match(/Mac/) ? "\u2318?+ R" : "F5";
            alert(chrome.i18n.getMessage("refreshAfterUpdate", [chrome.i18n.getMessage("extName"), a]))
        ...
        qa() && O.send({name: "initialData"}, function (a) {
          if (k.generated == false) {
            K.updateCache(a.preloadedPrefs);
            k.reset(a);
            k.signedIn && a.shoudLoadMyStuff && z.loadMyStuff();
            if (a.loadBookmarkResult) {
              I("[bookmark info already loaded]",
                a.loadBookmarkResult);
              z.cb_bm_loadBookmark_success(a.loadBookmarkResult, "prepareCtx")
            } else a.shouldLoadBookmarkByMyself && k.signedIn && z.loadBookmark()
          }
          if (a.globalData.user == "") k.notify(Da, 1E3); else {
        ...
        O.send({name: "initialData"}, function (a) {
          ...
          if (k.bookmark.saved) {
            z.cb_bm_loadBookmark_success(a.loadBookmarkResult, "showbar");
            if (!k.bookmark.saved) k.bookmark.title =
        ...
        if (a != undefined) {
          if (/\+/.test(a)) {
            a = a.split("+");
            if (a.length > 5) k.googleSearchSentence = a.join(" "); else k.keywords = a
          } else if (/\s/.test(a)) {
            a = a.split(" ");
            if (a.length > 5) k.googleSearchSentence = a.join(" "); else k.keywords = a
      ...
      showWindow: function (a) {
        if (Ma(window.location.href)) {
          if (qa()) {
            if ("data" in a) var c = a.data.linkTitle == null ? x.templinkTitle : a.data.linkTitle, d = a.data.url;
            if ($("#diigolet-panel-panel").length == 0) {
              L.init();
              na();
        ...
        if (a.data !== undefined) if (Ma(window.location.href)) if (typeof k.permissions.imagePermission !== "undefined" &&
          !k.permissions.imagePermission.imagePermission) {
          ca.init();
          ca.show("image")
        } else {
          if (qa()) {
            var c = a.data, d = k.bookmark.title ? k.bookmark.title : document.title, f = k.bookmark.url ? k.bookmark.url : window.location.href, h = [], i = k.bookmark.title ? k.bookmark.title : document.title, l = a.dtype;
            if (a.type == "tag") ka.showSavePupop("image", c, l); else if (a.type == "attach" || a.type == "outliner") {
              var m = {dataURL: c, title: d, tags: h, mode: 2, src_url: f, src_title: i, single_item: false};
              if (a.image_src) m.image_src = a.image_src;
              var t = a.type == "outliner" ?
                "outliner" : "image";
              chrome.storage.local.get(null, function (D) {
                D.researchMode === true && k.bookmark.saved === false ? z.uploadImage(m, t, false, f, {}, l, "", true, D.researchModeData) : z.uploadImage(m, t, false, f, {}, l, "", true);
                xa(chrome.runtime.id);
                na();
                if (a.type != "outliner") {
                  L.init();
        ...
        O.send({name: "initialData"}, function (a) {
          ...
          chrome.runtime.sendMessage({name: "isSaved", data: {saved: k.bookmark.saved, unread: k.bookmark.unread}});
          var c = k.bookmark, d = window.getSelection().toString();
          K.getUserData("tagsUsedLastTime", function (f) {
            f = f ? JSON.parse(f) : [];
            chrome.runtime.sendMessage({
              name: "sendCtx",
              data: {generated: k.generated, bm: c, lists: k.myBmLists, outliners: k.outliners, groups: k.myGroups, mytag: k.myTags, myTagsWithCount: k.myTagsWithCount, permissions: k.permissions, selection: d, lastUsedTags: f, isAnnotated: k.annotations.length > 0}
            })
          })
        })
      }, getCtx: function () {
        var a = k.bookmark, c = window.getSelection().toString();
        K.getUserData("tagsUsedLastTime", function (d) {
          d = d ? JSON.parse(d) : [];
          chrome.runtime.sendMessage({
            name: "sendCtx", data: {
              generated: k.generated, bm: a, lists: k.myBmLists, groups: k.myGroups, outliners: k.outliners,
              mytag: k.myTags, myTagsWithCount: k.myTagsWithCount, permissions: k.permissions, selection: c, lastUsedTags: d, isAnnotated: k.annotations.length > 0
            }
          })
        })
      }, getRecommendedTags: function () {
        if (k.recommendedTagsLoaded) chrome.runtime.sendMessage({name: "sendRetags", data: k.recommendedTags}); else {
          var a = k.bookmark;
          z.loadRecommendedTags(a.url, a.getTitle(), function () {
            chrome.runtime.sendMessage({name: "sendRetags", data: k.recommendedTags})
          })
        }
      }, getGroupTags: function (a) {
        var c = a.groupName;
        k.groupTagsDict[c] ? chrome.runtime.sendMessage({
          name: "sendGtags",
          data: k.groupTagsDict[c]
        }) : z.loadGroupTagsDictionary([c], function () {
          chrome.runtime.sendMessage({name: "sendGtags", data: k.groupTagsDict[c]})
        })
      }, redirect: function (a) {
        ...
        if (a.annotation) if (aa && aa.j) a.cmd == "remove" || a.cmd == "changeColor" ? aa.editItem(a.cmd, a.annotation) : aa.editItem(a.cmd, null, a.annotation)
    ...
    cache: {}, updateCache: function (a) {
      ...
      var d = this.cache[a];
      if (c == "b") d = d == "true"; else if (c == "n") d = Number(d);
      return d
    }, get: function (a, c) {
      ...
      O.send({name: "setUserData", details: {data: a}},
        c)
    }
  };
  K.get("prefs.autoloadBookmarkStatus", function (a) {
    ...
    } else O.send({name: "initialData"}, function (c) {
      k.user = c.globalData.user;
      k.realName =
        c.globalData.realName
    })
  });
  O.send({name: "getLastFocusedWindow"});
  K.get("prefs.autoImageClipper", function (a) {
    if (window.location.hostname != "maps.google.com") if (a == "true") if (Ma(window.location.href)) {
      L.init();
      xa(chrome.runtime.id);
      na();
      wa.init();
  ...
  K.get("prefs.autoShowHighlightIcon", function (a) {
    ...
    if (!(window.location.hostname == "mail.google.com" || window.location.hostname ==
      "docs.google.com")) if (a == "true") {
      k.autoShowHighlightIcon = true;
      if (Ma(window.location.href)) if (k.autoShowHighlightIcon == true) {
        Qb();
        $(document).ready(function () {
          Qb()
        })
  ...
  $(document).bind("keydown", function (a) {
    K.get(["prefs.shortcutBookmark", "prefs.shortcutBookmarkCtrl", "prefs.shortcutBookmarkAlt", "prefs.shortcutBookmarkKey", "prefs.shortcutReadlater", "prefs.shortcutReadlaterCtrl", "prefs.shortcutReadlaterAlt", "prefs.shortcutReadlaterKey", "prefs.shortcutAnnotate", "prefs.shortcutAnnotateCtrl", "prefs.shortcutAnnotateAlt",
      "prefs.shortcutAnnotateKey", "prefs.shortcutAnnotateArticle", "prefs.shortcutAnnotateArticleCtrl", "prefs.shortcutAnnotateArticleAlt", "prefs.shortcutAnnotateArticleKey", "prefs.shortcutAnnotatePDF", "prefs.shortcutAnnotatePDFCtrl", "prefs.shortcutAnnotatePDFAlt", "prefs.shortcutAnnotatePDFKey", "prefs.shortcutAnnotateScreenshot", "prefs.shortcutAnnotateScreenshotCtrl", "prefs.shortcutAnnotateScreenshotAlt", "prefs.shortcutAnnotateScreenshotKey", "prefs.shortcutOpenOutlinerSidebar", "prefs.shortcutOpenOutlinerSidebarCtrl",
      "prefs.shortcutOpenOutlinerSidebarAlt", "prefs.shortcutOpenOutlinerSidebarKey"], function (c) {
      if (c) if (c["prefs.shortcutBookmark"] == "true" && oa(c["prefs.shortcutBookmarkCtrl"]) == a.ctrlKey && oa(c["prefs.shortcutBookmarkAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutBookmarkKey"]) O.send({name: "initialData"}, function (d) {
        if (x.version != d.version) {
          ...
          if (d.globalData.user == "") k.notify(Da, 1E3); else {
            ...
            K.updateCache(d.preloadedPrefs);
            Aa.makeSnapshot();
            $("#diigolet-panel-panel").length == 0 && M([J, U, H, ga, Z, R], function (f) {
              f.init()
            });
            za.show();
            x.ready = true;
      ...
      }); else if (c["prefs.shortcutReadlater"] == "true" && oa(c["prefs.shortcutReadlaterCtrl"]) == a.ctrlKey && oa(c["prefs.shortcutReadlaterAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutReadlaterKey"]) O.send({name: "initialData"},
        function (d) {
          if (x.version != d.version) {
            d = navigator.platform.match(/Mac/) ? "\u2318?+ R" : "F5";
            alert(chrome.i18n.getMessage("refreshAfterUpdate", [chrome.i18n.getMessage("extName"), d]))
            ...
            if (d.globalData.user == "") k.notify(Da, 1E3); else {
              k.generated == false && z.cb_bm_loadBookmark_success(d.loadBookmarkResult, "prepareCtx");
              if (k.bookmark) k.bookmark.unread = true; else k.bookmark = ra.readlater();
              z.saveBookmark(null, function () {
              }, "readlater")
            }
          }
        }); else if (c["prefs.shortcutAnnotate"] == "true" && oa(c["prefs.shortcutAnnotateCtrl"]) ==
        a.ctrlKey && oa(c["prefs.shortcutAnnotateAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutAnnotateKey"]) O.send({name: "initialData"}, function (d) {
        if (x.version != d.version) {
          d = navigator.platform.match(/Mac/) ? "\u2318?+ R" : "F5";
          alert(chrome.i18n.getMessage("refreshAfterUpdate", [chrome.i18n.getMessage("extName"), d]))
        } else {
          ...
          if (d.globalData.user == "") k.notify(Da, 1E3); else {
            k.generated == false && z.cb_bm_loadBookmark_success(d.loadBookmarkResult, "prepareCtx");
            if ($("#diigolet-panel-panel").length ==
              0) {
      ...
      }); else if (c["prefs.shortcutAnnotateArticle"] == "true" && oa(c["prefs.shortcutAnnotateArticleCtrl"]) == a.ctrlKey && oa(c["prefs.shortcutAnnotateArticleAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutAnnotateArticleKey"]) Xb.open(); else if (c["prefs.shortcutAnnotatePDF"] == "true" && oa(c["prefs.shortcutAnnotatePDFCtrl"]) ==
        a.ctrlKey && oa(c["prefs.shortcutAnnotatePDFAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutAnnotatePDFKey"]) chrome.runtime.sendMessage({name: "openPdf", url: window.location.href}); else if (c["prefs.shortcutAnnotateScreenshot"] == "true" && oa(c["prefs.shortcutAnnotateScreenshotCtrl"]) == a.ctrlKey && oa(c["prefs.shortcutAnnotateScreenshotAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutAnnotateScreenshotKey"]) ka.initSelectedCapture(); else c["prefs.shortcutOpenOutlinerSidebar"] ==
      "true" && oa(c["prefs.shortcutOpenOutlinerSidebarCtrl"]) == a.ctrlKey && oa(c["prefs.shortcutOpenOutlinerSidebarAlt"]) == a.altKey && String.fromCharCode(a.keyCode) == c["prefs.shortcutOpenOutlinerSidebarKey"] && ma.toggle()
    })
  });
  ...
  window.addEventListener("message", function (a) {
    switch (a.data.type) {
      case "ifbookmark":
        O.send({name: "initialData"},
          function (c) {
            ...
            var d = k.bookmark, f = window.getSelection().toString();
            K.getUserData("tagsUsedLastTime", function (h) {
              h = h ? JSON.parse(h) : [];
              Sa({
                type: "sendCtx", data: {
                  generated: k.generated, bm: d, lists: k.myBmLists,
                  outliners: k.outliners, groups: k.myGroups, mytag: k.myTags, myTagsWithCount: k.myTagsWithCount, permissions: k.permissions, selection: f, lastUsedTags: h, isAnnotated: k.annotations.length > 0
                }
              })
            })
          });
      ...
      case "getRecommendedTags":
        if (k.recommendedTagsLoaded) Sa({type: "sendRetags", data: k.recommendedTags}); else {
          a = k.bookmark;
          z.loadRecommendedTags(a.url, a.getTitle(), function () {
  ...
  var L = {
    ...
    destroy: function () {
      ...
      f.show();
      f.children("div:first").children("p:first").children("a").click(function () {
        !$(this).attr("id") == "team_url" && window.open("http://www.diigo.com/sign-in?referInfo=" + encodeURIComponent("/images/diigo-logo.png#SIGNED_IN"))
      });
      f.children("div:first").children("span:first").click(function () {
        f.fadeOut()
      });
      if (f[0].timerId) {
        clearTimeout(f[0].timerId);
      ...
      if (d) f[0].timerId = setTimeout(function () {
        f.fadeOut("slow")
      }, d)
    }
  }, ca = {
    ...
    destroy: function () {
      ...
      switch (a) {
        ...
        case "highlight":
          ...
          d = '<span class="num green">' +
            d.annPermission.annotationQuota + "</span>";
        ...
        case "kindle":
          ...
          break;
        case "acache":
          h = "\u201cAdvanced Cache\u201d feature will help you backup the page completely. It packs all HTML, CSS, Images of the page together into one single file.   Even webpages that are password-protected, like webpages in your school library, can be saved.";
          i = "Advanced Cache";
          break
      }
      ...
      if (c.shown != true) {
        ...
        c.j.find("#diigo-premium-upgrade").click(function () {
          chrome.runtime.sendMessage({
            name: "googleEvent",
            msg: {eventType: "Chrome extension", action: "Click upgrade from over quota", label: a}
          });
    ...
    create: function (a, c) {
      ...
      for (var f in c) {
        ...
        if (d && typeof h == "function" && E.func.argumentNames(h)[0] == "$super") {
          var i = h;
          h = E.extend(E.func.wrap(function (l) {
            return function () {
  ...
  var ea = Ab.Class.create({
    initialize: function (a) {
      ...
      c.recBox = $(a.rec);
      c.recBox.css({font: "12px verdana", padding: "0 0 2px 2px", width: c.width});
      if (!c.recInput) {
      ...
      if (!c.contactList) {
        ...
        c.contactListInner.css({position: "relative", overflow: "auto", padding: "2px"});
      ...
      c.recInput.keydown(function (f) {
        var h;
        if (window.event) h = window.event.keyCode; else if (f) h = f.which;
        switch (h) {
          case 9:
          case 40:
      ...
      this.match.length >
      0 ? this.showContacts() : this.hideContacts()
      ...
      for (var a = this, c = 0; c < this.match.length; c++) {
        var d = this.match[c], f = ea.contacts[d], h = $("<div id=cI_" + c + ' class="cItem"/>');
        switch (f.type) {
          case "list":
      ...
      this.maxHeight && this.contactListInner.css({height: this.maxHeight + "px"});
      this.hoverItem(0)
      ...
      if (c[0]) {
        ...
        var d = a + this.contactListInner.height(), f = c.offset().top;
        c = f + c.height();
        if (f < a) this.contactListInner[0].scrollTop -= a - f; else if (c > d) this.contactListInner[0].scrollTop += c - d
      }
    }, outItem: function (a) {
      if (this.currentItem >= 0) {
        var c = $("#cI_" + this.currentItem);
      ...
      if (typeof a == "undefined") {
        ...
        if (this.currentItem == -1) {
          emailR = /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
          emailR.test(a) ? ea.contacts.push({type: "email", id: a}) : ea.contacts.push({type: "friend", id: a, extra: {real_name: a}});
      ...
      d = $('<a class="recItemRemove" href="javascript:void(0);" id="recItemRemove_' +
        a + '"></a>').html("x");
      c.append(d);
      c.attr("cIndex", a);
      ...
      if (!this.recItemsId || this.recItemsId.length == 0) return null; else {
        for (var a = [], c = [], d = [], f = 0; f < this.recItemsId.length; f++) {
          var h =
            ea.contacts[this.recItemsId[f]];
          switch (h.type) {
            case "friend":
              a.push(h.id);
              break;
  ...
  var Ga = {
    cache: {}, get2: function (a, c, d) {
      var f = this, h;
      if (h = f.cache[c]) {
        d && d(h);
        return h
      } else z.makeAnnotatedLink(a, c, function (i) {
    ...
    shown: false, j: null, tabs: {}, init: function () {
    ...
    onSignOut: function () {
      ...
      var a = this.j, c = $(window), d = c.width();
      c = c.height();
      var f = parseInt(a.css("left")), h = parseInt(a.css("top")), i = a.outerWidth(), l = a.outerHeight();
      ...
      var a = this;
      a.shown || K.get("share.lastOpenedTab", function (c) {
        a.shown = true;
        a.j.show();
        a.adjustPosition();
      ...
      var a = this, c = a.j = $(ob).eq(0).appendTo(document.body).hide();
  ...
  R.tabs.twitter = {
    connectedToTwitter: false, init: function () {
      var a = this;
    ...
    updateTwitterSignInStatus: function (a) {
      ...
      $("#diigolet-share-twitterLeftChars").text(a).toggleClass("full", a <= 0)
    }, defaultMsg: function () {
      function a(l) {
      ...
      var c = this, d = k.bookmark, f = d.title, h = d.description, i = d.url;
      k.pageComments.length > 0 || k.annotations.length > 0 ? Ga.get(d.url, function (l) {
  ...
  R.tabs.facebook =
    {
      init: function () {
        ...
        Ga.get(k.bookmark.url, function () {
          ...
          O.send({name: "openWindow", details: {url: "http://www.facebook.com/sharer.php?src=bm&v=4&i=1267884404&" + hc({u: k.bookmark.url, t: a}), name: "sharer", features: "toolbar=0,status=0,resizable=1,width=626,height=436"}})
        })
      }
    };
  R.tabs.gPlus = {
    init: function () {
      ...
      Ga.get(k.bookmark.url, function (a) {
        O.send({name: "openWindow", details: {url: "https://plus.google.com/share?url=" + a, name: "sharer", features: "toolbar=1,status=0,resizable=1,scrollbars=1,width=700,height=453"}})
      })
    }
  };
  R.tabs.gBuzz_viaGoogleReaderDeprecated = {
    init: function () {
      ...
      function c(d) {
        var f = k.bookmark.getTitle(), h = {url: k.bookmark.url, title: f, srcUrl: x.urls.getUserBookmarksPageURL(), srcTitle: "Diigo", snippet: ""};
        if (d) h.snippet += x.AnnotationFormatter.format({mode: 1});
        h.snippet +=
          a() + "<br/>";
        if (d) h.snippet += Y('<a style="padding-left:2em;" title="#{0}" href="#{1}">#{2}</a><br />', [f, d, "View the annotated version on Diigo"]);
        var i = $('<form action="http://www.google.com/reader/link" method="POST" accept-charset="utf-8"></form>').attr({target: "diigolet-GR________link_bookmarklet_frame"}).appendTo(document.body);
        $.each(h, function (l, m) {
      ...
      k.pageComments.length > 0 || k.annotations.length > 0 ? Ga.get(k.bookmark.url, function (d) {
        c(d)
      }) : c(null)
    }
  };
  ...
  R.tabs.email = {
    init: function () {
      ...
      var a = this, c = a.jCheckQuotes.is(":checked");
      a.jQuotes.html(c ? a.composeMessage() : "");
      if (c) k.pageComments.length > 0 || k.annotations.length > 0 ? Ga.get(k.bookmark.url, function (d) {
        a.updateURL(d)
      ...
      a = k.pageComments.length > 0 || k.annotations.length >
      0 ? Ga.get(k.bookmark.url) || "DIIGO_PERMALINK" : k.bookmark.url;
      a = Y('<p><a class="diigoAnnotatedLink" href="#{1}" title="#{0}">#{0}</a></p>', [k.bookmark.getTitle(), a]);
      c += '<p><strong>Quotes:</strong></p><blockquote style="border-left: 2px solid #eee; padding-left: 1em; margin-left: 1em;">' + a + x.AnnotationFormatter.format({mode: 1, includePageComments: true}) + "</blockquote>";
      return c
    }, trySend: function () {
      ...
      a = {
        toAllFriends: false,
        toUsers: [], toEmails: [], toLists: [], mode: 2, subject: a, body: this.composeMessage(Ea.string.stripTags(this.jTxtMsg.val()))
      };
      ...
      x.Messenger.send({name: "updateGlobalData", details: {myContacts: k.myContacts}});
  ...
  DlgShare_piku = {
    onMyContactsUpdate: function () {
      ...
      this.piku = new ea({rec: "#forwardTo", contacts: k.myContacts, maxRecipients: 20, width: "100%", maxHeight: 180, scanType: "all"})
    }
  };
  ...
  x.AnnotationFormatter =
    {
      format: function (a) {
        a = W({mode: 3, includePageComments: false, onlyPrivate: false}, a || {});
        var c = (a.mode & 2) == 2, d = (a.mode & 1) == 1;
        I("[AnnotationFormatter] format mode:", a.mode, "includeOthers", c, "includePrivate", d);
        var f = "";
        if (a.includePageComments) f += "<ul>" + k.pageComments.filter(function (h) {
        ...
        f += "<ul>" + k.annotations.filter(function (h) {
          return a.onlyPrivate ==
          true ? h.user == k.user && h.onlyInGroup == false : (c ? true : h.user == k.user) && (d ? true : h.mode != 2)
        }).map(function (h) {
          var i = "";
          if (h.type == ia) i = Ea.string.stripTags(h.content); else if (h.type == Wa) i = Ea.string.evalTpl('<img src="#{0}" title="#{1}" alt="#{1}">(image)', [h.extra.imageUrl, ""]); else if (h.type == fa) i = '<img src="https://www.diigo.com/images/v2/float_note.gif" />(floating sticky note)'; else if (h.type == 3) i = "(flash movie)" + h.content; else if (h.type == 4) i = "(video)" + h.content;
          return Ea.string.evalTpl('<li style="line-height:150%;margin-bottom:.6em;">#{0} #{1} #{2}</li>',
            [i, h.groups ? "" : "", "<ul>" + h.comments.filter(function (l) {
              return (c ? true : l.user == k.user) && (d ? true : l.mode != 2)
            }).map(function (l) {
              return '<li style="line-height:150%;margin-bottom:.6em;">' + Ea.string.stripTags(l.content) + Ea.string.evalTpl(' <span style="font-size:11px;color:#aaa">comment by <a href="#{0}">#{1}</a></span>', [x.urls.getUserHomepageURL(l.user), l.realName]) + "</li>"
            }).join("\n") + "</ul>"])
        }).join("\n") + "</ul>";
        return f
      }
  ...
  var ka = {
    editMode: false, drawColor: "red", fontSize: 14, actions: [], reTagShown: false, luTagShown: false, initTool: "", initSelectedCapture: function () {
      function a(s) {
        ...
        if (t.editMode == true) if (s == "diigosc-tag-btn") Na({type: "saveWithTag"}); else s == "diigosc-attach-btn" ? Na({type: "saveAttach"}) : t.diigoscDraw(s); else {
          var w = $("#diigolet_screenshot_center").width(), y = $("#diigolet_screenshot_center").height(), B = $("#diigolet_screenshot_center").offset().top, C = $("#diigolet_screenshot_center").offset().left;
          if ($("#diigosc-savepanel").hasClass("up")) {
            $("#diigosc-editpanel").hide();
            setTimeout(function () {
              chrome.runtime.sendMessage({name: "capture", target: s, cw: w, ch: y, ct: B, cl: C})
            }, 50)
          } else chrome.runtime.sendMessage({name: "capture", target: s, cw: w, ch: y, ct: B, cl: C})
        }
      }
      ...
      function d(s) {
        if (s.button == 0) {
          ...
          h(q, "display", "none");
          s.preventDefault();
          var w = s.pageX, y = s.pageY, B, C, F = document.getElementById("diigolet_screenshot_size"), la = document.getElementById("diigosc-editpanel"), ba =
            function (P) {
      ...
      function f() {
        var s = $("#diigolet_screenshot_center").offset().top - 30, w = $("#diigolet_screenshot_center").offset().left, y = $("#diigolet_screenshot_center").width(), B = $("#diigolet_screenshot_center").height(),
      ...
      function i(s, w, y, B) {
        var C = document.documentElement.scrollHeight, F = document.documentElement.scrollWidth, la = y >= 0 ? s + y : s, ba = B >= 0 ? w : w + B, T = y >= 0 ? F - s - y : F - s;
        w = B >= 0 ? w + B : w;
        s = y >= 0 ? F - s : F - s - y;
        y = C - w;
        F = F -
        ...
        var P = document.getElementById("diigolet_screenshot_right"), V = document.getElementById("diigolet_screenshot_bottom"), X = document.getElementById("diigolet_screenshot_left");
        h(B, "width", la + "px");
        h(B, "height", ba + "px");
        h(P, "width", T + "px");
        h(P, "height", w + "px");
      ...
      function m() {
        ...
        dragresize = new DragResize("dragresize", {maxLeft: document.documentElement.scrollWidth - 5, maxTop: document.documentElement.scrollTop + window.innerHeight, minTop: document.documentElement.scrollTop});
        ...
        dragresize.isElement = function (y) {
          if (y.className &&
            y.className.indexOf("drsElement") > -1) return true
        };
        ...
        dragresize.ondragmove = function () {
          var y = dragresize.elmX, B = dragresize.elmY, C = dragresize.elmW, F = dragresize.elmH;
      ...
      if (!($("#diigosc-all").length > 0)) {
        if (!Ma(window.location.href, true)) if (!/chrome-extension:\/\/.*\/pdf\/viewer.html/.test(window.location.href)) {
          alert(chrome.i18n.getMessage("alertUnsupportedPage"));
          return
        }
        if (typeof k.permissions.imagePermission !== "undefined" && !k.permissions.imagePermission.imagePermission) {
          ...
          var o, r, n, u;
          $(".diigosc-editpanel-btn div").bind("click",
            function (s) {
              function w() {
    ...
    reInit: function () {
      ...
      function c(t) {
        function D() {
          I("newLine");
          $('<input class="diigosc-textinput"></input>').appendTo("#diigosc-sc").css({top: q + "px", left: o + "px", width: "20px", color: f.drawColor, "font-size": f.fontSize + "px"}).focus().autoGrowInput({comfortZone: 20, minWidth: 20, maxWidth: maxW}).keydown(function (r) {
            if ($(this).width() + 10 > maxW && r.keyCode >= 48 || parseInt($(this).css("top")) -
              startT + 38 > maxH && r.keyCode == 13) return false;
            var n = r.target, u = r.keyCode;
            I(u);
            if (u == 13) {
              q += 18;
              D()
        ...
        var A = $("#bgcanvas").width(), G = $("#bgcanvas").height();
        f.saveText();
        var q = startT = t.y - 10, o = t.x;
        maxW = A - (o - offsetX);
        maxH = G - (q - offsetY);
        textFlag == 1 && D();
      ...
      function d(t, D) {
        function A(o, r) {
          function n() {
            ...
            l.beginPath();
            (function (ba, T, P, V) {
              var X = P / 2 * 0.5522848, ua = V / 2 * 0.5522848, Ha = ba + P, Ua = T + V;
              P = ba + P / 2;
              V = T + V / 2;
              l.moveTo(ba, V);
          ...
          function u() {
            ...
            l.beginPath();
            var ba = G < w ? s : F - s, T = q < y ? s : la - s, P = F - ba, V = la - T;
            l.moveTo(ba, T);
            l.lineTo(P, V);
            l.stroke();
            (function (X) {
              ...
              return Ha
            }([[4, 0], [-10, -5.5], [-10, 5.5]], Math.atan2(V - T, P - ba)), P, V))
          }

          var s = 4, w = o, y = r, B = Math.min(w, G) - s, C = Math.min(y, q) - s, F = Math.abs(w - G) + 2 * s, la = Math.abs(y - q) + 2 * s;
          $("#draw-canvas").css({left: B + "px", top: C + "px", cursor: "crosshair"}).attr({
            width: F,
            height: la
          });
        ...
        $(this).mousemove(function (o) {
          ...
          f.editMode == true && o.width * o.height !=
          0 && f.enableUndo();
          $.Draggable(o)
        })
      }
      ...
      var f = this;
      textFlag = 1;
      var h = document.getElementById("bgcanvas"), i = document.getElementById("draw-canvas"), l = i.getContext("2d"), m = h.getContext("2d");
      h = document.getElementById("bgcanvas");
      offsetX = h.offsetLeft;
      offsetY = h.offsetTop;
      $("#draw-canvas").unbind("mousedown");
      $("#bgcanvas").unbind().click(function (t) {
        a == "diigosc-text-btn" && c({x: t.pageX, y: t.pageY})
      }).mousedown(function (t) {
        if (i.width * i.height != 0) {
          f.saveAction({type: "draw"});
          m.drawImage(i, parseInt($(i).css("left")) -
            offsetX, parseInt($(i).css("top")) - offsetY)
      ...
      var a = this, c = $("#diigosc-sc").find('input[class="diigosc-textinput"]');
      if (c.length) {
        var d = "";
        c.each(function () {
          d += this.value
          ...
          c.each(function () {
            ...
            if (f) {
              var h = parseInt($(this).css("left")),
                i = parseInt($(this).css("top")), l = document.getElementById("bgcanvas").getContext("2d");
      ...
      if (this.actions.length != 0) if (a.width * a.height != 0) {
      ...
      var d = c.getContext("2d"), f = c.width;
      c = c.height;
      switch (a.type) {
        case "draw":
          this.actions.push({
      ...
      function f() {
        ...
        n.append(E.dom.buildOne("option", {value: 0}, ["Add to an Outliner"]));
        n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
        $(E.dom.buildOne("option", {value: -2}, ["Create an Outliner..."])).appendTo(n);
        n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
        M(k.outliners, function (u) {
          n.append(E.dom.buildOne("option", {value: u.id}, [u.title]))
        });
        n.append(E.dom.buildOne("option",
          {value: -5}, [Array(20).join("-")]));
        if (k.myBmLists.length) {
          n.append(E.dom.buildOne("option", {value: -8}, ["Add to a List"]));
          n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
      ...
      function h() {
        ...
        n.append(E.dom.buildOne("option", {value: 0}, ["Share to a Group"]));
        n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
        $(E.dom.buildOne("option", {value: -2}, ["Create a Group..."])).appendTo(n);
        n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
        M(k.myGroups, function (u) {
          n.append(E.dom.buildOne("option", {value: u.name}, [u.displayName]))
        });
        n.append(E.dom.buildOne("option", {value: -5}, [Array(20).join("-")]));
        $(E.dom.buildOne("option", {value: -2}, ["Create a Group..."])).appendTo(n);
        $(E.dom.buildOne("option", {value: -3}, ["Refresh"])).appendTo(n);
        n.change(function () {
          ...
          if (u != 0 && u != -1 && u != -2 && u != -3) {
            ...
            k.groupTagsDict[u] ? i("group", k.groupTagsDict[u]) : z.loadGroupTagsDictionary([u], function () {
      ...
      function i(n, u) {
        if (n == "recommended") $("#diigosc-recommended-tag .diigo-tags-container .loading").hide(); else if (n == "group") {
      ...
      function l(n, u) {
        var s = $("#diigosc-tag-input"), w = x.parseTags(s.val(), true), y = $.inArray(n, w);
        if (u === undefined) y >= 0 ? w.splice(y, 1) : w.push(n); else if (u) y == -1 && w.push(n); else y >= 0 && w.splice(y, 1);
        w = x.unparseTags(w);
        if (w.length) w += " ";
        s.val(w);
        D(s[0]);
      ...
      function m() {
        var n =
          $("#diigosc-tag-input"), u = x.parseTags(n.val(), true);
        $("#diigosc-tag-cloud").find(".diigosc-tag-cloud-item").each(function () {
          var s = $(this), w = s.text();
          $.inArray(w, u) != -1 ? s.addClass("selected") : s.removeClass("selected")
        });
        $("#diigosc-last-used-tag .diigo-tags-container,#diigosc-recommended-tag .diigo-tags-container").find(".diigo-tag").each(function () {
          var s = $(this), w = s.text();
          $.inArray(w, u) != -1 ? s.addClass("selected") : s.removeClass("selected")
        })
      }
      ...
      function t(n, u) {
        ...
        if (k.myTagsWithCount.length == 0) {
          var s = document.getElementById(n), w = document.createElement("span");
          w.id = "no-tag";
      ...
      if (a == "screenshot") {
        var G = document.getElementById("bgcanvas"), q = document.getElementById("draw-canvas");
        q.getContext("2d");
        ...
        if (q.width * q.height != 0) {
          A.saveAction({type: "draw"});
          o.drawImage(q, parseInt($(q).css("left")) -
            offsetX, parseInt($(q).css("top")) - offsetY);
      ...
      if ($("#diigosc-savepopup-all").length > 0) {
        ...
        new Nb("#diigosc-tag-input", {
          resultsClass: "diigosc ac_results2", data: o, mode: "multiple", multipleSeparator: " ,", onItemSelect: function () {
            U.clickedOn = true
          }, id: "diigsc-ac"
        });
        t("diigosc-tag-cloud-container", "diigosc-tag-cloud-item");
        $("#diigosc-savepopup-all").show();
        $("#diigosc-savepopup-wrapper").css("width", document.documentElement.scrollWidth +
          "px").css("height", document.documentElement.scrollHeight + "px");
      ...
      (function () {
        var n =
          document.title, u = window.location.href;
        $("#diigosc-title-input").val(n);
        O.send({name: "initialData"}, function (s) {
          s.preloadedPrefs["prefs.bookmark.privateByDefault"] == "true" && $("#diigosc-privatecheck").find(".op-checkbox-container").addClass("checked")
        });
        $("#diigosc-privatecheck").find(".op-checkbox-container").on("click", function () {
          $(this).toggleClass("checked")
        });
        $("#diigosc-lu-tag").on("click", function () {
          $("#diigosc-last-used-tag").find(".diigo-tag").each(function () {
            l($(this).addClass("selected").text(),
              true)
          })
        });
        f();
        ...
        if (A.reTagShown == false) if (k.recommendedTagsLoaded) {
          i("recommended", k.recommendedTags);
          A.reTagShown = true
        } else z.loadRecommendedTags(u, n, function () {
          i("recommended", k.recommendedTags);
          A.reTagShown = true
        })
      })();
      a == "image" && $("#diigosc-reedit").hide();
      $("#diigosc-title-input").blur(function () {
        $(this).val().length == 0 && $(this).val("Title is needed!").css("color",
          "red")
      }).focus(function () {
        $(this).val() == "Title is needed!" && $(this).val("").css("color", "black")
      });
      $("#diigosc-tag-input").focus(function () {
      ...
      $("#diigosc-content-bottom").find("a").click(function () {
        A.removeSelected()
      });
      ...
      $("#diigosc-lu-tag").click(function () {
        for (var n = $("#diigosc-last-used-tag").find(".diigo-tags-container").find(".diigo-tag"), u = n.length, s = 0; s < u; s++) $("#myTags").tagit("createTag", n[s].title)
      });
      $("#diigosc-re-tag").click(function () {
        for (var n = $("#diigosc-recommended-tag").find(".diigo-tags-container").find(".diigo-tag"), u = n.length, s = 0; s < u; s++) $("#myTags").tagit("createTag", n[s].title)
      });
      ...
      $("#diigosc-list-addBtn").on("click", function () {
        if (!$(this).hasClass("processing")) {
          var n = $("#diigosc-list-addInput").val(), u = $("#diigosc-list-add .diigo-alert-tip"), s = [], w = k.myBmLists.length, y;
          for (y = 0; y < w; y++) s.push(k.myBmLists[y].title);
          if (n.match(/^\s*$/)) u.show().find("span").text("Input a name"); else if ($.inArray(n, s) !== -1) u.show().find("span").text("Name existed !"); else {
            $(this).addClass("processing");
            z.createList(n)
          }
        }
      ...
      $("#diigosc-saveBtn").click(function () {
        ...
        if (!(n.length == 0 || n == "Title is needed!")) {
          ...
          u.length > 0 && K.setUserData({
            tagsUsedLastTime: JSON.stringify(u.slice(0,
              8))
          });
          var s = $("#diigosc-lists").val(), w = $("#diigosc-groups").val(), y = window.location.href;
          if (typeof DiigoData !== "undefined") y = DiigoData.bookmark.url;
          var B = document.title, C = $("#diigosc-privatecheck").find(".op-checkbox-container").hasClass("checked") ? 2 : 0;
          n = {dataURL: a == "screenshot" ? $("#diigosc-thumbnail").find("img").attr("src") : c, title: n, tags: u, mode: C, src_url: y, src_title: B, single_item: true, list: s, groups: w};
          I(n);
          a == "screenshot" ? z.uploadImage(n, "capture", false, y, {}, "png", "", false) : z.uploadImage(n, "image",
            false, y, {}, d, "", false);
          xa(chrome.runtime.id);
          na();
          L.init();
          L.notifyWait(chrome.i18n.getMessage("uploadimage_uploading"), {timeout: 0});
          A.reInit();
          A.removeSelected()
        }
      })
      ...
      var c = this;
      if (!a) {
        var d = document.getElementById("bgcanvas"), f = document.getElementById("draw-canvas");
        f.getContext("2d");
      ...
      var l = {
        dataURL: a, title: h, tags: k.bookmark.tags ? k.bookmark.tags : [], mode: 2, src_url: i, src_title: k.bookmark.title ? k.bookmark.title :
          document.title, single_item: false
      };
      ...
      chrome.storage.local.get(null, function (t) {
        ...
        if (D) {
          if ($(D).find("#extension-data")) {
            var A = $(D).find("#extension-data").attr("saved"), G = $(D).find("#extension-data").attr("url");
            D = D.title;
            if (A == "true") {
        ...
        } else if (typeof DiigoData !==
          "undefined" && !DiigoData.bookmark.saved) diigoNetwork.pdfCreateItem({}, DiigoData, function () {
          if (DiigoData) l.src_url = DiigoData.bookmark.server_url;
          t.researchMode === true && k.bookmark.saved === false ? z.uploadImage(l, "capture", false, i, {}, "png", "", true, t.researchModeData) : z.uploadImage(l, "capture", false, i, {}, "png", "", true)
        }); else {
          if (typeof DiigoData !== "undefined") l.src_url = DiigoData.bookmark.server_url;
          t.researchMode === true && k.bookmark.saved === false ? z.uploadImage(l, "capture", false, i, {}, "png", "", true, t.researchModeData) :
            z.uploadImage(l, "capture", false, i, {}, "png", "", true)
        }
        xa(chrome.runtime.id);
        na();
  ...
  chrome.runtime.onMessage.addListener(function (a) {
    if (a.name == "dataURL") {
      var c = a.cw, d = a.ch, f = a.ct, h = a.cl;
      $("#bgcanvas").attr("width", c);
      $("#bgcanvas").attr("height", d);
      $("#bgcanvas").css("top", f + "px");
      $("#bgcanvas").css("left", h + "px");
      ka.editMode = true;
      var i = document.getElementById("bgcanvas"), l = i.getContext("2d"),
        m = document.createElement("img");
      document.getElementById("diigolet_screenshot_center");
      var t = f - document.documentElement.scrollTop, D = h - document.documentElement.scrollLeft;
      m.onload = function () {
        l.drawImage(m, D * window.devicePixelRatio, t * window.devicePixelRatio, c * window.devicePixelRatio, d * window.devicePixelRatio, 0, 0, c, d);
        ...
        Na({
          type: "init",
          dataUrl: A, initTool: ka.initTool
        });
        $("#diigo-screenshot-frame").css({top: f + "px", left: h + "px", width: c + "px", height: d + "px"}).show();
        if (a.target == "diigosc-tag-btn") ka.showSavePupop("screenshot", A); else a.target == "diigosc-attach-btn" && ka.attachToBookmark(A)
      };
      m.src = a.dataURL
    }
  });
  ...
  jQuery.Draggable = function (a, c) {
    ...
    a[0]._draggable = this;
    var m = this.options = i.extend({
      handle: "", cursor: "move", axis: "", onDrag: function () {
        return true
      }, beforeDrag: function () {
      }, afterDrag: function () {
      }
    }, c || {});
    m.handle = m.handle ? i(m.handle,
      a) : a;
    m.handle.css({cursor: m.cursor});
    var t, D;
    this.destroy = function () {
  ...
  (function (a) {
    a.fn.autoGrowInput = function (c) {
      ...
      this.filter("input:text").each(function () {
        var d = c.minWidth || a(this).width(), f = "", h = a(this), i = a("<tester/>").css({
          position: "absolute", top: -9999, left: -9999, width: "auto", fontSize: h.css("fontSize"), fontFamily: h.css("fontFamily"), fontWeight: h.css("fontWeight"), letterSpacing: h.css("letterSpacing"),
          whiteSpace: "nowrap"
        });
        i.insertAfter(h);
  ...
  var wa = {
    diigoImageClipMode: false, imageSrc: "", shown: false, init: function () {
      this.create()
    }, create: function () {
      var a = this;
      if (!($("#diigo-image-clipper").length >
        0)) {
        var c = document.createElement("div");
        c.id = "diigo-image-clipper";
        c.innerHTML = '<div id="diigo-image-menu"><div id="diigo-logo"></div><div id="diigo-save-and-tag"class="diigo-action" title="Tag as a stand-alone item"></div><!----\><div id="diigo-attach"class="diigo-action" title="Attach it to the page URL"></div></div>';
        ...
        var d;
        $("#diigo-image-menu").bind("mouseenter", function () {
          d != null && clearTimeout(d);
      ...
      $(document).bind("mousemove", function (d) {
        if (d.target.tagName == "IMG") {
          if (a.diigoImageClipMode) {
            ...
            d = d.target;
            var f = $(d).offset().left, h = $(d).offset().top, i = $(d).width(), l = $(d).height();
            if (a.shown ==
              false) if (i >= 200 || l >= 200) {
              c = h + l > document.body.scrollTop + window.innerHeight ? setTimeout(function () {
                $("#diigo-image-clipper").css("top", h + "px").css("left", f + i - 22 + "px").show()
              }, 350) : setTimeout(function () {
                $("#diigo-image-clipper").css("top", h + l - 24 + "px").css("left", f + i - 22 + "px").show()
              }, 350);
              a.shown = true
            }
          }
          ...
          i = d.width();
          l = d.height();
          if (i >= 200 || l >= 200) h + l > document.body.scrollTop +
          window.innerHeight ? $("#diigo-image-clipper").css("top", h + "px").css("left", f + i - 22 + "px").show() : $("#diigo-image-clipper").css("top", h + l - 24 + "px").css("left", f + i - 22 + "px").show()
        } else if (!(d.target.id == "diigo-save-and-tag" || d.target.id == "diigo-attach" || d.target.id == "diigo-image-menu" || d.target.id == "diigo-logo")) {
      ...
      if (qa()) if (typeof k.permissions.imagePermission !==
        "undefined" && !k.permissions.imagePermission.imagePermission) {
        ca.init();
        ca.show("image")
      } else chrome.runtime.sendMessage({name: "saveImageToDiigo", details: {srcUrl: wa.imageSrc}, Type: "tag"})
    }, attachToBookmarkOnClick: function () {
      if (qa()) if (typeof k.permissions.imagePermission !== "undefined" && !k.permissions.imagePermission.imagePermission) {
        ca.init();
        ca.show("image")
      } else chrome.runtime.sendMessage({name: "saveImageToDiigo", details: {srcUrl: wa.imageSrc}, Type: "attach"})
    }
  };
  (function (a) {
    var c, d;
    a.fn.Gtooltip = function (f) {
      var h =
        {fontSize: 12, bgColor: "black", fColor: "white", position: "bottom", content: ""};
      return this.each(function () {
        function i() {
          D.text(m.data("gtooltip"));
          var G;
          G = m.offset().left;
          var q = m.offset().top, o = m.width(), r = m.height(), n = t.width(), u = t.height(), s, w, y, B, C = navigator.userAgent.toLowerCase();
          ...
          if (l.position == "top") A.css("-webkit-transform", "rotate(180deg)"); else l.position == "bottom" && A.css("-webkit-transform", "rotate(360deg)");
          t.addClass("show")
        }

        var l = a.extend(h, f || {}), m = a(this), t, D, A;
  ...
  (function (a) {
    if (typeof define ===
      "function" && define.amd) define(["jquery"], a); else if (typeof module !== "undefined" && module.exports) module.exports = a(require("jquery")); else a(jQuery)
  })(function (a) {
    function c(h) {
    ...
    a.fn.scrollTo = function (h, i, l) {
      ...
      return this.each(function () {
        function t(u) {
          var s = a.extend({}, l, {
            queue: true, duration: i, complete: u && function () {
              u.call(A, q, l)
            }
          });
          G.animate(o, s)
        ...
        if (h !== null) {
          var D = c(this), A = D ? this.contentWindow || window : this, G = a(A), q = h, o = {}, r;
          switch (typeof q) {
            case "number":
            case "string":
              if (/^([+-]=?)?\d+(\.\d+)?(px|%)?$/.test(q)) {
          ...
          a.each(l.axis.split(""), function (u, s) {
            var w = s === "x" ? "Left" : "Top", y = w.toLowerCase(), B = "scroll" + w, C = G[B](), F = f.max(A, s);
            if (r) {
              ...
              w = q[y];
              o[B] =
                w.slice && w.slice(-1) === "%" ? parseFloat(w) / 100 * F : w
            }
            if (l.limit && /^\d+$/.test(o[B])) o[B] = o[B] <= 0 ? 0 : Math.min(o[B], F);
            if (!u && l.axis.length > 1) if (C === o[B]) o = {}; else if (m) {
              t(l.onAfterFirst);
              o = {}
            }
          });
    ...
    f.max = function (h, i) {
      var l = i === "x" ? "Width" : "Height", m = "scroll" + l;
      if (!c(h)) return h[m] - a(h)[l.toLowerCase()]();
      l = "client" + l;
      var t = h.ownerDocument || h.document, D = t.documentElement;
      t = t.body;
  ...
  (function (a) {
    ...
    a.scrollmarker.fn.extend =
      a.scrollmarker.extend = a.extend;
    a.scrollmarker.fn.extend({
      init: function () {
        var c = this, d = this.place.attr("class").match(/id_([\w]+)/)[0];
        a("#" + d).length != 0 && a("#" + d).remove();
        this.marker = a('<div class="diigo-scrollmarker" title="Diigo highlight marker"><div class="inner"></div></div>').attr("id", d).appendTo(a("body")).click(function () {
          c.locate()
        }).addClass(this.options.markerClass || this.place.data("scrollmarker-class"));
        if (this.place.hasClass("blue")) this.marker.addClass("blue"); else if (this.place.hasClass("green")) this.marker.addClass("green");
        ...
        this.marker.height();
        var d = a(document).height(), f = a(window).height();
        ...
        var c = this;
        a(document).scrollTo(this.place.offset().top -
          50, 300, function () {
  ...
  var zb = function () {
    ...
    c.init = function () {
      ...
      $("#diigolet-highlight-footer").on("click", "a", function (f) {
        if (d.enable) {
          ...
          } else if (f === "diigolet-highlight-share-cancelbtn") d.hide(); else if (f === "diigolet-highlight-share-twitter") O.send({name: "openWindow", details: {url: "https://twitter.com/intent/tweet?text=" + encodeURIComponent(d.editor.val()) + " @diigo", name: "sharer", features: "toolbar=1,status=0,resizable=1,scrollbars=1,width=700,height=453"}});
          else if (f === "diigolet-highlight-share-facebook") {
            f = encodeURIComponent(document.title);
            var h = encodeURIComponent(d.ann.prettyTxt), i = encodeURIComponent(d.url);
            O.send({name: "openWindow", details: {url: "http://www.facebook.com/sharer.php?s=100&p[title]=" + f + '&p[summary]="' + h + '"&p[url]=' + i + "&p[images][0]=https://www.diigo.com/images/v6/logo.png", name: "sharer", features: "toolbar=1,status=0,resizable=1,scrollbars=1,width=700,height=453"}})
          }
        }
      });
      $("#diigolet-highlight-share-top").find(".diigolet-question-mark").hover(function () {
    ...
    j: null, k: null, init: function () {
      ...
      var c = $('<div class="diigo-orphanHighlight-item diigo-' + a.extra.color + '"><span class="diigo-orphanHighlight-item-content"></span><div class="diigo-orphanHighlight-item-copy" title="Copy this highlight">Copy</div></div>');
      ...
      $("#diigo-orphanHighlight-top").find(".diigolet-question-mark").hover(function () {
          $(this).find(".diigolet-question-mark-tip").fadeIn()
        },
        function () {
          $(this).find(".diigolet-question-mark-tip").fadeOut()
      ...
      this.j.css({left: $(window).width() - this.j.outerWidth() - 10 + "px", top: "78px"});
      this.j.show()
    }, hide: function () {
      this.j.hide()
    }
  }, aa = {
    j: null, k: null, shown: false, init: function () {
      var a = this;
      this.j = $('<div id="diigo-annotationList"><div id="diigo-annotationList-top" class="_dragHandle"><b></b><span>Annotations</span><div id="diigo-annotationList-close"></div></div><div id="diigo-annotationList-main"><div id="diigo-annotationList-notification">Added to clipboard</div><div id="diigo-annotationList-box" class="diigo-customize-scrollbar"></div></div><div id="diigo-annotationList-toolbar"><span><b class="diigo-orphan-warning"></b> Some highlighted text now missing on this page.</span><a id="diigo-annotationList-copyall" href="#">Copy all</a></div></div>\n').appendTo($("body")).hide();
      for (var c = k.annotations, d = 0; d < c.length; d++) a.add(c[d]);
      a.updatecontent();
      k.orphanHighlights.length === 0 && $("#diigo-annotationList-toolbar").find("span").toggle(k.orphanHighlights.length > 0);
      $("#diigo-annotationList-close").on("click", function () {
        a.hide()
      });
      $("#diigo-annotationList-copyall").on("click", function (f) {
        f.preventDefault();
        var h = x.AnnotationFormatter.format({mode: 1, onlyPrivate: true});
        z.makeAnnotatedLink(k.bookmark.url, 3, function (i) {
          i && i.url && x.Messenger.send({
            name: "copy", details: {
              html: '"' + h +
                '"\n ' + i.url
            }
      ...
      if (a.onlyInGroup == false && a.user == k.user) {
        ...
        if (a.type == ia) {
          d = a.extra.color;
          d = $('<div class="diigo-annotationList-item diigo-' +
          ...
          d.find(".diigo-annotationList-item-copy").on("click", function (i) {
            ...
            $(this).parent().parent().find(".diigo-annotationList-commentItem").each(function () {
              l += "\n" + $(this).text()
            });
            z.makeAnnotatedLink(k.bookmark.url, 3, function (m) {
              m && m.url && x.Messenger.send({name: "copy", details: {text: '"' + l + '"\n ' + m.url}})
            });
            $("#diigo-annotationList-notification").fadeIn().delay(1E3).fadeOut()
          });
          var h = this;
          d.find(".diigo-annotationList-item-delete").on("click", function (i) {
            ...
            if (!a.onlyInGroup) {
              N.find(a.id) ?
                N.find(a.id).del() : h.editItem("remove", a);
              document.getElementById("diigo-reader-frame").contentWindow.postMessage({type: "removeAnnotation", data: {ann: a.id}}, "*")
            }
            M(pa(a.groups, function (l) {
              return l.name
            }), function (l) {
              a.del(l)
            })
      ...
      $("#diigo-annotationList-box").find(".diigo-annotationList-item").length ===
      0 && $("#diigo-annotationList-box").append('<div id="diigo-annotationList-noItem">You have no annotations yet. Create a highlight or sticky note to see it here.</div>');
      ...
      if (this.j) {
        var f = c ? this.j.find('[data-ann-id="' +
          c.id + '"]') : this.j.find('[data-ann-id="' + d.annotationId + '"]');
        if (f && f.length > 0) switch (a) {
          case "remove":
            f.remove();
          ...
          case "updateStickyNote":
            ...
            a = c.type;
            if (a == ia) f.find(".diigo-annotationList-commentItem").text(d.content); else a == fa && f.find(".diigo-annotationList-item-content").text(d.content);
            break;
          case "addInlineComment":
            $("#diigo-annotationList-noItem").remove();
            $('<div class="diigo-annotationList-commentItem" data-cid="' + d.id + '">' + d.content + "</div>").appendTo(f.find(".diigo-annotationList-commentContainer"));
            break;
          case "delInlineComment":
            f.find(".diigo-annotationList-commentContainer").find('.diigo-annotationList-commentItem[data-cid="' + d.id + '"]').remove();
            break;
          default:
        }
      }
  ...
  setTimeout(function () {
    Sb() && K.get("prefs.showPdfButton", function (a) {
      if (a === "true") {
        ...
        $("#diigo-enter-pdf").on("click", function () {
          k.permissions.pdf ? chrome.runtime.sendMessage({name: "openPdf", url: window.location.href}) : $("#diigo-pdf-premium").show()
        })
      }
    })
  }, 1E3);
    ...
    Xb = {
      j: null, init: function () {
        ...
        this.j.find("#diigo-ghost").on("dragover", function (c) {
          ...
          a.j.find("#diigo-reader-frame")[0].contentWindow.postMessage({type: "event", data: {e: c.originalEvent}}, "*")
        });
        window.addEventListener("message", function (c) {
          ...
          if (c.type == "processStart") {
            c = document.location;
            c = {spec: c.href, host: c.host, prePath: c.protocol + "//" + c.host, scheme: c.protocol.substr(0, c.protocol.indexOf(":")), pathBase: c.protocol + "//" + c.host + c.pathname.substr(0, c.pathname.lastIndexOf("/") + 1)};
            a.j.find("#diigo-reader-frame")[0].contentWindow.postMessage({type: "doc_html", data: {html: document.body.innerHTML, uri: c, title: document.title}}, "*")
          } else if (c.type == "processFinish") a.hideLoading(); else if (c.type == "refreshBookmark") chrome.runtime.sendMessage({name: "refreshBookmark"});
      ...
      j: null, k: null, port: null, opened: false, showed: false, outlinerId: null, init: function () {
        window.CSP = true;
        var a = this;
        if (!this.j) this.j = $('<div id="diigo-outliner-frame"><div id="diigo-outliner-toggle-btn"><div class="expand" title="Expand outliner sidebar"></div><div class="collapse" title="Collapse outliner sidebar"></div></div><div id="diigo-ghost"><input type="text"></div><!--<div id="diigo-outliner-frame-close"></div>--\><iframe id="diigo-outliner-iframe" src="" frameborder="0"></iframe></div>\n').appendTo("body");
        $(document).on("dragstart", function () {
          a.j.find("#diigo-ghost").show()
        });
        $(document).on("dragend", function () {
        ...
        this.j.find("#diigo-ghost").on("dragover", function (c) {
          a.port.postMessage({type: "dragover-event", pos: {x: c.originalEvent.clientX - $(a.j).offset().left, y: c.originalEvent.clientY}})
        }).on("dragleave", function () {
          a.port.postMessage({type: "dragleave-event"})
        }).on("drop",
          function (c) {
            var d = {type: "drop-event", text: c.originalEvent.dataTransfer.getData("text/plain"), url: window.location.href, pos: {x: c.originalEvent.clientX - $(a.j).offset().left, y: c.originalEvent.clientY}};
            ...
            } else if (c.originalEvent.dataTransfer.getData("text/html") &&
              $(c.originalEvent.dataTransfer.getData("text/html"))[0].tagName == "META") if ($(c.originalEvent.dataTransfer.getData("text/html"))[1].tagName == "IMG") {
        ...
        window.addEventListener("message", function (c) {
          if (c.data.type == "port") a.port = c.ports[0]; else c.data.type == "outliner" && a.j.find("iframe").attr("src", c.data.url)
        });
        this.detectCSP()
      }, detectCSP: function () {
        var a =
        ...
        setTimeout(function () {
          a.j.find("#diigo-outliner-iframe").attr("src") == "" && K.get("prefs.outlinerFrameUrl", function (c) {
            c ||
            (c = "https://www.diigo.com/outliner/index");
            if ($(document.body).hasClass("noCSP")) {
              if (/csp=true/.test(c)) c = c.replace(/[\?&]csp=true/i, "")
            } else if (c == "https://www.diigo.com/outliner/index") c += "?csp=true"; else /csp=true/.test(c) || (c += "&csp=true");
