...
$(document).ready(function () {
  ...
  buildSelect();
  prefs2UI();
  $(".diigolet-question-mark").hover(function () {
  ...
  if (Bg.GlobalData.permissions.autoShowAnnotation) $(".tip-premium").addClass("show"); else {
    $(".tip-no-premium").addClass("show");
    $("#search-tip-premium").show()
  }
  (function () {
    var a = $("#check-autoloadBookmarkStatus"), b = $("#check-autoload"), c = $("#check-showSearchResultDirectly"), e = $("#check-comboSearch");
    a.change(function () {
    ...
    c.change(function (d) {
      if (Bg.GlobalData.permissions.autoShowAnnotation) c.is(":checked") && e.attr("checked", true); else {
        d.preventDefault();
        c.attr("checked",
          false);
    ...
    $("#shortcut-annotate select").change(function () {
      ...
      $('#shortcut-bookmark select option[value="' +
...
function buildSelect() {
  for (var a = ["Q", "D"], b = $("<select></select>"), c = 48; c < 91; c++) if (!(c > 57 && c < 65)) {
...
function shortcutSwitchState(a) {
  $("#check-shortcut-" + a).is(":checked") ? $("#check-shortcut-" + a + "-ctrl,#check-shortcut-" + a + "-alt,#shortcut-" + a + " select").removeAttr("disabled") : $("#check-shortcut-" + a + "-ctrl,#check-shortcut-" + a + "-alt,#shortcut-" + a + " select").attr("disabled", "true")
}
...
function prefs2UI() {
  ...
  $("#check-showPdfButton").attr({
    checked: Prefs.get("prefs.showPdfButton") ==
      "true"
  });
  shortcutSwitchState("bookmark");
  shortcutSwitchState("annotate");
...
function UI2prefs() {
  Prefs.set({"prefs.bookmark.privateByDefault": $("#check-bmPrivateAsDefault").is(":checked")});
  Prefs.set({"prefs.bookmark.unreadByDefault": $("#check-bmUnreadAsDefault").is(":checked")});
  Prefs.set({"prefs.autoloadBookmarkStatus": $("#check-autoloadBookmarkStatus").is(":checked")});
  Prefs.set({"prefs.autoload": $("#check-autoload").is(":checked")});
  Prefs.set({"prefs.contextMenu": $("#check-contextMenu").is(":checked")});
  Prefs.set({"prefs.comboSearch": $("#check-comboSearch").is(":checked")});
  Prefs.set({"prefs.SearchO": $("#check-searcho").is(":checked")});
  Prefs.set({"prefs.autoShowHighlightIcon": $("#check-autoshowicon").is(":checked")});
  Prefs.set({"prefs.autoImageClipper": $("#check-autoImageClipper").is(":checked")});
  Prefs.set({"prefs.shortcutAnnotate": $("#check-shortcut-annotate").is(":checked")});
  Prefs.set({"prefs.shortcutBookmark": $("#check-shortcut-bookmark").is(":checked")});
  Prefs.set({"prefs.shortcutReadlater": $("#check-shortcut-readlater").is(":checked")});
  Prefs.set({"prefs.shortcutAnnotateCtrl": $("#check-shortcut-annotate-ctrl").is(":checked")});
  Prefs.set({"prefs.shortcutAnnotateAlt": $("#check-shortcut-annotate-alt").is(":checked")});
  Prefs.set({"prefs.shortcutBookmarkCtrl": $("#check-shortcut-bookmark-ctrl").is(":checked")});
  Prefs.set({"prefs.shortcutBookmarkAlt": $("#check-shortcut-bookmark-alt").is(":checked")});
  Prefs.set({"prefs.shortcutReadlaterCtrl": $("#check-shortcut-readlater-ctrl").is(":checked")});
  Prefs.set({"prefs.shortcutReadlaterAlt": $("#check-shortcut-readlater-alt").is(":checked")});
  Prefs.set({"prefs.shortcutAnnotateKey": $("#shortcut-annotate select").val()});
  Prefs.set({"prefs.shortcutBookmarkKey": $("#shortcut-bookmark select").val()});
  Prefs.set({"prefs.shortcutReadlaterKey": $("#shortcut-readlater select").val()});
  Prefs.set({"prefs.autoCloseReadLater": $("#check-autoCloseReadLater").is(":checked")});
  Prefs.set({"prefs.directlyShowSearchResults": $("#check-showSearchResultDirectly").is(":checked")});
  Prefs.set({"prefs.showPdfButton": $("#check-showPdfButton").is(":checked")})
}
...
var refreshWindow = {
  j: null, k: null, shown: false, init: function () {
          ...
          user: Bg.GlobalData.user, transId: null, complete: function (b) {
            if (b.resp) {
              ...
              forEachWebTab(function (c) {
                Messenger.send(c.id,
                  {name: "myStuffLoaded", details: b.resp.result, fromTabId: null})
              })
            }
          }
        })
    ...
    if (a.shown != true) {
      ...
      this.j.css("left", (window.screen.availWidth - 356) / 2 + "px").css("top",
        b + "px");
      a.j.show();
      a.shown = true;
...
function showUpgradeWin() {
  var a = $('<div class="modelWin-wrapper"><div class="modelWin"><div class="modelWin-close"></div><div class="modelWin-content">The ability to show highlights and notes automatically when you return to your annotated pages is a premium feature, and the free trial has expired.</div><div class="modelWin-action"><a class="modelWin-primary-btn" id="upgrade" href="https://www.diigo.com/premium" target="_blank">Upgrade now</a></div></div></div>').appendTo($("html"));
  $("#upgrade").on("click",
    function () {
      refreshWindow.show();
      a.remove()
