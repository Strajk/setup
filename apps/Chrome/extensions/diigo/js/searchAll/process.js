var cloneRoot = null, waitCount = 1;
...
function toBase64(c) {
  ...
  if (d % 3) {
    ...
    if (d % 3 == 2) {
      ...
      a += toBase64Table[(c.charCodeAt(b) &
        3) << 4];
...
function processImage(c, a) {
  var d = a.cloneNode(false), b = a.src;
  d.url = b;
...
function processStyleSheet(c) {
  ...
  for (var a = [], d = 0; d < c.cssRules.length; d++) {
    var b = c.cssRules[d];
    if (b.type == b.IMPORT_RULE) a.push(processStyleSheet(b.styleSheet)); else b.type != b.CHARSET_RULE && a.push(b.cssText)
  }
  return a.join("\n")
}
...
function processStyles(c) {
  if (document.styleSheets) for (var a = 0; a < document.styleSheets.length; a++) {
    var d = document.styleSheets[a], b = document.createElement("style");
...
function cleanPlugins() {
  for (var c = ["embed", "object", "applet"], a = 0, d = c.length; a < d; a++) for (var b = cloneRoot.getElementsByTagName(c[a]), e = 0, f = b.length; e < f; e++) if (b[e]) b[e].style.display = "none"
}
