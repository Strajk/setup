var TwitterOAuth = {
  URL_OAUTH_BASE: "https://api.twitter.com/oauth/", URL_API_BASE: "https://api.twitter.com/1.1/", comsuerKey: "jPjTPeXVlxhx5Zqw3uTmGw", consumerSecret: "eLOSDUs7aHHn4p1ocSYz2xRUVNUfkDSjtf2DQlKUY", init: function () {
    var a = this;
    a.reset();
    ...
    var b = this, c = {method: a.method || "POST", action: (a.oauth ? b.URL_OAUTH_BASE : b.URL_API_BASE) + a.url, parameters: {oauth_signature_method: "HMAC-SHA1", oauth_consumer_key: b.comsuerKey}}, e = {consumerSecret: b.consumerSecret, tokenSecret: b.oauth_token_secret};
    ...
    $.ajax({
      type: c.method, url: c.action, timeout: 6E4, data: OAuth.getParameterMap(c.parameters),
    ...
    a.reset();
    a.makeRequest({
      url: "request_token", oauth: true, method: "GET", successCallback: function (b) {
        var c = OAuth.getParameterMap(b);
        if (!"oauth_token oauth_token_secret".split(" ").every(function (e) {
          return !!c[e]
        })) {
          error("[oauth request token error]. data not correct",
            b);
          return null
        }
        a.oauth_token = c.oauth_token;
        a.oauth_token_secret = c.oauth_token_secret;
        chrome.tabs.create({url: a.URL_OAUTH_BASE + "authorize?oauth_token=" + a.oauth_token, selected: true}, function (e) {
          a.oauthPinTab = e;
    ...
    if (a) {
      ...
      chrome.tabs.update(D.switchToTabIdAfterTwitterOauth,
        {selected: true});
      c.makeRequest({
        url: "access_token", method: "POST", oauth: true, params: {oauth_verifier: a}, successCallback: function (e) {
          var f = OAuth.getParameterMap(e);
          if ("oauth_token oauth_token_secret user_id screen_name".split(" ").every(function (d) {
            return !!f[d]
          })) {
            ...
            forEachWebTab(function (d) {
              Messenger.send(d.id, {name: "twitterSignIn", details: {screen_name: c.screen_name}})
            })
          } else {
            error("[oauth access_token] failed",
              e);
            c.reset()
          }
    ...
    var a = this;
    setTimeout(function () {
      chrome.tabs.executeScript(a.oauthPinTab.id, {file: "js/twitterGrabPin.js"})
    }, 500);
  ...
  changeUser: function () {
    ...
    Prefs.setUserData({twitterOauth: this.rot13(btoa(JSON.stringify({oauth_token: this.oauth_token, oauth_token_secret: this.oauth_token_secret, user_id: this.user_id, screen_name: this.screen_name})))})
  }, rot13: function (a) {
    return a.replace(/[a-zA-Z]/g, function (b) {
      return String.fromCharCode((b <= "Z" ? 90 : 122) >= (b = b.charCodeAt(0) +
        13) ? b : b - 26)
    })
    ...
    if (a) try {
      ...
      if (!"oauth_token oauth_token_secret user_id screen_name".split(" ").every(function (c) {
        return !!a[c]
      })) {
  ...
  clearStoredOauthInfo: function () {
    ...
    return (this.oauth_token || this.loadOauthInfo()) && {screen_name: this.screen_name}
  }, API_statuses_home_timeline: function () {
    this.makeRequest({
      url: "statuses/home_timeline.json", method: "GET", params: {count: 3}, successCallback: function (a) {
        debug("[twitter home timeline]", a)
      }
    })
  }, API_friendships_exists: function () {
    this.makeRequest({
      url: "friendships/exists.json", method: "GET", params: {user_a: this.screen_name, user_b: "diigo"},
    ...
    this.makeRequest({
      url: "statuses/update.json", method: "POST", params: {status: a.status}, successCallback: function (c) {
