var D = window.D || {}, newClickVersion = "3.3.91";
...
function resetGlobalData() {
  GlobalData = {signedIn: false, user: "", userId: null, realName: "", userLevel: 0, myTags: [], myTagsWithCount: [], myBmLists: [], myGroups: [], myContacts: [], permissions: {capture: true, image: true, premium: true, snapshot: true, highlight: true}}
}

var SignInManager = D.SignInManager = {
  checkSignInCookie: function (a) {
    var b = this;
    chrome.cookies.get({url: "https://www.diigo.com", name: "diigoandlogincookie"}, function (c) {
      if (c) var d = c.value.split("-.-")[1];
      b.onUsernameSeen(d);
      a && a()
    })
  }, onUsernameSeen: function (a) {
    if (a) if (this.isSignedIn()) {
      if (a != GlobalData.user) {
        this._onSignOut();
        this._onSignIn(a)
      }
    } else this._onSignIn(a); else this.isSignedIn() && this._onSignOut()
  }, isSignedIn: function () {
    ...
    forEachWebTab(function (b) {
      Messenger.send(b.id, {name: "signIn", details: {user: a}, fromTabId: null})
    });
    ...
    forEachWebTab(function (a) {
      Messenger.send(a.id,
        {name: "signOut", fromTabId: null})
    })
  }, loadMyStuff: function () {
    debug("[SignInManager] loadMyStuff", GlobalData.user);
    (new WebAPICall).invoke("user_loadMyStuff", {what: "myTags myGroups myProfile myBookmarkLists myContacts permissions"}, {
      user: GlobalData.user, transId: null, complete: function (a) {
        if (a.resp) {
          GlobalData.permissions = a.resp.result.permissions;
          if (typeof localStorage["prefs.directlyShowSearchResults"] === "undefined" && typeof localStorage["prefs.shortcutAnnotate"] !== "undefined") if (GlobalData.permissions.autoShowAnnotation) {
            localStorage["prefs.comboSearch"] =
              "true";
          ...
          if (typeof b !== "undefined" && b == false) if (localStorage.free_trial_count) {
          ...
          forEachWebTab(function (c) {
            Messenger.send(c.id, {
              name: "myStuffLoaded",
              details: a.resp.result, fromTabId: null
            })
          })
        }
      }
  ...
  table: {}, init: function () {
    ...
    if (b.status == "loading") {
      ...
      if (SignInManager.isSignedIn()) {
        ...
        if (isURLApplicable(b)) {
          ...
          e.invoke("bm_loadBookmark", {url: b, what: "bookmarkInfo annotations pageComments"}, {
            user: GlobalData.user, complete: function (f) {
              d.table[a].loading =
                false;
              if (!f.cancelled) if ((f = f.resp) && f.code == 1) {
                d.table[a].result = f.result;
                d.user = f.user;
                f.result.saved && d.updateBrowserActionIcon(c.id, true);
                Messenger.send(a, {name: "refreshBookmarkInfo"});
                Prefs.get("prefs.autoload") == "true" && d.hasPrivateAnnotation(f.result.annotations, f.user) && sendRunCommand(a, {type: "autoshow"})
              }
            }
          })
        }
      }
    } else if (b.status == "complete") if (d.table[a]) {
      if (d.table[a].loading != true) {
        b = d.table[a].result;
        Prefs.get("prefs.autoload") == "true" && d.hasPrivateAnnotation(b.annotations, d.user) && sendRunCommand(a,
          {type: "autoshow"})
      }
    ...
    return (a =
      this.table[a]) && a.loading
  }, updateBrowserActionIcon: function (a, b) {
    chrome.browserAction.setIcon({tabId: a, path: "img/" + (b ? "logo-19-bookmarked.png" : "logo-19.png")})
  }
};

function isURLApplicable(a, b) {
  var c = parseUri(a), d = c.path;
  if (!/^https?$/i.test(c.protocol)) return false;
  if (/\.?diigo\./i.test(c.host)) if (/^\/annotated\//i.test(d)) return false; else if (/(player|slides?)\.diigo\./i.test(c.host) && /^\/(list|feed)/i.test(d)) return false; else if (/^\/bookmark\//i.test(d)) return false; else if (/^\/user\//i.test(d)) return false; else if (/^\/cached/i.test(d)) return false; else {
...
var WebAPI = {
  saveBookmark: function (a, b) {
    ...
    this.webCall("user_loadMyStuff", {what: "myTags myGroups myProfile myBookmarkLists myContacts permissions"}, null, a)
    ...
    this.webCall("mhtml_upload_done", a, null, b)
                        ...
                        b, c, d) {
    var e = new WebAPICall;
    c != "saveLink" && chrome.browserAction.setIcon({tabId: b.tabId, path: "/img/doing-icon.png"});
    e.invoke(a, b, {
      user: GlobalData.user, complete: function (f) {
        console.log(f.status, f.resp);
        if (f) if (f.resp.code == 1) {
          if (c != "saveLink") {
            ...
            if (a == "bm_saveBookmark" || a == "bm_deleteBookmark") chrome.tabs.sendMessage(b.tabId,
              {name: "saveBookmarkSuccess", data: f.resp.result, ctx: b}); else a == "mhtml_upload_done" && chrome.tabs.sendMessage(b.tabId, {name: "saveMHTMLSuccess", result: f.resp.result})
          }
          if (d) if (a == "list_create") {
            GlobalData.outliners.push(f.resp.result.list);
            GlobalData.outliners.sort(function (h, i) {
              return h.title > i.title ? 1 : -1
            });
            console.log(a, d);
            d({outliners: GlobalData.outliners, lists: GlobalData.myBmLists, newOutliner: f.resp.result.list})
          } else if (a == "user_loadMyStuff") {
            extend(GlobalData, f.resp.result);
            d({
              outliners: GlobalData.outliners,
              lists: GlobalData.myBmLists, groups: GlobalData.myGroups
            })
          } else d()
        }
      }
...
var Messenger = {
  send: function (a, b, c) {
    ...
    showToolbar: function (a) {
      ...
      chrome.tabs.captureVisibleTab({format: "png"}, function (b) {
        getSelectedTab(function (c) {
          ...
          chrome.tabs.sendMessage(c.id, {name: "dataURL", dataURL: b, cw: a.cw, ch: a.ch, ct: a.ct, cl: a.cl, target: a.target})
        })
      })
      ...
      (new WebAPICall).invoke(d.cmd, d.data, {
        user: GlobalData.user, transId: d.transId, isInGroup: d.isInGroup, complete: function (e) {
          console.log(d.cmd, e.status, e.resp, c);
          e && c({status: e.status, resp: e.resp, cmd: e.cmd})
        }
      })
    }, ifSignIn: function () {
      chrome.cookies.get({url: "http://www.diigo.com", name: "diigoandlogincookie"},
        function (a) {
          a && getSelectedTab(function (b) {
      ...
      getSelectedTab(function (a) {
        console.log("tab", a);
        a.openerTabId && chrome.tabs.get(a.openerTabId, function (b) {
          var c, d = RegExp("https?://(?:.+\\.)?google\\.[a-z]{2,3}(?:\\.[a-z]{2})?(?:\\:[0-9]+)?/(?:advanced_)?search.*(([?&]q=)([^&]+))"), e = RegExp("https?://(?:.+\\.)?google\\.[a-z]{2,3}(?:\\.[a-z]{2})?(?:\\:[0-9]+)?/(?:webhp[^#]*)?(#((?:.+&)?q=)([^&]+)(.*)&fp=[^&]+)");
          ...
          chrome.tabs.sendMessage(a.id, {name: "googleSearchRecommend", googleSearchKeyWord: c})
        })
      })
      ...
      chrome.tabs.query({}, function (c) {
        c.forEach(function (d) {
          if (d.url === "https://www.diigo.com/outliner") chrome.tabs.executeScript(d.id, {code: "window.postMessage({action: 'refreshPage'}, '*');"},
            function () {
              console.log(d, "notify update list")
            }); else d.id !== b.tab.id && chrome.tabs.sendMessage(d.id, {name: "refreshOutliner"})
        })
      })
      ...
      forEachWebTab(function (f) {
        Messenger.send(f.id, {name: "globalDataChanged", details: {changes: c}, fromTabId: b.tab.id})
      })
      ...
      switch (a.type) {
        case "highlight":
          ...
          break;
        case "image":
          GlobalData.permissions.imagePermission =
            c;
          break;
        case "pdf":
      ...
      forEachWebTab(function (d) {
        Messenger.send(d.id, {name: "globalDataChanged", details: {changes: {permissions: GlobalData.permissions}}, fromTabId: b.tab.id})
      })
    }, initialData: function (a, b, c) {
      c({
        extensionID: D.extensionID, globalData: GlobalData, preloadedPrefs: Prefs.get(Prefs.prefsToPreload), tabId: b.tab.id, version: D.version, loadBookmarkResult: Preloader.preloadedResultForTab(b.tab.id), shouldLoadBookmarkByMyself: !Preloader.isPreloadingForTab(b.tab.id),
      ...
      chrome.browserAction.setIcon({tabId: b.tab.id, path: "/img/doing-icon.png"})
    ...
    }, activateMeAfterSignIn: function (a,
                                        b) {
      ...
      D.switchToTabIdAfterTwitterOauth =
        b.tab.id;
      ...
      $.ajax({
        url: "http://api.bit.ly/shorten?version=2.0.1&login=diigo&apiKey=R_051efe0ca04a325db066155db77c2d08&longUrl=" + encodeURIComponent(d), timeout: 1E4, type: "GET", complete: function (e, f) {
          if (f == "success") {
            try {
              var g = JSON.parse(e.responseText)
            } catch (h) {
              error("error parsing json from bit.ly",
                e.responseText);
              c(d);
              return
            }
      ...
      var c = a.details;
      (new CachePage({urlMD5: c.urlMD5, text: c.text, html: c.html, groups: c.groups, tabId: b.tab.id, username: GlobalData.user, ispdf: c.ispdf, pdfurl: c.pdfurl})).start()
    }, googleEvent: function (a) {
      if (window.ga) {
        a = a.msg;
        window.ga("send", "event", a.eventType, a.action, a.label)
      }
    }, pickPromotionMessage: function (a,
                                       b, c) {
      ...
      a = a.details;
      if (a.text) b(a.text); else a.html && c(a.html)
      ...
      a.url.substring(a.url.length - 4) == ".pdf" ? chrome.tabs.create({
        url: "/pdf/viewer.html?file=" +
          a.url
      ...
      var c = b.tab.id, d = b.tab, e = b.tab.url;
      (new WebAPICall).invoke("bm_loadBookmark", {url: e, what: "bookmarkInfo annotations pageComments"}, {
        user: GlobalData.user, complete: function (f) {
          Preloader.table[c].loading =
            false;
          if (!f.cancelled) if ((f = f.resp) && f.code == 1) {
      ...
      $.ajax({
        type: "POST", url: D.config.UPLOAD_SERVER + "/import_all/kindle", data: {toolbar_data: JSON.stringify(a.data)}, success: function () {
          chrome.tabs.sendMessage(b.tab.id, {name: "kindleImportSuccess", title: a.data.title})
        }, error: function (c) {
          console.log(c);
          chrome.tabs.sendMessage(b.tab.id,
            {name: "kindleImportFailed", title: a.data.title})
        }
      })
    }, importMHTML: function (a, b) {
      var c = a.details;
      chrome.tabs.sendMessage(b.tab.id, {name: "updateNoticeAndToolboar", status: "hidden"});
      chrome.pageCapture.saveAsMHTML({tabId: b.tab.id}, function (d) {
        chrome.tabs.sendMessage(b.tab.id, {name: "updateNoticeAndToolboar", status: "show"});
          ...
          e.onreadystatechange = function () {
            if (e.readyState == 4) if (e.status == 200) {
              e.responseText.replace(/[\r\n]/g,
                "");
              WebAPI.mhtml_upload_done({link_id: c.link_id, charset: c.charset, tabId: b.tab.id})
            }
          };
          try {
            e.send(d)
      ...
      a.annotation && chrome.tabs.sendMessage(b.tab.id, {name: "addAnnotation", annotation: a.annotation})
    }, editItem: function (a, b) {
      a.annotation && a.cmd && chrome.tabs.sendMessage(b.tab.id, {name: "editItem", annotation: a.annotation, cmd: a.cmd})
    }, sample: function () {
    }
  }
};
...
function sendRunCommand(a, b) {
  Messenger.send(a, {name: "run", details: extend({extensionID: D.extensionID, version: D.version, logLevel: D.logLevel}, b || {})})
}
...
function showToolbar(a, b) {
  ...
  } else getSelectedTab(function (c) {
    if (/^https?:\/\/chrome\.google\.com\/(extensions|webstore)/i.test(c.url)) alert(chrome.i18n.getMessage("alertGallery")); else if (isURLApplicable(c.url)) {
      chrome.tabs.executeScript(c.id, {file: "js/checkTab.js"}, function () {
        debug("executeScript callback")
      });
      EventMachine.onActivate();
...
function init() {
  ...
  localStorage.version && localStorage.newClickVersion != newClickVersion && chrome.browserAction.setBadgeText({text: "New"});
  if (localStorage.version != D.version) if (localStorage.version && Upgrader.isVersionBefore(localStorage.version, "3.3.30")) {
  ...
  chrome.tabs.onUpdated.addListener(function (a, b, c) {
    ...
    /https:\/\/www.diigo.com\/user\/(.*)\?from=extension/.test(c.url) && b.status == "complete" && chrome.tabs.sendMessage(a, {name: "showTutorial"});
    ...
    if (b.status == "complete" && /diigo\.(com|cn)\/images\/diigo-logo.png#SIGNED_IN/i.test(c.url)) {
      ...
      localStorage.gcc_login_send || chrome.cookies.get({
    ...
    SignInManager.checkSignInCookie(function () {
      ...
      Prefs.get("prefs.SearchO") ===
      "true" && Prefs.get("prefs.comboSearch")
    })
  });
...
var Prefs = {
  prefsToPreload: "prefs.bookmark.privateByDefault".split(" "), _observers: [], observeChanges: function (a) {
  ...
  _locationCounter: 0, _activationCounter: 0, _lastReloadTime: new Date,
  _shouldReload: false, init: function () {
  }, onTabUpdated: function (a, b, c) {
  ...
  init: function () {
    ...
    chrome.contextMenus.create({
      title: "Tag as a stand-alone image", contexts: ["image"], parentId: a, onclick: function (b, c) {
        D.saveImageToDiigo(b, c, "tag")
      }
    });
    chrome.contextMenus.create({
      title: "Attach it to the bookmark", contexts: ["image"], parentId: a, onclick: function (b, c) {
        D.saveImageToDiigo(b, c, "attach")
      }
    });
    chrome.contextMenus.create({
      title: "Save to Diigo", contexts: ["link"], onclick: function (b, c) {
        ...
        d.onreadystatechange = function () {
          if (d.readyState ==
            ...
            chrome.tabs.sendMessage(c.id, {name: "showWindow", data: {linkTitle: f, url: b.linkUrl}})
          }
        };
        d.send()
      }
    ...
    chrome.contextMenus.create({
      title: "Save Bookmark", parentId: a, onclick: function () {
        showWindow()
      }
    });
    chrome.contextMenus.create({
      title: "Annotate Article", parentId: a, onclick: function (b, c) {
        chrome.tabs.executeScript(c.id,
          {file: "js/checkTab.js"}, function () {
          });
        chrome.tabs.sendMessage(c.id, {name: "annotate"})
      }
    });
    chrome.contextMenus.create({
      title: "Annotate PDF", parentId: a, onclick: function () {
        chrome.tabs.create({url: "/pdf/viewer.html"})
      }
    });
    chrome.contextMenus.create({
      title: "Annotate Screenshot", parentId: a, onclick: function (b, c) {
        chrome.tabs.executeScript(c.id, {file: "js/checkTab.js"}, function () {
        });
    ...
    chrome.contextMenus.create({
      title: "Read Later", parentId: a, onclick: function (b,
                                                           c) {
        chrome.tabs.sendMessage(c.id, {name: "readlater"})
      }
    });
    chrome.contextMenus.create({
      title: "Sticky note", parentId: a, onclick: function () {
        showToolbar("stickynote")
      }
    });
    a = chrome.contextMenus.create({title: "Diigo", contexts: ["selection"]});
    chrome.contextMenus.create({
      title: "Highlight", contexts: ["selection"], parentId: a, onclick: function (b, c) {
        showToolbar("highlight");
        chrome.tabs.sendMessage(c.id, {name: "contextmenuHighlight"})
      }
    });
    chrome.contextMenus.create({
      title: "Search Diigo+Google for selection", parentId: a,
      contexts: ["selection"], onclick: function (b, c) {
...
D.saveImageToDiigo = function (a, b, c) {
  if (/data:image\/.*;base64/.test(a.srcUrl)) {
    ...
    chrome.tabs.sendMessage(b.id, {name: "saveImageToDiigo", data: a.srcUrl, type: c, dtype: d, image_src: a})
  } else {
    ...
    d.onload = function () {
      if (this.status == 200) {
        var e = new Blob([this.response], {type: this.response.type}), f = this.response.type.slice(6), g = new FileReader;
        g.onload = function () {
          chrome.tabs.sendMessage(b.id, {
            name: "saveImageToDiigo",
            data: g.result, type: c, dtype: f, image_src: a
          })
        };
        g.readAsDataURL(e)
      }
...
WebAPICall.prototype = {
  invoke: function (a, b, c) {
    this.callOptions = c;
    var d = c.transId || WebAPICall.transIdStepper++, e = c.user;
    d = {cmd: a, v: 13, _nocache: Math.random(), json: JSON.stringify(b), user: e, transId: d};
    ...
    if (c.isInGroup) {
      ...
      e = a == "user_signIn" ? evalTpl(b + "/chappai/pv=#{pv}/ct=tb/cv=#{cv}/cmd=#{cmd}", {pv: 13, cv: D.version, cmd: a}) : evalTpl(b + "/chappai/pv=#{pv}/ct=tb/cv=#{cv}/user=#{user}/cmd=#{cmd}/", {
        pv: 13,
        cv: D.version, user: e, cmd: a
      })
    }
    ...
    var e = this;
    e.xhr = $.ajax({
      type: "POST", url: a, data: b, complete: function (f) {
        try {
...
var WebAPICall_Static = {
  transIdStepper: 1, _baseUrl: null, setup: function (a) {
...
var Upgrader = {
  check: function () {
    var a = D.version, b = Prefs.get("version") || "0";
    this.compareVersion(a, b) > 0 && this.upgrade(b)
  }, upgrade: function (a) {
    debug("Performing upgrade from", a, "to", D.version);
    this.isVersionBefore(a, "1.6.2.4") && Prefs.set({"prefs.autoloadBookmarkStatus": true});
...
$(document).ready(init);
typeof browser == "undefined" && chrome.runtime.onInstalled.addListener(function (a) {
  if (a.reason == "install") {
    var b = chrome.runtime.getManifest();
    chrome.cookies.getAll({url: "https://www.diigo.com"}, function (c) {
      sendAnalysisData({gcc_cookie_installed_ext_chrome: 1, gcc_cookie_ext_chrome_version: b.version, gcc_cookie_ext_chrome_last_version: localStorage.version || "none", gcc_cookie_visited_diigo: c.length > 0 ? 1 : 0})
    });
    chrome.tabs.query({url: "https://www.diigo.com/common/install"}, function (c) {
      for (var d = 0, e = c.length; d <
      e; d++) {
        var f = c[d];
        chrome.cookies.get({url: "https://www.diigo.com", name: "diigoandlogincookie"}, function (g) {
          if (g) (username = g.value.split("-.-")[1]) && chrome.tabs.update(f.id, {url: "https://www.diigo.com/user/" + username + "?from=extension"})
        })
      }
    })
  }
